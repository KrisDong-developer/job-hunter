...
import { sendApplication, APPLICATION_SEND_ACTION, } from './guard/actions/application.js';
import { syncInbox, INBOX_SYNC_ACTION } from './guard/actions/inbox.js';
import { sendReply, REPLY_SEND_ACTION } from './guard/actions/reply.js';
import { probeContactStage, STAGE_PROBE_ACTION, } from './guard/actions/stage.js';
...
export function createHostRuntime(options = {}) {
     ...
     */
    const cooldownUntilOf = (platformId) => {
        const opened = store;
        if (opened === undefined)
            return null;
     ...
     */
    const crawlQuotaOf = (platformId) => {
        const opened = store;
        if (opened === undefined)
            return { used: 0, limit: DAILY_CRAWL_LIMIT };
     ...
     */
    const platformGate = (platformId, options) => {
        const opened = store;
        if (opened === undefined)
            return 'lease_lost';
    ...
    const syncDeadlineTodos = () => {
        ...
        try {
            const now = clock();
            const existing = new Map(opened.todo.listOpen({ kind: 'deadline', limit: 200 }).map((todo) => [todo.ref ?? '', todo]));
    ...
    const openDataLayer = () => {
        ...
        const indeedOverride = opened.setting.get('adapter-config', 'platform', 'indeed');
        registry.register(createIndeedAdapter({
            config: mergeIndeedConfig(indeedOverride),
            delayRangeMs: [REQUEST_DELAY_MIN_MS, REQUEST_DELAY_MAX_MS],
        }));
        logger?.info(`[${PLUGIN_ID}] 适配器 indeed 已注册（配置来源：${indeedOverride === undefined ? '代码默认' : 'DB 覆盖'}${indeedOverride === undefined ? '；⚠️ 中国大陆站已停运，默认 host 不可用' : ''}）`);
        ...
        const hiredchinaOverride = opened.setting.get('adapter-config', 'platform', 'hiredchina');
        registry.register(createHiredChinaAdapter({
            config: mergeHiredChinaConfig(hiredchinaOverride),
            delayRangeMs: [REQUEST_DELAY_MIN_MS, REQUEST_DELAY_MAX_MS],
        }));
        logger?.info(`[${PLUGIN_ID}] 适配器 hiredchina 已注册（配置来源：${hiredchinaOverride === undefined ? '代码默认' : 'DB 覆盖'}）`);
        ...
        const sinojobsOverride = opened.setting.get('adapter-config', 'platform', 'sinojobs');
        registry.register(createSinoJobsAdapter({
            config: mergeSinoJobsConfig(sinojobsOverride),
            // P5：请求之间要随机延时，别踩出规律性的节奏
            delayRangeMs: [REQUEST_DELAY_MIN_MS, REQUEST_DELAY_MAX_MS],
        }));
        logger?.info(`[${PLUGIN_ID}] 适配器 sinojobs 已注册（配置来源：${sinojobsOverride === undefined ? '代码默认' : 'DB 覆盖'}）`);
        ...
        platforms() {
            ...
            return registry.list().map((adapter) => {
                ...
                return {
                    ...
                    yield: opened === undefined
                        ? { baseline: null, samples: 0, lastFound: null, level: 'insufficient' }
