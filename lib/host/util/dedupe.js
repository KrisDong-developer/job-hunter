 ...
 */
const WEAK_COMPANY_WORDS = new Set(COMPANY_SUFFIXES.filter((word) => !LEGAL_SUFFIXES.includes(word)));
 ...
 */
export function normalizeCityForDedupe(city) {
    const first = city
        .trim()
        // 常见分隔：· ・ - — – | / 、 以及中英文逗号与空格后接区名
        .split(/[·・\-—–|/、,，\s]/)
 ...
 * 而不是把整个行业词表重新引入硬键。
 */
export function companyTierOf(left, right) {
    ...
    if (left.companyKey !== '' && left.companyKey === right.companyKey)
        return 'weak';
    return 'none';
}
/**
...
export function compareJobs(left, right, threshold = 0.9) {
    ...
    const salaryAnchored = left.salaryBucket !== UNKNOWN_SALARY_BUCKET && right.salaryBucket !== UNKNOWN_SALARY_BUCKET;
    ...
    if (companyTier === 'weak') {
        return {
            merge: false,
            basis: `公司名只差行业词/机构词（都归一到「${left.companyKey}」），城市与 ${salaryNote} 一致` +
