...
export function createScheduler(deps) {
    ...
    const tick = async () => {
        ...
        try {
            ...
            for (const plan of due) {
                ...
                const platformOutcomes = outcomes.map(toPlatformOutcome);
                const cutDecisions = cut.map((platformId) => ({
                    platformId,
                    reason: 'round_budget',
    ...
    return {
        ...
        async runPlan(planId, reason) {
            ...
            const { outcomes: laneOutcomes, cut: cutIds } = await runInLanes({
                platforms: targets,
                launch: async (platformId) => await deps.run({
                    planId,
                    platformId,
            ...
            const outcomes = laneOutcomes.map(toPlatformOutcome);
