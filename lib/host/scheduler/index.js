...
export function createScheduler(deps) {
    ...
    const isPaused = () => deps.store.setting.get(PAUSE_KEY, PAUSE_SCOPE) === true;
    const pausedReason = () => {
        const record = deps.store.setting.get(PAUSE_KEY, PAUSE_SCOPE);
        if (record === true)
            return '定时已被一键暂停';
        if (record !== null && typeof record === 'object' && typeof record.reason === 'string')
    ...
    const decide = (plan, at, exceptReason) => {
        if (!plan.enabled || !plan.schedule.enabled)
            return { kind: 'skip', reason: 'plan_disabled' };
        if (exceptReason !== 'manual' && exceptReason !== 'catch-up') {
            if (isPaused())
                return { kind: 'skip', reason: 'global_pause' };
        }
        if (!deps.canSchedule())
            return { kind: 'skip', reason: 'lease_lost' };
        const engine = deps.store.plan.engineState(plan.id);
        if (engine.riskPaused)
            return { kind: 'skip', reason: 'risk_paused' };
        ...
        for (const platformId of plan.platforms) {
            ...
            if (gateReason !== null)
                return { kind: 'skip', reason: gateReason };
        }
    ...
    const finishPlanRun = (plan, summary, error) => {
        const at = clock();
        ...
        const riskSignal = code === 'BLOCKED' || code === 'NOT_LOGGED_IN' || code === 'RISK' || code === 'RATE_LIMITED';
        ...
        if (pause) {
            ...
            deps.logger?.warn(`[scheduler] ${plan.name} 失败（连续 ${String(failStreak)} 次）→ 退避到 ` +
