...
export function createScheduler(deps) {
     ...
     */
    const recordPlatformFailure = (platformId, failStreak) => {
    ...
    const finishPlanRun = (plan, summary, error) => {
        ...
        // SR-22：风控信号（验证码/登录墙/限流/平台配额耗尽）**单独识别**，直接风控暂停，不等凑够次数。
        // PLATFORM_QUOTA（quota-exhausted）：平台侧今日额度用完，退避重试无意义 → 也走风控暂停，
        // 人工第二天确认恢复。
    ...
    return {
        ...
        resumeRisk(planId) {
            ...
            for (const platformId of deps.store.plan.get(planId)?.platforms ?? [])
                clearPlatformCooldown(platformId);
