...
import { formatSseFrame, SSE_HEARTBEAT_FRAME, SSE_HEARTBEAT_MS } from './http/sse.js';
const JSON_HEADERS = {
 ...
 */
function writeBytes(res, result) {
    const headers = {
 ...
 */
export function serveEventStream(req, res, bus) {
    ...
    const parsedLastId = typeof rawLastId === 'string' && rawLastId !== '' ? Number.parseInt(rawLastId, 10) : Number.NaN;
 ...
 */
export function registerHttpRoutes(ctx, runtime) {
