...
import { toAccountDto } from '../platform/session.js';
 ...
 */
function unknownAccountOf(platformId) {
    return {
        platformId,
        loggedIn: false,
 ...
 */
export function adapterSnapshotOf(store, platformId) {
    if (store === undefined)
        return { health: 'healthy', failStreak: 0, lastOkAt: null, reason: null, fields: [] };
    return readAdapterHealth(store, platformId);
}
/** 全部注册适配器的健康快照（`/health` 与 `/crawl/status` 共用同一份）。 */
export function adapterHealthDtos(store, registry) {
...
export function buildHealth(deps) {
...
export function buildCrawlStatus(deps) {
...
export function buildPlatforms(deps) {
    const opened = deps.store;
...
export function buildLoginStatuses(deps) {
    const session = deps.sessionOf();
    const loginFlow = deps.loginFlowOf();
    return deps.registry.list().map((adapter) => {
 ...
 * 比空态更难查。
 */
export function unavailableSchedulerStatus(readOnlyReason, lease) {
    return {
        scheduling: false,
        readOnly: true,
        readOnlyReason,
