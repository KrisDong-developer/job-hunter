...
import { DEFAULT_FIFTYONE_CONFIG, mergeFiftyOneConfig, } from '../platform/adapters/fiftyone-job/config.js';
...
import { DEFAULT_HIREDCHINA_CONFIG, mergeHiredChinaConfig, } from '../platform/adapters/hiredchina/config.js';
...
export const ADAPTER_SPECS = [
    ...
    {
        id: 'liepin',
        build: (override, delayRangeMs, logger) => createLiepinAdapter({
    ...
    {
        id: 'zhipin',
        build: (override, delayRangeMs, logger) => createZhipinAdapter({
    ...
    // Indeed（cn.indeed.com）：2026-09-21 `probe:indeed-login` 两跑实证 cn 站**未登录可搜**
    // （旧「停运」结论过时）。Cloudflare 风控在（cf_clearance 过期可能再撞验证墙，
    // 判墙即停），首访可能被送 secure.indeed.com/auth 登录墙。列表补充字段走同页
    // 内嵌 mosaic-provider-jobcards 载荷（发布日期真源，零额外请求）。
    {
        id: 'indeed',
        ...
        noOverrideNote: 'cn 站实测可用，但 Cloudflare 风控在（撞验证墙属正常，判墙即停）',
    },
    ...
    {
        id: 'linkedin',
        build: (override, delayRangeMs, logger) => createLinkedInAdapter({
