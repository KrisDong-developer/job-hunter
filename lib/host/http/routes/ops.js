...
export async function systemReveal(ctx) {
...
export async function llmCalls(ctx) {
 ...
 */
export async function guardUsage(ctx) {
    const { runtime, req, segments, method } = ctx;
    ...
    return json(200, runtime.guardUsage(platformId === null || platformId === '' ? undefined : platformId));
}
/** 原 router.ts 的 `GET /settings`。 */
export async function settingsGet(ctx) {
    const { runtime, segments, method } = ctx;
    // ── 插件配置（P5）─────────────────────────────────────────────────
...
export async function settingsPatch(ctx) {
    const { runtime, req, segments, method } = ctx;
...
function settingsPatchOf(body) {
    ...
    if (typeof retention === 'object' && retention !== null) {
        ...
        const days = (key) => typeof source[key] === 'number' ? source[key] : undefined;
        const next = {};
        for (const key of [
            'crawlRunsDays',
            'auditLogDays',
        ...
        if (typeof source['autoCleanEnabled'] === 'boolean')
            next['autoCleanEnabled'] = source['autoCleanEnabled'];
