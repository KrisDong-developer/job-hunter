...
export async function groups(ctx) {
    ...
    if (!(method === 'GET' && segments.length === 2 && segments[0] === 'dedup' && segments[1] === 'groups'))
        return undefined;
    requireData(runtime);
...
export async function sweep(ctx) {
    ...
    if (!(method === 'POST' && segments.length === 2 && segments[0] === 'dedup' && segments[1] === 'run'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    if (store === undefined)
...
export async function group(ctx) {
    const { runtime, segments, method } = ctx;
    if (!(method === 'GET' && segments.length === 3 && segments[0] === 'dedup' && segments[1] === 'groups'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    if (store === undefined)
...
export async function dropGroup(ctx) {
    const { runtime, segments, method } = ctx;
...
export async function split(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'POST' && segments.length === 4 && segments[0] === 'dedup' && segments[1] === 'groups' && segments[3] === 'split'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    if (store === undefined)
