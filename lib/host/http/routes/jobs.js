...
function buildJobQuery(query) {
    ...
    if (orderByRaw !== null && orderByRaw !== '' && !JOB_ORDER_VALUES.includes(orderByRaw)) {
...
export async function views(ctx) {
    const { runtime, req, segments, method } = ctx;
    ...
    if (jobService === undefined)
        throw dataNotReady(runtime);
    if (method === 'GET')
        return json(200, jobService.jobViews());
    if (method === 'PUT') {
...
export async function exportList(ctx) {
    const { runtime, req, segments, method } = ctx;
...
export async function read(ctx) {
    ...
    if (jobService === undefined)
        throw dataNotReady(runtime);
    const updated = jobService.markRead(id, new Date().toISOString());
