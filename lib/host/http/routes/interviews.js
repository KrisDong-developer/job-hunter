...
export async function questionsList(ctx) {
    const { runtime, req, segments, method } = ctx;
 ...
 */
export async function questionsAdd(ctx) {
    const { runtime, req, segments, method } = ctx;
...
export async function questionsPatch(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'PATCH' && segments.length === 2 && segments[0] === 'questions'))
        return undefined;
    requireData(runtime);
...
/** `DELETE /questions/:id`。 */
export async function questionsRemove(ctx) {
    const { runtime, segments, method } = ctx;
    if (!(method === 'DELETE' && segments.length === 2 && segments[0] === 'questions'))
        return undefined;
    requireData(runtime);
    const id = parseRecordId(segments[1] ?? null, '错题');
