...
function writeInputOf(body) {
    ...
    if (typeof body['jobId'] === 'number')
        ...
        input.jobId = null;
    if (typeof body['applicationId'] === 'number')
        input.applicationId = body['applicationId'];
    else if (body['applicationId'] === null)
        input.applicationId = null;
    if (typeof body['role'] === 'string')
        input.role = body['role'];
    if (body['comp'] !== undefined)
        input.comp = body['comp'];
    ...
    if (typeof body['state'] === 'string')
        input.state = parseState(body['state']);
    if (typeof body['note'] === 'string')
        input.note = body['note'].trim() === '' ? null : body['note'];
    else if (body['note'] === null)
        input.note = null;
    return input;
}
function parseState(raw) {
    if (!OFFER_STATES.includes(raw)) {
...
export async function list(ctx) {
    ...
    if (!(method === 'GET' && segments.length === 1 && segments[0] === 'offers'))
        return undefined;
    requireData(runtime);
...
export async function create(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'POST' && segments.length === 1 && segments[0] === 'offers'))
        return undefined;
    requireData(runtime);
...
export async function get(ctx) {
    ...
    if (!(method === 'GET' && segments.length === 2 && segments[0] === 'offers'))
        return undefined;
    requireData(runtime);
...
export async function patch(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'PATCH' && segments.length === 2 && segments[0] === 'offers'))
        return undefined;
    requireData(runtime);
...
export async function remove(ctx) {
    const { runtime, segments, method } = ctx;
    if (!(method === 'DELETE' && segments.length === 2 && segments[0] === 'offers'))
        return undefined;
    requireData(runtime);
    const id = parseRecordId(segments[1] ?? null, 'Offer');
    if (!runtime.offers().remove(id))
...
export async function byJob(ctx) {
    const { runtime, segments, method } = ctx;
