 ...
 */
export async function greetingBatch(ctx) {
    const { runtime, req, segments, method } = ctx;
    // ── 预览：只读 ────────────────────────────────────────────────────
    if (segments.length === 3 && segments[0] === 'greeting' && segments[1] === 'send-batch' && segments[2] === 'preview') {
        if (method !== 'POST')
            throw new DomainError('INVALID_INPUT', '批量预览只支持 POST');
        requireData(runtime);
        const body = await readObject(req);
        const jobIds = Array.isArray(body['jobIds'])
            ? body['jobIds'].filter((item) => typeof item === 'number' && item > 0)
            : [];
        if (jobIds.length === 0) {
    ...
    if (segments.length === 2 && segments[0] === 'greeting' && segments[1] === 'send-batch') {
        if (method !== 'POST')
            throw new DomainError('INVALID_INPUT', '批量发送只支持 POST');
        requireData(runtime);
        const body = await readObject(req);
        if (body['confirm'] !== true) {
            throw new DomainError('INVALID_INPUT', '批量发送需要显式确认', {
                hint: '先调 POST /greeting/send-batch/preview，把逐条话术全文给用户看过，' +
 ...
 */
export async function contactStageUpdate(ctx) {
    const { runtime, req, segments, method } = ctx;
    ...
    const body = await readObject(req);
    const to = body['to'];
    if (typeof to !== 'string' || !MANUAL_CONTACT_STAGES.includes(to)) {
        throw new DomainError('INVALID_INPUT', 'to 必须是可手工标记的接触态', {
            hint: `合法取值：${MANUAL_CONTACT_STAGES.join(' / ')}。` +
 ...
 */
export async function greetings(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'GET' && segments.length === 1 && segments[0] === 'greetings'))
        return undefined;
    requireData(runtime);
    const jobId = Number.parseInt(req.query.get('jobId') ?? '', 10);
    const stageRaw = req.query.get('stage');
    if (stageRaw !== null && stageRaw !== '' && !CONTACT_STAGES.includes(stageRaw)) {
    ...
    return json(200, {
        items: runtime.pipeline().listGreetings({
            ...
            ...(stageRaw === null || stageRaw === '' ? {} : { stage: stageRaw }),
