import { COMPANY_ORDER_VALUES } from '../../../shared/contract/enums/job.js';
 ...
 */
function toProfileDto(company, profile) {
    return {
        id: company.id,
        name: company.name,
...
function toEnrichmentDto(record) {
    return {
...
export async function enrich(ctx) {
    ...
    if (store === undefined || browser === undefined)
    ...
    if (pickUrl !== undefined && !/^\/company\/\d+/.test(pickUrl) && !pickUrl.startsWith('https://www.tianyancha.com/company/')) {
    ...
    }, companyId, pickUrl === undefined ? undefined : { url: pickUrl, name: pickName });
...
export async function list(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'GET' && segments.length === 1 && segments[0] === 'companies'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    ...
    const orderBy = orderByRaw !== null && COMPANY_ORDER_VALUES.includes(orderByRaw)
        ? orderByRaw
        : undefined;
...
export async function review(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'PATCH' && segments.length === 2 && segments[0] === 'companies'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    const jobService = runtime.jobs();
...
export async function detail(ctx) {
    ...
    if (!(method === 'GET' && segments.length === 2 && segments[0] === 'companies'))
        return undefined;
    requireData(runtime);
    const store = runtime.store();
    const jobService = runtime.jobs();
