...
import { GREETING_TONES } from '../../../shared/contract/enums/pipeline.js';
...
function resumeIdOfSegment(segments) {
    return parseRecordId(segments[1] ?? null, '简历');
}
export async function greetingTemplatesList(ctx) {
    const { runtime, segments, method } = ctx;
...
export async function greetingTemplateGenerate(ctx) {
    const { runtime, req, segments, method } = ctx;
    if (!(method === 'POST' &&
        segments.length === 4 &&
        segments[0] === 'resumes' &&
    ...
    if (toneRaw !== undefined && !GREETING_TONES.includes(toneRaw)) {
...
export async function greetingTemplateSave(ctx) {
    const { runtime, req, segments, method } = ctx;
...
export async function greetingTemplateRemove(ctx) {
    const { runtime, segments, method } = ctx;
    if (!(method === 'DELETE' &&
        segments.length === 4 &&
        segments[0] === 'resumes' &&
