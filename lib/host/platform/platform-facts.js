...
export const PLATFORM_FACTS = {
    ...
    zhaopin: {
        ...
        applicationSideEffect: '智联会在投递的同时**替你发一句招呼语**（内容由平台生成，实测形如「您好，请问<岗位>职位还在招人吗？…」）。' +
            '这句不是你写的，但对方会收到。',
    ...
    liepin: {
        ...
        greetingSideEffect: '猎聘会在会话里插入一条**「打招呼语建议」系统卡片**（文案「我们为您生成了合适的打招呼语，去使用＞」），' +
    ...
    zhipin: {
        ...
        greetingSideEffect: 'BOSS 会先替你发一句**平台默认招呼语**（内容由平台生成），然后才发你这段话术 —— ' +
            '对方会连着看到两条开场消息。',
        authRequirement: { crawl: 'none', detail: 'required', actions: 'required' },
    },
    waiqi: {
        maturity: {
            ...
            notes: '真实响应夹具（页面 + 载荷 + 详情）。服务端翻页坏 → 声明 1 页（≤50 条/页）是平台事实，不是保守取舍。' +
    ...
    indeed: {
        maturity: {
            level: 'calibrated',
            verifiedAt: '2026-09-21',
            notes: '2026-09-21 `probe:indeed-login` 两跑实证（产物 `.probe-indeed-capture/`）：cn.indeed.com 搜索**未登录即可用**（广州市 16 条，jobKey/标题/公司/城市全中）—— 2026-09-18「已停运」结论过时。' +
    ...
    hiredchina: {
        maturity: {
            level: 'calibrated',
            verifiedAt: '2026-09-20',
            notes: '真实页面夹具（列表 p1/p2 + 详情，来自 hcweb 同源子域）。列表卡片不在 DOM 里（数据在 RSC 流 initialData.list，卡片是客户端渲染）→ 解析走 payload 通道；薪资约四成 keep.secret（→ Negotiable）故 salary_raw 不进必需字段；详情页 SSR 直出、选择器已全套校准。翻页满页判据（payload 无分页元信息）。',
        },
        authRequirement: { crawl: U, detail: U, actions: 'required' },
    },
    linkedin: {
        maturity: {
            level: 'calibrated',
            verifiedAt: '2026-09-20',
            notes: '2026-09-21 按公开稳定结构实现；**两轮真机校准**：`probe:linkedin-login`（登录侧）+ ' +
