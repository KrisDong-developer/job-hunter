    ...
    try {
        ...
        if (arg.selectors.chatInput !== '') {
            for (const input of Array.from(document.querySelectorAll(arg.selectors.chatInput))) {
                if (visible(input))
                    return { kind: 'chat-input', hrName: '', tip: '' };
            }
        }
    }
    catch {
...
export function applyDialogStateInPage(arg) {
    ...
    return {
        ...
        confirmDisabled: confirm !== null && (className.split(/\s+/).includes('disabled') || confirm.hasAttribute('disabled')),
...
export function readChatMessagesInPage(arg) {
    ...
    for (const node of nodes) {
        ...
        states.push(lower.includes('error') ? 'failed' : lower.includes('loading') || lower.includes('sending') ? 'pending' : 'delivered');
    }
    if (states.length === 0)
        return { state: 'missing' };
