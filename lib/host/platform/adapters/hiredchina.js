...
export function detectBlockInPage(arg) {
    ...
    const challenge = document.querySelector('script[src*="challenge-platform"], form[action*="cf_chl"], iframe[src*="challenge-platform"], [class*="cf-browser-verification"]');
    if (challenge !== null)
        return 'captcha';
    if (/justamoment|_cf_chl_opt|cf-browser-verification|cf_chl_/.test(compact))
        return 'captcha';
    const captcha = document.querySelector('.geetest_panel, .geetest_holder, iframe[src*="captcha"], #captcha, [class*="verify-wrap"], [class*="slide-verify"]');
    if (captcha !== null)
        return 'captcha';
    ...
    if (cards === 0 && /login|Sign in|登录/.test(compact) && compact.length < 800)
        return 'login-required';
    if (cards === 0 && compact.length < 120)
        return 'blank';
    return null;
}
/** 构造 HiredChina 适配器。 */
