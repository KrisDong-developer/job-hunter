...
import { countCardsInPage, detectBlockInPage, extractDetailInPage, extractJobsInPage, fetchDetailInPage, fetchListInPage, hasNextPageInPage, isLoggedInInPage, } from './page.js';
/** 构造神仙外企适配器。 */
export function createWaiqiAdapter(options = {}) {
    ...
    const dimensions = [
        ...
        },
        {
            ...
            hint: '对应接口 businessCategoryIdList（**2026-09-21 探针全量实测**：30 项，去掉"不限"）。' +
        ...
        },
        {
            ...
            multi: true,
            hint: '对应接口 companyTypeList（数组，可多选）：美企 / 德企 / 中德合资 … ' +
            ...
            ? {
                detail: {
                    async extract(page) {
                        ...
                        if (id === '') {
                            throw new Error(`神仙外企：当前页地址里没有岗位 id（${page.url()}）—— 无法构造详情接口请求`);
                        }
                        ...
                        const block = request.code === 1022 ? 'login-required' : request.code === 429 ? 'rate-limited' : null;
                        if (block !== null) {
                            throw new PlatformBlockedError(block, `详情接口 code=${String(request.code)}：${request.message === '' ? '（无说明）' : request.message}`);
                        }
                        if (!request.ok) {
                            throw new Error(`神仙外企：详情接口未返回可用数据（code=${request.code === null ? '?' : String(request.code)}，status=${String(request.status)}，message=${request.message === '' ? '（空）' : request.message}）`);
                        }
