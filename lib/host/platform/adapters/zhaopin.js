...
export function detectBlockInPage(arg) {
    ...
    if (cards === 0) {
        ...
        // 也有一两百字筛选器文案，120 会把那种页面误判成空白）
        if (compact.length < arg.signals.blankTextLength)
            return 'blank';
    }
    return null;
}
 ...
 */
export function applyEntryStateInPage(arg) {
    const clean = (value) => value === null || value === undefined ? '' : value.replace(/\s+/g, ' ').trim();
    let holder = null;
    try {
