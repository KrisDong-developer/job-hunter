...
import { DEFAULT_ZHAOPIN_CONFIG, ZHAOPIN_BLOCK_SIGNALS, ZHAOPIN_COMPANY_SIZE_OPTIONS, ZHAOPIN_COMPANY_TYPE_OPTIONS, ZHAOPIN_DEFAULT_MAX_PAGES, ZHAOPIN_EDUCATION_OPTIONS, ZHAOPIN_FINANCING_OPTIONS, ZHAOPIN_JOB_STATUS_OPTIONS, ZHAOPIN_MAX_PAGES, ZHAOPIN_POSTED_WITHIN_OPTIONS, ZHAOPIN_SALARY_OPTIONS, ZHAOPIN_SORT_OPTIONS, ZHAOPIN_WORK_EXPERIENCE_OPTIONS, } from './config.js';
import { detectBlockInPage } from './page/block.js';
import { extractJobDetailInPage } from './page/detail.js';
import { currentPageInPage, extractJobsInPage, isLoggedInInPage, nextPageUrlInPage, totalPagesInPage, } from './page/list.js';
...
export function createZhaopinAdapter(options = {}) {
     ...
     */
    const detectBlockOf = async (page) => await page.evaluate(detectBlockInPage, {
    ...
    const dimensions = [
        ...
        },
        {
            ...
            hint: '默认就是「最新发布」（order=4，不配置即生效）；页面另有「智能匹配 / 薪酬最高」两项，参数值无实测证据，故不提供',
        },
        /**
        ...
        // ── 2026-09-23 追加的三个维度：参数名来自真机全选后的地址栏
        // （`sl=0000%2C4000&fs=7&cs=1`），取值域照抄 base/data 字典；`sl` 的生效性
        // 单独实测过（`sl=0000,4000` 把深圳 java 从满页 20 条砍到 10 条）。
        // 键名与 BOSS 直聘的概念键对齐（salary/scale/stage）：同一概念跨平台同名，
        // 界面顺序与兜底标签共用一份；取值域是各平台自己的码（方案本就单平台）。
        {
            key: 'salary',
            label: '薪资',
            values: ZHAOPIN_SALARY_OPTIONS,
            wire: { target: 'url', param: config.urlParams.salaryParam },
            hint: '值是站点的月薪区间码（如 10001,15000 = 10K-15K），URL 上站点自己把逗号编成 %2C。需登录',
        },
        {
            key: 'stage',
            label: '融资阶段',
            values: ZHAOPIN_FINANCING_OPTIONS,
            wire: { target: 'url', param: config.urlParams.financingParam },
            hint: '实测 fs=7（已上市）；「有融资」在站点字典里是分号拼的多码，编码未实测，不提供。需登录',
        },
        {
            key: 'scale',
            label: '公司规模',
            values: ZHAOPIN_COMPANY_SIZE_OPTIONS,
            wire: { target: 'url', param: config.urlParams.companySizeParam },
            hint: '实测 cs=1（20人以下）。需登录',
        },
        {
