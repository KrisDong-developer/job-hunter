 ...
 */
export function createZhaopinActions(ctx) {
     ...
     */
    const fetchTalkRows = async (page) => {
        const collected = [];
        const seen = new Set();
        for (let pageNo = 1; pageNo <= config.talkListMaxPages; pageNo += 1) {
            ...
            if (!result.ok) {
                throw new Error(`智联会话列表接口没调通（第 ${String(pageNo)} 页）：${result.message}` +
            ...
            if (result.code !== 200) {
                ...
                if (block !== null) {
                    throw new PlatformBlockedError(block, `会话列表接口返回 code=${String(result.code)}：${result.message}`);
                }
                throw new Error(`智联会话列表接口返回 code=${String(result.code)}（第 ${String(pageNo)} 页）：${result.message}`);
            }
            ...
            if (rows.length < config.talkListPageSize)
                break;
        }
        return collected;
    };
    ...
    const readInbox = async (page) => {
        ...
        const waitTarget = config.imSelectors.listContainer !== ''
    ...
    const detectStage = async (page, job) => {
        ...
        try {
            ...
            if (error instanceof PlatformBlockedError)
                throw error;
        ...
        const row = (jobIdFromUrl === ''
            ? undefined
            : rows.find((item) => item.jobNumber === jobIdFromUrl)) ??
            rows.find((item) => (company !== '' && item.companyName.trim() === company) ||
    ...
    const sendResume = async (page, job, filePath) => {
        if (page.mouse === undefined) {
            return {
                ...
                delivery: 'missing',
                evidence: 'none',
                message: '当前页面没有 CDP 鼠标能力 —— 拒绝用 DOM 事件冒充真人点击' +
        ...
        if (filePath !== null) {
            return {
                ...
                delivery: 'missing',
                evidence: 'none',
                message: '智联的投递只用**平台内简历**：`application/preparation` 返回的是平台自己的简历列表' +
        ...
        const onLoginPage = () => /^https?:\/\/passport\.zhaopin\.com\/login/i.test(page.url());
        if (onLoginPage()) {
            return {
                ok: false,
                delivery: 'missing',
        ...
        if (!entry.found) {
            return {
                ...
                delivery: 'missing',
                evidence: 'none',
                message: `详情页上找不到投递入口（${config.applySelectors.entry}）—— 可能未登录、或页面版式已变。` +
                    '**没有点任何东西。**',
            };
        }
        if (entry.text !== ZHAOPIN_APPLY_ENTRY_TEXT) {
            return {
                ...
                delivery: 'missing',
                evidence: 'none',
                message: `投递入口的文案是「${entry.text}」，不是「${ZHAOPIN_APPLY_ENTRY_TEXT}」—— 通常意味着` +
        ...
        if (config.dwellBeforeApplyMs[1] > 0) {
            await page.waitForTimeout(dwellBeforeActMs({
        ...
        if (onLoginPage()) {
            return {
                ...
                delivery: 'missing',
                evidence: 'dom',
                message: `点了「${ZHAOPIN_APPLY_ENTRY_TEXT}」之后被弹到登录页（${page.url().slice(0, 80)}）—— ` +
