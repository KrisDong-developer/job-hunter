...
export function extractDetailInPage(arg) {
    ...
    const sectionMatched = (node) => {
        if (arg.selectors.jdSectionTitles.length === 0)
            return false;
        let heading = '';
        try {
    ...
    const jdTextOf = () => {
        ...
        const chosen = matched.length > 0 ? matched : [sections[0]];
        return chosen
    ...
    const notes = [];
    if (title === '')
        notes.push('详情标题未锚定（h1 待校准）');
    if (salaryRaw === '')
        notes.push('详情薪资未锚定（items-start/shrink-0 金额元素待校准）');
    if (company === '')
        notes.push('详情公司名未锚定（渐变卡 p.font-medium 待校准）');
    if (jdText === '')
        notes.push('JD 未锚定（div.prose.prose-sm 待校准）');
    return {
        platformJobId: '',
        title,
        salaryRaw,
