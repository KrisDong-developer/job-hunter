...
import { LIEPIN_BODY_FIELDS } from './config.js';
/**
 ...
 */
export function buildContactListBody(arg) {
    const params = new URLSearchParams();
 ...
 */
export function liepinFiltersOf(criteria) {
    const filters = {};
    for (const [key, field] of Object.entries(LIEPIN_BODY_FIELDS)) {
        // `postedWithinDays` 是类型化数值槽（criteriaToSearchCriteria 转成 number），
        // 其余七个走 `platform` 命名空间 —— 与 zhipin 的 `zhipinFiltersOf` 同一读法。
        const value = key === 'postedWithinDays'
            ? criteria.postedWithinDays !== undefined && criteria.postedWithinDays > 0
                ? String(criteria.postedWithinDays)
                : ''
            : platformCriterion(criteria, key);
        if (value !== '')
            filters[field] = value;
    }
    return filters;
...
export function buildSearchRequestBody(criteria, cityCode) {
    const page = criteria.page === undefined ? 1 : criteria.page;
    const filters = liepinFiltersOf(criteria);
    const form = {
        city: cityCode,
        dq: cityCode,
        pubTime: '',
