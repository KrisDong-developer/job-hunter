...
export function detectBlockInPage(arg) {
    ...
    if (/lagou\.com\/s\/list_/.test(location.href))
        return 'captcha';
    ...
    const captcha = document.querySelector('.geetest_panel, .geetest_holder, .geetest_box, #nc_1_wrapper, iframe[src*="captcha"], [class*="verify-wrap"], [class*="slide-verify"], .slide-verify-panel');
    if (captcha !== null)
        return 'captcha';
    // 拉勾 WAF 滑块文案（与 URL 特征双保险）。
    if (/请滑动滑块进行验证|为了更好的访问体验|请完成验证|滑动滑块/.test(compact))
        return 'captcha';
    ...
    if (cards === 0 && /手机号登录|邮箱登录|扫码登录/.test(compact) && compact.length < 800)
