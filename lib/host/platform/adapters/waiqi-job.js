...
export function buildWaiqiRequestBody(criteria, cityCodes, page, size = WAIQI_MAX_PAGE_SIZE) {
    ...
    if (businessCategory !== '')
        body['businessCategoryIdList'] = businessCategory;
...
export function detectBlockInPage(arg) {
    ...
    const captcha = document.querySelector('.geetest_panel, .geetest_holder, iframe[src*="captcha"], #captcha, [class*="verify-wrap"], .waf-nc-title, script[name^="aliyunwaf_"]');
    if (captcha !== null)
        return 'captcha';
    if (/访问过于频繁|操作频繁|请稍后再试|访问受限|请求异常|系统繁忙/.test(compact))
        return 'rate-limited';
    ...
    if (arg.cardCount === 0 && compact.length < 80)
        return 'blank';
    return null;
}
...
//# sourceMappingURL=waiqi-job.js.map