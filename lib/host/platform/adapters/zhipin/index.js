...
import { DEFAULT_ZHIPIN_CONFIG, ZHIPIN_MAX_PAGES, ZHIPIN_MAX_SCROLL_ROUNDS, ZHIPIN_PAGE_SIZE, } from './config.js';
...
export function createZhipinAdapter(options = {}) {
     ...
     */
    const fillSalariesFromApi = async (page, jobs) => {
        ...
        if (missing.length === 0)
            return jobs;
        const pages = Math.min(config.joblistMaxPages, Math.max(1, Math.ceil(missing.length / config.joblistPageSize)));
        ...
        for (let index = 1; index <= pages; index += 1) {
            ...
            if (got.size === 0) {
                ...
                logger?.warn(`[zhipin] 薪资接口没能取到数据（第 ${String(index)} 页，` +
                    (payload === null ? '请求失败 / 未登录' : `code=${String(code)}`) +
                    '）—— 保持 DOM 结果，这一批薪资留空');
                break;
            }
            for (const [id, salary] of got)
                salaries.set(id, salary);
            if (missing.every((job) => salaries.has(job.platformJobId)))
                break;
        }
        if (salaries.size === 0)
            return jobs;
        return jobs.map((job) => {
            ...
            const filled = { ...job, salaryRaw: salary };
            if (notes.length === 0)
                delete filled.notes;
            else
                filled.notes = notes;
