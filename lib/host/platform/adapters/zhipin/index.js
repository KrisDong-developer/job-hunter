...
import { DEFAULT_ZHIPIN_CONFIG, ZHIPIN_DEFAULT_SCROLL_ROUNDS, ZHIPIN_MAX_PAGES, ZHIPIN_MAX_SCROLL_ROUNDS, ZHIPIN_PAGE_SIZE, } from './config.js';
...
export function createZhipinAdapter(options = {}) {
    ...
    const enrichFromApi = async (page, jobs) => {
        ...
        if (missing.length === 0)
            return jobs;
        const pages = Math.min(Math.max(config.joblistMaxPages, remembered.rounds), Math.max(1, Math.ceil(missing.length / config.joblistPageSize)));
        ...
        for (let index = 1; index <= pages; index += 1) {
            ...
            if (missing.every((job) => extrasById.has(job.platformJobId)))
                break;
        }
