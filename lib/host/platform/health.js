...
import { isFieldMiss } from './validate.js';
/** 读某个平台的健康快照（给 /health 与诊断用）。 */
export function readAdapterHealth(store, platformId) {
    const platform = store.platform.get(platformId);
    const fields = store.fieldHealth.list(platformId).map((record) => ({
        field: record.field,
 ...
 */
export function applyFieldPresence(store, platformId, presence, options) {
 ...
 */
export function recordRunFailure(store, platformId, errorCode, message, options) {
 ...
 * 后者是人的断言，语义不同，不该互相冒充。
 */
export function recordRunSuccess(store, platformId, now) {
