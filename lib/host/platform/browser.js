 ...
 */
export function normalizeWaitForSelector(page) {
    // 先把**原始**方法读出来，按 Playwright 的真实签名（第二参数是配置对象）拿；
    // 写回时则走 `page.waitForSelector`（`PageLike` 声明的数字签名）——
    // 一读一写用两个类型，是因为这两个签名在参数位置上是逆变的，互相不可赋值。
    const original = page.waitForSelector;
    if (typeof original !== 'function')
        return page;
    page.waitForSelector = async (selector, timeoutMs) => {
        try {
 ...
 */
export function wireResponseSubscription(page) {
