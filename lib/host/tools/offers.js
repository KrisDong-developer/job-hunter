...
const COMP_TEXT_KEYS = ['equityNote', 'penaltyNote', 'overtimeNote'];
export function offersTools(runtime) {
    const tool = toolDefiner(runtime);
    return [
        tool({
            name: 'offer_manage',
            description: 'Offer（拿到手之后的报价）：登记 / 改条件 / 改状态 / 并排对比 / 删除。' +
            ...
            async run(args) {
                ...
                if (action === 'get') {
                    ...
                    if (filled === 0)
                        lines.push('· （还什么都没填）');
                    if (offer.note !== null)
                        lines.push(`备注：${offer.note}`);
                    return { text: lines.join('\n') };
                }
                if (action === 'compare') {
                    const rawIds = Array.isArray(args['offerIds']) ? args['offerIds'] : [];
                    const offerIds = rawIds.filter((value) => typeof value === 'number' && value > 0);
                ...
                if (action === 'remove') {
                    ...
                    return await runtime.guard().run({ action: 'offer.remove', actor: 'model', danger: 'mid', payload: { offerId: id } }, async () => {
                ...
                if (action === 'state') {
                    ...
                    if (state === undefined)
                        throw new DomainError('INVALID_INPUT', 'state 需要 offerId 与 state');
                    return await runtime.guard().run({ action: 'offer.state', actor: 'model', danger: 'mid', payload: { offerId: id, state } }, async () => {
                        const offer = service.setState(id, state);
                        return {
                            text: `offer #${String(offer.id)}（${offer.companyName}）状态改为「${offer.state}」。` +
                ...
                const identity = {
                    ...
                    ...(asString(args['companyName']) === undefined ? {} : { companyName: asString(args['companyName']) }),
                    ...(typeof args['jobId'] === 'number' ? { jobId: args['jobId'] } : {}),
                    ...(asString(args['role']) === undefined ? {} : { role: asString(args['role']) }),
                    ...(asString(args['deadline']) === undefined ? {} : { deadline: asString(args['deadline']) }),
                    ...(asString(args['note']) === undefined ? {} : { note: asString(args['note']) }),
                    ...(asString(args['state']) === undefined ? {} : { state: asString(args['state']) }),
                };
                if (action === 'create') {
                    return await runtime.guard().run({
                        ...
                        return {
                            text: `已登记 offer #${String(offer.id)}：${offer.companyName}｜${offer.role || '（没填岗位）'}` +
                ...
                if (action === 'update') {
                    const id = positiveId(args['offerId'], 'offerId');
                    return await runtime.guard().run({ action: 'offer.update', actor: 'model', danger: 'mid', payload: { offerId: id, compKeys: Object.keys(flat).length } }, async () => {
                        ...
                        return {
                            text: `已更新 offer #${String(offer.id)}（${offer.companyName}）` +
                                `${offer.annualCash === null ? '' : `，年现金总包 ${formatMoney(offer.annualCash)}`}。`,
                        };
                    });
                }
...
function compOf(args) {
    ...
    if (typeof args['bonusInContract'] === 'boolean')
        target['bonusInContract'] = args['bonusInContract'];
    return comp;
}
