...
export function outreachTools(runtime) {
    ...
    return [
        ...
        tool({
            ...
            async run(args) {
                ...
                const jobIds = (Array.isArray(args['jobIds']) ? args['jobIds'] : []).filter((item) => typeof item === 'number' && item > 0);
                // ── 批量：逐条过闸门、逐条回执 ──────────────────────────────
                if (jobIds !== undefined && jobIds.length > 0) {
                    ...
                    const lines = [
                        ...
                        ...result.receipts.map((receipt, index) => {
                            ...
                            if (receipt.ok)
                                return `${String(index + 1)}. ✅ ${who}：已发送，${String(receipt.textLength ?? 0)} 字`;
        ...
        tool({
            ...
            parameters: schema({
                jobId: int('岗位 id'),
                stage: enumStr([...MANUAL_CONTACT_STAGES], '要标记的接触态（不传则只探测）。"未接触/无记录"不是能标出来的状态。'),
            }, ['jobId']),
            ...textResult,
            async run(args) {
                requireData(runtime);
                ...
                if (stage !== undefined) {
                    if (!MANUAL_CONTACT_STAGES.includes(stage)) {
                    ...
                    return {
                        text: `岗位 #${String(jobId)} 的接触态：${CONTACT_STAGE_LABEL[from]} → **${CONTACT_STAGE_LABEL[greeting.stage]}**。\n` +
