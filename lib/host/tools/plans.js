        ...
        tool({
            ...
            parameters: schema({
                ...
                platforms: {
                    type: 'array',
                    items: { type: 'string' },
                    description: '目标平台，**只填一个 id**（一个方案只抓一个平台：筛选条件是按平台自己的取值域与参数名定义的，' +
            ...
            async run(args) {
                ...
                switch (action) {
                    ...
                    case 'create': {
                         ...
                         */
                        const targets = patch['platforms'] ?? [];
                        if (targets.length !== 1) {
                            throw new DomainError('INVALID_INPUT', 'create 的 platforms 要**恰好一个**平台 id', {
                                hint: `可选平台：${runtime.registry().list().map((adapter) => adapter.id).join(' / ') || '（一个都没有）'}。` +
                                    '一个方案只抓一个平台 —— 要同时抓多个就建多个方案（时段与额度各自独立）。',
                            });
                        }
                        // 校验先跑一次，好把"和哪个方案重复"如实回报（SR-43：只提示，不合并）
                        const checked = plans.validate({ ...patch, name, platforms: targets });
                        const plan = plans.create({ ...patch, name, platforms: targets });
