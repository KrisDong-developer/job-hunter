...
import { asString, enumStr, int, positiveId, requireData, schema, str, textResult, toolDefiner, } from './kit.js';
export function resumesTools(runtime) {
    ...
    return [
        ...
        tool({
            name: 'resume_import',
            description: '把**粘贴进来的简历正文**解析成一版新简历（A3），或把用户**自己的** PDF/DOCX 存成某版简历的附件（R9）。' +
                '**中危**：模型发起时需要审批。' +
                ...
                if (action === 'upload') {
                    ...
                    if (fileName === undefined || contentBase64 === undefined) {
                        throw new DomainError('INVALID_INPUT', 'upload 需要 resumeId、fileName 与 contentBase64', {
                            hint: 'contentBase64 是**用户自己的**文件内容。模型手上没有文件时，' +
                    ...
                    return await runtime.guard().run({
                        ...
                        return {
                            text: `已把「${file.fileName}」存成简历 #${String(resumeId)} 的附件` +
