import { INTERVIEW_KINDS, INTERVIEW_KIND_LABEL, INTERVIEW_STATES, INTERVIEW_STATE_LABEL } from '../../shared/contract/enums/interview.js';
import { DELIVERY_STATE_LABEL } from '../../shared/contract/enums/job.js';
import { APPLICATION_CHANNELS, APPLICATION_CHANNEL_LABEL, APPLICATION_STAGES, APPLICATION_STAGE_LABEL } from '../../shared/contract/enums/pipeline.js';
import { DomainError } from '../util/errors.js';
import { asString, enumStr, int, positiveId, renderText, requireData, schema, str, textResult, toInt, toolDefiner, } from './kit.js';
export function applicationsTools(runtime) {
    ...
    return [
        tool({
            ...
            parameters: schema({
                ...
                resumeFileId: int('关联的简历附件 id（resume_list 的 files[].id）；不填 = 用平台内简历。**不接受文件路径**'),
            }, []),
            outputSchema: {
                type: 'object',
            ...
            async run(args) {
                ...
                const jobIds = (Array.isArray(args['jobIds']) ? args['jobIds'] : []).filter((item) => typeof item === 'number' && Number.isInteger(item) && item > 0);
                // ── 批量：逐条过闸门、逐条回执 ──────────────────────────────
                if (jobIds.length > 0) {
                    ...
                    if (jobIds.length > limit) {
                        throw new DomainError('GUARD_DENIED', `模型单次调用最多涉及 ${String(limit)} 个岗位（§22.4 批量上限），这次是 ${String(jobIds.length)} 个`, {
        ...
        tool({
            ...
            description: '面试错题本：记下被问到的题 / 当时怎么答的 / 下次怎么答更好，或按主题翻出来复习（G6）。' +
            ...
            async run(args) {
                ...
                if (action === 'list') {
                    ...
                    if (items.length === 0) {
                        return {
                            text: '错题本还是空的。面试完趁记得，用 interview_questions(action="add") 把被问到的题记下来 —— ' +
                    ...
                    return {
                        text: [
                            ...
                            ...items.map((item) => `#${String(item.id)} ${item.question}｜被问 ${String(item.times)} 次` +
                ...
                if (action === 'add') {
                    ...
                    return await runtime.guard().run({
                        ...
                        return {
                            text: `已记下：${note.question}（这题累计被问 ${String(note.times)} 次）。` +
                                (note.times > 1
                ...
                const patch = {
                    ...(asString(args['question']) === undefined ? {} : { question: asString(args['question']) }),
                    ...(asString(args['myAnswer']) === undefined ? {} : { myAnswer: asString(args['myAnswer']) }),
                    ...(asString(args['betterAnswer']) === undefined
                        ? {}
                        : { betterAnswer: asString(args['betterAnswer']) }),
                    ...(asString(args['topic']) === undefined ? {} : { topic: asString(args['topic']) }),
                };
                if (action === 'update') {
                    return await runtime.guard().run({ action: 'interview.question.update', actor: 'model', danger: 'mid', payload: { questionId, keys: Object.keys(patch) } }, async () => {
                        const note = service.updateQuestion(questionId, patch);
                        return { text: `已更新错题 #${String(note.id)}（${note.question}）。` };
                    });
                }
                if (action === 'remove') {
                    return await runtime.guard().run({ action: 'interview.question.remove', actor: 'model', danger: 'mid', payload: { questionId } }, async () => {
