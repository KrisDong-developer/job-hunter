...
import { CRAWL_STATE_LABEL, CONTACT_STAGE_LABEL, HEALTH_STATE_LABEL, runReasonLabel } from '../../shared/enums.js';
import { formatLocalMoment, formatWeekdays } from '../../shared/time-format.js';
import { describeWindow } from '../scheduler/schedule.js';
import { APPLICATION_CHANNELS, APPLICATION_CHANNEL_LABEL, APPLICATION_STAGES, APPLICATION_STAGE_LABEL, ASSESSMENT_KINDS, ASSESSMENT_KIND_LABEL, ASSESSMENT_STATES, ASSESSMENT_STATE_LABEL, CAMPUS_BATCHES, CAMPUS_BATCH_LABEL, CAMPUS_STAGES, CAMPUS_STAGE_LABEL, COVER_LETTER_LANGUAGES, INTERVIEW_KINDS, INTERVIEW_KIND_LABEL, INTERVIEW_STATES, INTERVIEW_STATE_LABEL, JOB_STATES, JOB_FLAG_LABEL, REMOTE_KINDS, REMOTE_KIND_LABEL, RESUME_FORMATS, RESUME_LANGUAGES, RESUME_TEMPLATES, TRIPARTITE_STATES, TRIPARTITE_STATE_LABEL, VISA_STANCES, VISA_STANCE_LABEL, } from '../../shared/enums.js';
...
import { DETAIL_KEYS, formatDetailLine, formatJobDetailTitle, formatJobListLine, summarizeJd, } from '../../shared/tool-format.js';
...
const ARRAY_OF_OBJECT = { type: 'array', items: { type: 'object' } };
...
function buildTools(runtime) {
    ...
    const tool = (spec) => defineTool(runtime, spec);
    return [
        tool({
            name: 'job_search',
        ...
        tool({
            ...
            parameters: schema({
                action: {
                    ...
                    description: '要做的操作。dimensions = 看当前平台支持哪些筛选维度；pause/resume = 全局一键暂停定时（只停定时，手动仍可用）',
                },
        ...
        tool({
            name: 'crawl_run',
            description: '手动触发一次抓取（会真的打开浏览器访问招聘站）。模型发起时这是中危操作，需要用户审批。',
            timeoutMs: 5 * 60 * 1000,
            parameters: schema({
                platformId: str('平台 id；不填用第一个已注册平台'),
                keyword: str('搜索关键词'),
            ...
            async run(args) {
                ...
                return await runtime.guard().run({
                    ...
                    const summary = planId === undefined
                        ? await runtime.crawl({
                    ...
                    return {
                        text: `抓取完成（${summary.run.platformId}）：发现 ${String(summary.run.found)} 条，` +
                            `新增 ${String(summary.run.inserted)}、更新 ${String(summary.run.updated)}` +
        ...
        tool({
            ...
            async run() {
                ...
                const skipNotes = scheduler.planStatus
                    ...
                    .map((item) => `· ${item.name}：${item.lastDecision?.message ?? item.lastDecision?.reason ?? '原因未知'}`);
                const lines = [
                    ...
                    '平台：',
                    ...platforms.map((platform) => `· ${platform.displayName}｜健康 ${HEALTH_STATE_LABEL[platform.health]}｜` +
                    ...
                    ...(status.recentRuns.length === 0
                        ...
                        : [
                            ...
                            ...status.recentRuns.slice(0, 5).map((run) => {
                                ...
                                return (`· ${formatLocalMoment(run.startedAt, now) ?? run.startedAt}` +
        ...
        tool({
            name: 'greeting_draft',
            description: '为某个岗位生成打招呼话术。**只生成，不发送** —— 发送要用 greeting_send 并经过用户审批。' +
            ...
            async run(args) {
                ...
                const highlights = Array.isArray(args['highlights'])
                    ? args['highlights'].filter((item) => typeof item === 'string').slice(0, 3)
                    : undefined;
                ...
                const source = draft.via === 'llm' ? '来源：模型生成' : `来源：内置模板（${draft.notes.join('；') || '未调用模型'}）`;
                const egress = draft.outboundFields.length === 0
                    ? '本次没有数据发给模型'
        ...
        tool({
            name: 'greeting_send',
            description: '**发送**打招呼（高危，必须经用户审批）。会依次检查：功能开关 → 隐身 → 额度 → 冷却期 → 用户审批。' +
            ...
            async run(args) {
                ...
                return {
                    jobId: result.jobId,
                    text: `已向「${result.company || result.title}」发送打招呼` +
        ...
        tool({
            name: 'inbox_sync',
            description: '**同步收件箱**：把平台会话列表（HR 消息）读进本地消息表，之后用 inbox 工具即可看到。' +
            ...
            async run(args) {
                ...
                return {
                    text: `${platformId} 收件箱同步完成：读到 ${String(result.fetched)} 条，` +
        ...
        tool({
            name: 'contact_stage',
            description: '**探测某个岗位在平台上的接触阶段**（HR 是否已读 / 已回复）。低危（只看不发，也不写任何东西），' +
            ...
            async run(args) {
                ...
                const stageText = result.stage === null ? '判不出来（会话不在列表里，或状态标记认不出来）' : CONTACT_STAGE_LABEL[result.stage];
                return {
                    text: `岗位 #${String(result.jobId)}（${result.platformId}）在平台上的接触阶段：**${stageText}**。\n` +
                        `${result.note}`,
                };
            },
        }),
        tool({
            ...
            description: '**投递简历**（高危，必须经用户审批）。与 `application_send`（"记一笔我投了"）不同 —— ' +
            ...
            async run(args) {
                ...
                return {
                    text: `已向「${result.company || result.title}」投递简历` +
        ...
        tool({
            ...
            async run() {
                ...
                for (const item of items) {
                    lines.push(`#${String(item.id)} ${item.name}` +
                        `${item.isDefault ? '（当前启用）' : ''}｜${item.direction || '未填方向'}｜` +
                        `${RESUME_LANGUAGE_LABEL[item.language]}｜rev ${String(item.rev)}`);
                    lines.push(`  技能 ${String(item.counts.skills)} · 经历 ${String(item.counts.experiences)} · ` +
        ...
        tool({
            ...
            async run(args) {
                ...
                if (id === 0)
                    throw new DomainError('INVALID_INPUT', 'resumeId 必须是正整数');
                ...
                for (const experience of c.experiences.slice(0, 5)) {
                    lines.push(`经历：${experience.company}｜${experience.title}｜${experience.start ?? '?'}–${experience.end ?? '至今'}` +
                ...
                if (resume.files.length > 0) {
                    lines.push('', `已有附件：${resume.files.map((file) => `${file.format}（${String(file.bytes)} 字节）`).join('、')}`);
                }
                return { text: lines.join('\n') };
            },
        }),
        tool({
            name: 'resume_save',
            description: '新建或修改一版简历。**中危**：模型发起时需要用户审批（简历是用户资产，不该被悄悄改）。' +
                '不传 resumeId 就是新建。',
            parameters: schema({
            ...
            async run(args) {
                ...
                const patch = {
                    ...
                    ...(asString(args['name']) === undefined ? {} : { name: asString(args['name']) }),
                    ...(asString(args['direction']) === undefined
                        ? {}
                        : { direction: asString(args['direction']) }),
                ...
                return await runtime.guard().run({
                    ...
                    const service = runtime.resumes();
                    const resume = resumeId === undefined
                        ? service.create({
                    ...
                    return {
                        text: `${resumeId === undefined ? '已新建' : '已更新'}简历 #${String(resume.id)}「${resume.name}」` +
        ...
        tool({
            name: 'resume_tailor',
            description: '针对某个岗位生成简历定制建议（**只产出建议，不改你手上的简历**）。**中危**，模型发起时需要审批。' +
        ...
        tool({
            name: 'resume_export',
            description: '把某一版简历导出成附件（pdf / docx / html）。**中危**，模型发起时需要审批。' +
            ...
            async run(args) {
                ...
                const target = explicit ?? runtime.resumes().list().find((item) => item.isDefault)?.id ?? runtime.resumes().list()[0]?.id;
                if (target === undefined) {
                ...
                return await runtime.guard().run({
                    ...
                    return {
                        text: `已导出简历 #${String(target)} 的 ${file.format} 附件：${file.fileName}` +
        ...
        tool({
            ...
            description: '记录一次投递（简历发出）。**高危**，必须经用户审批 —— 审批文案会显示岗位、渠道与用了哪版简历。',
            timeoutMs: 3 * 60 * 1000,
            parameters: schema({
                jobId: int('岗位 id'),
            ...
            async run(args) {
                ...
                if (channel !== undefined && !APPLICATION_CHANNELS.includes(channel)) {
                ...
                return {
                    ...
                    text: `已记录投递 #${String(application.id)}：${application.companyName ?? ''}「${application.jobTitle ?? ''}」` +
        ...
        tool({
            ...
            description: '推进/修正一条投递的阶段（已投递→已查看→面试中→已面试→Offer/已拒绝/无回复）。' +
            ...
            async run(args) {
                ...
                if (id === 0)
                    throw new DomainError('INVALID_INPUT', 'applicationId 必须是正整数');
                const to = asString(args['to']);
                if (to === undefined)
                    throw new DomainError('INVALID_INPUT', 'to 必填');
                return await runtime.guard().run({
                    ...
                    const application = runtime.pipeline().advance({
                        ...
                        ...(asString(args['evidenceRef']) === undefined
                            ? {}
                            : { evidenceRef: asString(args['evidenceRef']) }),
                    });
                    runtime.events().publish('application.advanced', { id, stage: application.stage });
                    return {
                        text: `投递 #${String(id)}：${APPLICATION_STAGE_LABEL[before.stage]} → ` +
        ...
        tool({
            ...
            async run(args) {
                ...
                for (const message of inbox.items.slice(0, 20)) {
                    ...
                    lines.push(`#${String(message.id)} [${who}] ${message.at.slice(0, 16).replace('T', ' ')}` +
        ...
        tool({
            name: 'message_reply',
            description: '在**已有会话里**回复一条 HR 消息（真的发到平台上）。**高危**，必须经用户审批' +
            ...
            async run(args) {
                ...
                if (messageId === 0)
                    throw new DomainError('INVALID_INPUT', 'messageId 必须是正整数');
                ...
                return {
                    text: `已回复「${result.company === '' ? result.title : result.company}」（回复的是消息 #${String(result.messageId)}，` +
        ...
        tool({
            ...
            description: '面试日程：新增/改期/改状态/查冲突。**中危**：模型发起时需要审批。' +
            ...
            async run(args) {
                ...
                if (action === 'list') {
                    ...
                    if (list.length === 0)
                        return { text: `未来 ${String(days)} 天没有面试安排。` };
                    return {
                        text: [
                            `共 ${String(list.length)} 场：`,
                            ...list.map((item) => `#${String(item.id)} ${item.at.slice(0, 16).replace('T', ' ')}｜` +
                ...
                if (action === 'state') {
                    ...
                    return await runtime.guard().run({
                        ...
                        const interview = service.setState(id, state, {
                ...
                return await runtime.guard().run({
                    ...
                    const interview = service.upsert({
                        ...
                        ...(asString(args['kind']) === undefined ? {} : { kind: asString(args['kind']) }),
                        ...(asString(args['place']) === undefined ? {} : { place: asString(args['place']) }),
                        ...(asString(args['link']) === undefined ? {} : { link: asString(args['link']) }),
                        ...(asString(args['contact']) === undefined ? {} : { contact: asString(args['contact']) }),
                    ...
                    return {
                        text: `${String(args['interviewId'] ?? '') === '' ? '已新增' : '已更新'}面试 #${String(interview.id)}：` +
        ...
        tool({
            name: 'interview_prep',
            description: '面试准备包：技能差距、公司风险标注、之前记过的错题、通勤提醒与检查清单。低危，只读。',
            ...
            async run(args) {
                ...
                const id = toInt(args.interviewId, 0, 1, Number.MAX_SAFE_INTEGER);
                if (id === 0)
                    throw new DomainError('INVALID_INPUT', 'interviewId 必须是正整数');
        ...
        tool({
            ...
            parameters: schema({
                what: {
                    ...
                    description: '要看哪一块，默认 all。box = 箱线图（P25–P75）；baseline = 与**你自己岗位库**的基准对比；' +
        ...
        tool({
            name: 'overseas_check',
            description: '海外/远程支线：识别岗位的**工签/Sponsorship 立场**与工作模式，做**时区双重换算**，' +
            ...
            async run(args) {
                ...
                if (action === 'visa') {
                    ...
                    return {
                        text: `岗位 #${String(jobId)}：${VISA_STANCE_LABEL[visa.stance]}` +
                ...
                if (action === 'filter') {
                    ...
                    if (stance !== undefined && !VISA_STANCES.includes(stance)) {
                        throw new DomainError('INVALID_INPUT', `不支持的工签立场：${stance}`, {
                            hint: `合法取值：${VISA_STANCES.join(' / ')}`,
                        });
                    }
                    const items = service.filterJobs({
                        ...(stance === undefined ? {} : { stance: stance }),
                        ...(remote === undefined ? {} : { remoteKind: remote }),
                    });
                    if (items.length === 0) {
                        return {
                            text: '没有匹配的岗位。注意：**没有识别过的岗位不会出现在这里** —— ' +
                    ...
                    return {
                        text: [
                            ...
                            ...items.slice(0, 20).map((item) => `#${String(item.jobId)} ${item.title}｜${item.companyName ?? ''}｜` +
                ...
                if (resumeId === 0)
                    throw new DomainError('INVALID_INPUT', 'english-check 需要 resumeId');
        ...
        tool({
            ...
            description: '为某个岗位写 Cover Letter。**中危**：模型发起时需要审批（会调用模型并读取简历）。' +
