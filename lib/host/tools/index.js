...
function buildTools(runtime) {
    ...
    return [
        ...
        tool({
            ...
            async run() {
                ...
                const lines = [
                    ...
                    '平台：',
                    ...platforms.map((platform) => `· ${platform.displayName}｜健康 ${platform.health}｜` +
                    ...
                    ...(status.recentRuns.length === 0
                        ...
                        : [
                            ...
                            ...status.recentRuns
                                .slice(0, 5)
                                .map((run) => `· ${run.startedAt} ${run.platformId} ${run.state}｜` +
        ...
        tool({
            name: 'job_report',
            description: '数据看板：投递漏斗、渠道/简历版本归因、薪资分位。低危，只读。' +
                '样本量太小时会明确说明"别据此下结论"，不会硬给你一个百分比。',
            parameters: schema({
                what: {
                    type: 'string',
                    enum: ['funnel', 'attribution', 'salary', 'all'],
                    description: '要看哪一块，默认 all',
                },
                city: str('薪资分位的城市范围'),
                q: str('薪资分位的岗位关键词'),
            }),
            ...
            async run(args) {
                ...
                if (what === 'salary' || what === 'all') {
                    const band = analytics.salaryBand({
                        ...(asString(args['city']) === undefined ? {} : { city: asString(args['city']) }),
                        ...(asString(args['q']) === undefined ? {} : { keyword: asString(args['q']) }),
                    });
