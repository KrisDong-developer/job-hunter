...
const pageText = (page) => page.evaluate(() => document.body?.innerText?.slice(0, 4000) ?? '', null);
/** 判墙：登录页 URL / 人机验证文案。宁误报不硬闯 —— 撞墙就停，绝不绕过。 */
async function blockedOf(page) {
    if (page.url().includes('/login'))
        return { kind: 'blocked', reason: 'login-wall' };
    const text = await pageText(page);
    if (/滑动验证|拖动滑块|请完成验证|安全验证/.test(text))
        return { kind: 'blocked', reason: 'captcha' };
    return null;
}
/** 轮询等页面文本出现标志词（waitForSelector 只认选择器，文本标志自己轮）。 */
async function waitForMarker(page, marker, timeoutMs) {
...
async function readCandidatesOf(page) {
...
export const tianyanchaProvider = {
    id: 'tianyancha',
    async search(name, page) {
        ...
        while (Date.now() < deadline) {
            ...
            if (blocked !== null)
                return blocked;
            const hasResults = await page.evaluate((selector) => document.querySelector(selector) !== null, COMPANY_LINK);
            if (hasResults) {
