 ...
 */
export function resumeVersionTextOf(input) {
    if (input.label === null)
        return '平台内简历（平台侧那一份，本机未登记版本）';
 ...
 * 登录了也还是一样投不出去。
 */
export function applicationPlatformBlocker(deps, platformId) {
 ...
 */
export function applicationReadinessOf(deps, jobId) {
 ...
 */
export async function sendApplication(deps, guardToken, input) {
    // ── 第一行：机制化强制 ────────────────────────────────────────────
    ...
    try {
        const result = await sendResume(page, { title: job.title, company: job.companyName ?? '', sourceUrl: job.sourceUrl }, input.filePath);
        if (!result.ok) {
        ...
        deps.logger?.info(`[guard] 已向「${job.companyName ?? job.title}」投递简历（delivery=${result.delivery}）`);
