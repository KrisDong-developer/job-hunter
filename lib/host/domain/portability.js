...
import { asRealOrNull, asText } from '../store/row.js';
...
export function csvEncode(header, rows) {
...
const IMPORT_FIELDS = Object.keys(HEADER_ALIASES);
...
const APPLICATION_HEADER = ['岗位标题', '公司', '渠道', '阶段', '投递时间', '阶段更新', '简历版本', '备注'];
const GREETING_HEADER = ['岗位标题', '公司', '平台', '模板', '接触态', '阶段时间', '发送时间', '发送方式', '内容'];
const MESSAGE_HEADER = ['岗位标题', '公司', '平台', '方向', '时间', '是否已读', '内容'];
const INTERVIEW_HEADER = ['岗位标题', '公司', '轮次', '时间', '时区', '形式', '地点', '链接', '联系人', '状态'];
const COMPANY_HEADER = ['公司', '归一化名', '行业', '规模', '性质', '在库岗位数', '外包信号分', '诈骗信号分', '已拉黑', '备注'];
function stamp(now) {
    return now.slice(0, 16).replace(/[-:]/g, '').replace('T', '-');
}
function tableNames(store) {
    return store.db
        .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%' ORDER BY name")
        .all().map((row) => asText(row['name']));
}
/** 全量结构化备份（各表的原始行）。 */
function jsonPayload(store, now) {
    const tables = {};
    const counts = {};
...
function jobRows(store) {
    return store.db
        .prepare(`SELECT j.title, c.name AS company_name, j.salary_raw, j.city, j.district, j.exp_req, j.edu_req,
...
function csvBundle(store) {
    const jobs = jobRows(store);
    const applications = store.db
        .prepare(`SELECT j.title, c.name AS company_name, a.channel, a.stage, a.sent_at, a.stage_at, a.resume_id, a.note
    ...
    const greetings = store.db
        .prepare(`SELECT j.title, c.name AS company_name, g.platform_id, t.name AS template_name, g.stage,
    ...
    const messages = store.db
        .prepare(`SELECT j.title, c.name AS company_name, m.platform_id, m.direction, m.at, m.read_at, m.content
    ...
    const interviews = store.db
        .prepare(`SELECT j.title, c.name AS company_name, i.round, i.at, i.tz, i.kind, i.place, i.link, i.contact, i.state
    ...
    const companies = store.db
        .prepare(`SELECT c.name, c.name_norm, c.industry, c.size, c.nature, p.job_count, p.outsourcing_score,
 ...
 */
export function exportData(deps, clock = systemClock) {
    const now = clock();
    ...
    return {
        ...
        entries: listed,
        note: `归档：结构化数据 + ${String(listed.filter((item) => item.name.startsWith('files/')).length)} 个附件（${String(attachmentBytes)} 字节）。` +
...
function jobInputOf(row, companyId) {
    ...
    const tags = tagsRaw === ''
        ? []
        : tagsRaw.startsWith('[')
            ? JSON.parse(tagsRaw).filter((item) => typeof item === 'string')
 ...
 */
export function importJobs(deps, payload, clock = systemClock) {
    ...
    if (looksLikeBinary(content)) {
        return {
            ...
            moreErrors: 0,
            note: '不支持 .xlsx 二进制：直接解析它需要解包 + 解析 XML，做错了会把脏数据写进岗位库。' +
    ...
    if (content.includes('\ufffd')) {
        return {
            ...
            moreErrors: 0,
            note: '中文 Excel 默认另存为 GBK。请用「另存为 → CSV UTF-8（逗号分隔）」重新导出，' +
    ...
    if (payload.format === 'csv') {
        ...
        if (parseError === null) {
            ...
            if (array.length === 0)
                parseError = 'JSON 里没有找到岗位数组（既不是数组，也没有 tables.job）。';
            else
                rows = array.map((raw) => rowFromJson(raw));
    ...
    for (let index = 0; index < rows.length; index += 1) {
        ...
        if (title === '') {
            skipped += 1;
            if (errors.length < ELLIPSIS_ERRORS)
                errors.push({ row: rowNo, message: '缺少岗位标题，跳过。' });
            else
                moreErrors += 1;
            continue;
        }
    ...
    return {
        ...
        errors,
        moreErrors,
        note: `导入完成：新增 ${String(inserted)} 条，更新 ${String(updated)} 条，跳过 ${String(skipped)} 条。` +
