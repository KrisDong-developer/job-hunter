import { REMOTE_KINDS, VISA_STANCES } from '../../shared/contract/enums/overseas.js';
 ...
 */
export function timezoneDisplay(atIso, counterpartTz, localTz) {
    ...
    return {
        ...
        warning: Math.abs(counterpart.offsetHours - local.offsetHours) >= 6
            ? '两边差 6 小时以上 —— 这种跨时区的面试最容易记错，确认时请把两边的时间都念一遍。'
            : null,
    };
}
/** 取某个时区在某时刻的 UTC 偏移小时数（用 Intl 反推，不引依赖）。 */
function offsetHoursOf(date, tz) {
...
export function createOverseasService(deps) {
    const { store } = deps;
    ...
    return {
        analyzeJob(jobId) {
            ...
            deps.logger?.info(`[overseas] 岗位 #${String(job.id)}：工签=${visa.stance}，模式=${remote.kind}` +
        ...
        filterJobs(input) {
            ...
            for (const job of rows) {
                ...
                if (input.stance !== undefined && stance !== input.stance)
                    continue;
                if (input.remoteKind !== undefined && remote !== input.remoteKind)
                    continue;
                out.push({
                    jobId: job.id,
                    title: job.title,
        ...
        async draftCoverLetter(input) {
            ...
            const resume = input.resumeId === undefined
            ...
            if (ai === undefined || input.useLlm === false) {
                const record = store.branch.createCoverLetter({ jobId: job.id, resumeId: resume?.id ?? null, language, content: fallback(), via: 'rule', notes: ['使用模板生成（未调用模型）'] }, clock());
                return { ...record };
            }
            const result = await ai.call({
                ...
                parse: (raw) => {
                    ...
                    return {
                        ...
                        notes: Array.isArray(notes) ? notes.filter((item) => typeof item === 'string').slice(0, 6) : [],
                    };
                },
...
function readJobRemote(store, jobId) {
    const row = store.db.prepare('SELECT remote_kind FROM job WHERE id = ?').get(jobId);
    const value = row?.remote_kind;
    return typeof value === 'string' && REMOTE_KINDS.includes(value) ? value : null;
}
