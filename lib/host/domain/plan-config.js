...
import { normalizePlatformOverrides, normalizePostProcess, normalizeSchedule, platformOverrideOf, } from '../store/repo/plans.js';
...
export function validatePlanConfig(input, context) {
    ...
    for (const platformId of platforms.filter((id) => platformOverrideOf({ platformOverrides: overrides }, id).enabled)) {
        const adapter = context.registry.get(platformId);
        if (adapter === undefined)
            continue;
        ...
        if (adapter.auth === undefined && (crawlAuth === 'required' || crawlAuth === 'unknown')) {
            notices.push(`${name}抓取是否需要登录${crawlAuth === 'required' ? '是' : '尚未验证'}，` +
        ...
        if (city !== undefined && city !== '') {
            ...
            if (values.length > 0 && !values.includes(city)) {
                notices.push(`${name}不认识城市「${city}」—— 它会返回空结果，建议去掉这个平台或换成它支持的城市`);
            }
        }
    }
    return {
