...
export function validatePlanConfig(input, context) {
    ...
    if (platforms.length > 1) {
        throw new DomainError('INVALID_INPUT', `一个方案只能抓一个平台，收到 ${String(platforms.length)} 个：${platforms.join(' / ')}`, {
            hint: '筛选条件是按平台自己的取值域与参数名定义的，两个平台共用一份条件必然有一家收错。' +
    ...
    for (const [key, raw] of Object.entries(criteriaInput)) {
        ...
        if (spec.multi) {
            ...
            if (rejected.length > 0) {
                ...
                throw new DomainError('INVALID_INPUT', `${spec.label}不接受取值「${rejected.join('、')}」`, { hint: `可选取值：${list.slice(0, 40).join(' / ')}${list.length > 40 ? ` …等 ${String(list.length)} 个` : ''}。${spec.hint}` });
            }
        ...
        if (spec.numeric) {
            ...
            if (!Number.isFinite(parsed) || parsed <= 0) {
                throw new DomainError('INVALID_INPUT', `${spec.label}（${platforms.join('/')}）需要一个正整数，收到「${inputValue}」`, { hint: spec.hint });
            }
            if (spec.max !== undefined && parsed > spec.max) {
                // 上限是**平台自己的**（waiqi 只有 1 页、BOSS 的页码不可寻址…）——
                // 单平台方案下一旦超了就是硬拒：以前多平台时它只会"静默截断 + 提一句"，
                // 而用户以为自己抓了 5 页。
                throw new DomainError('INVALID_INPUT', `${spec.label}（${platforms.join('/')}）最大 ${String(spec.max)}，收到 ${String(parsed)}`, { hint: spec.hint });
            }
