import { RESUME_COMPARE_CAVEAT, SALARY_BASIS_LABEL, } from '../../shared/dto.js';
import { APPLICATION_CHANNEL_LABEL, APPLICATION_STAGE_LABEL, CONTACT_STAGE_LABEL, } from '../../shared/enums.js';
...
export function createAnalyticsService(deps) {
     ...
     */
    const everReached = (record, stages) => {
        if (stages.includes(record.stage))
            return true;
        return store.pipeline
            .listStageEvents('application', record.id, 200)
            .some((event) => stages.includes(event.toStage));
    };
    /**
     * 时间窗：两端都可缺省（缺省即不设该端）。ISO 字符串直接比大小 —— 我们全库都用
     * `toISOString()` 落库，同格式的字符串比较与时间先后一致。
     */
    const inWindow = (at, filter) => {
        if (filter.from !== undefined && at < filter.from)
            return false;
        if (filter.to !== undefined && at > filter.to)
            return false;
        return true;
    };
    /** 投递记录是否命中「简历版本 / 方向」。这两个维度只有 application 有。 */
    const matchesResume = (record, filter) => {
        if (filter.resumeId !== undefined && record.resumeId !== filter.resumeId)
            return false;
        if (filter.direction !== undefined && filter.direction !== '') {
            ...
            if (resume === undefined || resume.direction !== filter.direction)
                return false;
        }
        return true;
    };
    ...
    const matchesJob = (jobId, filter, index) => {
        if ((filter.city ?? '') === '' && (filter.keyword ?? '') === '')
            return true;
        const facts = jobId === null ? undefined : index.get(jobId);
        // 岗位被删了就证不出它命中城市/关键词 —— 宁可不算，也不要"猜它算"。
        if (facts === undefined)
            return false;
        if ((filter.city ?? '') !== '' && facts.city !== filter.city)
            return false;
    ...
    return {
        funnel(filter = {}) {
            ...
            const applications = store.pipeline
                ...
                .filter((record) => inWindow(record.sentAt, filter) &&
                matchesJob(record.jobId, filter, index) &&
                matchesResume(record, filter));
            const greetings = store.pipeline
                .listGreetings({ limit: 1000 })
                .filter((item) => item.jobId !== null && inWindow(item.sentAt, filter) && matchesJob(item.jobId, filter, index));
            const greeted = greetings.length;
            const delivered = greetings.filter((item) => item.stage !== 'greeted' && item.stage !== 'none').length;
            const read = greetings.filter((item) => ['read', 'replied', 'interview_scheduled'].includes(item.stage)).length;
            const replied = greetings.filter((item) => ['replied', 'interview_scheduled'].includes(item.stage)).length;
            const applied = applications.length;
            const interviewed = applications.filter((record) => everReached(record, ['interviewed', 'offer'])).length;
            ...
            const steps = raw.map((step, index) => {
                ...
                return {
                    ...step,
                    rate: !comparable || previous === null || previous.count === 0 ? null : step.count / previous.count,
                };
            });
        ...
        attribution(filter = {}) {
            ...
            const summarize = (keyOf, labelOf) => {
                ...
                return [...buckets.entries()]
                    ...
                    const replied = list.filter((record) => ['replied', 'interview_scheduled'].includes(contactOf(record.jobId))).length;
                    const interviewed = list.filter((record) => everReached(record, ['interviewed', 'offer'])).length;
            ...
            // 按渠道
            const byChannel = summarize((record) => record.channel, (key) => APPLICATION_CHANNEL_LABEL[key] ?? key);
            ...
            const byResume = summarize((record) => (record.resumeId === null ? 'none' : String(record.resumeId)), (key) => {
                if (key === 'none')
                    return '（未记录简历）';
        ...
        salaryBox(options = {}) {
            const basis = options.basis ?? 'monthly_min';
            ...
            return {
                ...
                note: `口径：${SALARY_BASIS_LABEL[basis]}。P25–P75 是高亮的箱体，` +
        ...
        salaryBaseline(filter = {}) {
            ...
            const scope = [filter.city ?? '', filter.keyword ?? ''].filter((part) => part !== '').join(' / ') || '全部岗位';
            const enoughSample = applied.count >= MIN_SAMPLE;
            const medianGap = all.median === null || applied.median === null ? null : applied.median - all.median;
            return {
                scope,
                all,
                applied,
                medianGap,
                enoughSample,
                note: `基准来自**你自己抓到的岗位库**（${String(all.count)} 条有薪资下限的岗位），不是行业数据 —— ` +
                    `本项目没有数据源，编一个基准会误导你。` +
                    (enoughSample
         ...
         */
        resumeCompare(filter = {}) {
            const applications = store.pipeline.listApplications().filter((application) => {
                if (filter.from !== undefined && application.sentAt < filter.from)
                    return false;
                if (filter.to !== undefined && application.sentAt > filter.to)
                    return false;
                if (filter.resumeId !== undefined && application.resumeId !== filter.resumeId)
                    return false;
                if (filter.city !== undefined || filter.keyword !== undefined) {
                    ...
                    if (job === undefined)
                        return false;
                    if (filter.city !== undefined && filter.city !== '' && job.city !== filter.city)
                        return false;
            ...
            for (const resume of store.resume.list())
                resumeNames.set(resume.id, resume.name);
        ...
        salaryBand(options = {}) {
            const jobs = store.job
                ...
                .filter((job) => {
                if (options.from !== undefined && job.firstSeenAt < options.from)
                    return false;
                if (options.to !== undefined && job.firstSeenAt > options.to)
                    return false;
                return true;
            });
 ...
 */
function salaryValuesOf(jobs, basis) {
    const values = [];
...
export function salaryBoxOf(jobs, basis) {
