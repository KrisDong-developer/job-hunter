...
async function executeCrawl(deps, adapter, options, clock) {
    ...
    let pages = 0;
    let failure = null;
    const burst = deps.createBurstGuard?.() ?? new BurstGuard();
        ...
        for (const jobId of writtenJobIds) {
            try {
                ...
                const outcome = applyDedup({
                    ...
                    candidatesFor: (self) => (job.companyId === null
                        ? []
                        : store.job
                            .query({ companyId: job.companyId })
                            .map(dedupCandidateOf)
                            .filter((item) => item !== undefined)).filter((item) => item.id !== self.id && item.platformId !== self.platformId),
                }, candidate, now());
