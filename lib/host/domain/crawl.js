...
async function executeCrawl(deps, adapter, options, clock) {
    ...
    let seenTouched = 0;
    for (const rejected of partition.rejected) {
        if (rejected.raw.platformJobId.trim() === '')
            continue;
        if (store.job.touch(adapter.id, rejected.raw.platformJobId, now()))
            seenTouched += 1;
    }
    if (seenTouched > 0) {
        deps.logger?.info(`[crawl] ${adapter.id}：${String(seenTouched)} 条"见到了但没写成功"（字段断言没过）—— 已只刷新「最近见到」`);
    }
