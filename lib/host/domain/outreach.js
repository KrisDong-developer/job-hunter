...
export const GREETING_TONE_LABEL = {
    formal: '正式',
    warm: '热情',
    concise: '简短',
};
...
export function scanInjection(text, limit = 3) {
    const hits = [];
 ...
 */
export function validateGreetingText(text) {
    const trimmed = text.trim();
    if (trimmed.length < GREETING_MIN_CHARS)
        return `话术过短（${trimmed.length} 字）`;
    if (trimmed.length > GREETING_MAX_CHARS)
        return `话术过长（${trimmed.length} 字，上限 ${GREETING_MAX_CHARS}）`;
    for (const { name, pattern } of HARD_BLOCKED_PATTERNS) {
        pattern.lastIndex = 0;
        if (pattern.test(trimmed))
            return `话术里出现了${describePattern(name)}`;
    }
...
export function createOutreachService(deps) {
    const { store, ai } = deps;
    ...
    const draft = async (input) => {
        ...
        if (injectionHits.length > 0) {
            deps.logger?.warn(`[outreach] 岗位 ${String(job.id)} 的 JD 里发现疑似注入：${injectionHits.map((hit) => hit.name).join('、')}`);
 ...
 */
export function buildTemplateGreeting(input) {
    ...
    const parts = [`${opener}看到${company}在招「${input.title}」，我很感兴趣。`];
    if (skillText !== '') {
        parts.push(input.tone === 'concise'
            ? `我有${skillText}相关经验。`
            : `我做过${skillText}相关的工作，和这个岗位的要求比较对口。`);
    }
    if (input.expReq !== '')
        parts.push(`岗位要求${input.expReq}，我的情况基本匹配。`);
    if (input.tone !== 'concise' && input.city !== '')
        parts.push(`我目前在${input.city}，可以随时沟通。`);
