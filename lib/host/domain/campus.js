...
export function createCampusService(deps) {
     ...
     */
    const deadlineOwner = (deadline) => {
        const campusId = deadline.kind === 'apply-close'
            ? deadline.refId
    ...
    const advanceCampusStage = (campusId, to, evidenceRef) => {
        ...
        if (CAMPUS_STAGES.indexOf(to) < CAMPUS_STAGES.indexOf(before.stage))
            return;
    ...
    return {
        create(input) {
            ...
            if (companyId === null && companyName !== '') {
                ...
                if (nameNorm === '')
                    note = note ?? companyName;
                else
                    companyId = store.company.ensure({ name: companyName, nameNorm }, clock()).id;
            }
        ...
        addTripartite(input) {
            ...
            if (record.campusApplicationId !== null) {
                ...
                if (campus !== undefined && campus.stage === 'intent') {
                    deps.logger?.warn(`[campus] 校招记录 #${String(campus.id)} 还在"意向"就先建了三方 —— ` +
 ...
 */
export function syncDeadlineTodos(store, deadlines, now) {
    const existing = new Map(store.todo.listOpen({ kind: 'deadline', limit: 200 }).map((todo) => [todo.ref ?? '', todo]));
    for (const deadline of deadlines) {
        const ref = `${deadline.kind}:${String(deadline.refId)}`;
        const open = existing.get(ref);
        if (!deadline.urgent) {
            if (open !== undefined)
                store.todo.closeByRef('deadline', ref, now);
            continue;
        }
