...
import { toResumeDto, toResumeFileDto } from '../store/repo/resumes.js';
import { GREETING_MAX_CHARS, validateGreetingText } from './outreach.js';
import { DomainError, messageOf } from '../util/errors.js';
import { isoNow, systemClock } from '../util/time.js';
export function createResumeService(deps) {
    const { store } = deps;
    const clock = deps.clock ?? systemClock;
    const filesDir = deps.filesDir;
    const toDto = (record) => toResumeDto(record, store.resume.listFiles(record.id).map(toResumeFileDto), store.tailoring.countFor(record.id));
    const requireResume = (id) => {
    ...
    const dirFor = (resumeId) => join(filesDir, `resume-${String(resumeId)}`);
    return {
        list(options = {}) {
        ...
        update(id, patch) {
            requireResume(id);
            const record = store.resume.update(id, {
        ...
        preview(id, template = 'concise') {
            const record = requireResume(id);
            return renderResumeHtml(record.content, { template });
        },
        async exportResume(id, input = {}) {
            const record = requireResume(id);
            ...
            if (format === 'html') {
                ...
                if (pdf === undefined) {
                    throw new DomainError('ADAPTER_BROKEN', '当前环境没有可用的 PDF 渲染器', {
                        hint: 'PDF 需要一份 headless Chromium（抓取用的浏览器是 headful 的，page.pdf() 用不了）。' +
                            '可以先用 docx 或 html 导出。',
                        detail: { format },
                    });
                }
        ...
        async importResume(input) {
            ...
            if (text.length > RESUME_IMPORT_MAX_CHARS) {
                throw new DomainError('INVALID_INPUT', `简历文本太长了（${String(text.length)} 字，上限 ${String(RESUME_IMPORT_MAX_CHARS)}）`, { hint: '把无关的页眉页脚、隐私声明删掉再试 —— 太长的文本既烧额度也解析不准。' });
            }
            ...
            if (ai === undefined || input.useLlm === false) {
                notes.push('没有调用模型（用途「简历导入解析」默认关闭，或本次要求只用规则）—— ' +
                ...
                const result = await ai.call({
                    ...
                    outputSpec: '只输出 JSON：{"content": {"basics":{"name","title","city","years"},' +
            ...
            if (input.save !== false) {
                ...
                deps.logger?.info(`[resume] 从粘贴文本导入简历 #${String(created.id)}（${via}，${String(text.length)} 字）`);
            }
            return { resume, content, via, notes, issues: inspectOf(content) };
        },
        uploadFile(resumeId, input) {
            ...
            if (bytes.byteLength > ATTACHMENT_MAX_BYTES) {
                throw new DomainError('INVALID_INPUT', `附件太大（${formatBytes(bytes.byteLength)}，上限 ${formatBytes(ATTACHMENT_MAX_BYTES)}）`, { hint: '主流招聘站对简历附件普遍限 5–10MB；先压缩或另存为更小的 PDF。' });
            }
            ...
            deps.logger?.info(`[resume] 收到上传附件：${join(dir, stored)}（${String(bytes.byteLength)} 字节，${format}）`);
            const file = store.resume.addFile({
                resumeId: record.id,
                format,
        ...
        async tailor({ jobId, resumeId, useLlm }) {
            ...
            const targetKeywords = techTokensOf([job.title, job.tags.join(' '), jdText].filter(Boolean).join(' '));
            ...
            if (ai === undefined || useLlm === false) {
                const created = store.tailoring.create({ resumeId: record.id, jobId: job.id, content: fallback, via: 'rule', notes: baseNotes }, clock());
            ...
            const result = await ai.call({
                ...
                outputSpec: '只输出 JSON：{"content": <改写后的完整简历对象，结构与输入完全一致>, "notes": ["改了什么", ...]}。不要输出其它内容。',
                maxTokens: 4096,
                temperature: 0.3,
                ...
                parse: (raw) => {
                    ...
                    return {
                        ...
                        notes: Array.isArray(notes)
                            ? notes.filter((item) => typeof item === 'string').slice(0, 10)
                            : [],
                    };
                },
                validate: (value) => {
            ...
            if (via === 'llm' && modelNotes.length === 0)
                notes.unshift('模型没有说明改了什么');
            ...
            deps.logger?.info(`[resume] 为岗位 #${String(job.id)} 生成了定制（${via}），采用与否待用户决定`);
            return tailoringDto(created);
        },
        listTailorings(options) {
            return store.tailoring.list(options).map(tailoringDto);
        },
        adopt(tailoringId, adopted) {
        ...
        listGreetingTemplates(resumeId) {
            return store.pipeline.listTemplates({ resumeId }).map(toGreetingTemplateDto);
        },
        async generateGreetingTemplate({ resumeId, tone = 'formal' }) {
        ...
        saveGreetingTemplate({ resumeId, id, name, body }) {
            const record = requireResumeRecord(store, resumeId);
 ...
 */
export function ruleTailor(content, job) {
    ...
    if (movedSkills > 0)
        notes.push(`把与「${job.title}」相关的技能排到了前面（顺序变了，内容没变）`);
    ...
    const experiences = content.experiences.map((experience) => {
        ...
        if (highlights.some((text, index) => text !== experience.highlights[index]))
            reorderedHighlights += 1;
        return { ...experience, highlights };
    });
    if (reorderedHighlights > 0)
        notes.push(`重排了 ${String(reorderedHighlights)} 段经历里的成果顺序，突出与岗位相关的部分`);
    // ③ 项目：同样只重排
    ...
    if (content.summary.trim() === '')
        notes.push('原简历没有个人简介，按已有技能自动补了一句（没有新增事实）');
    return {
        content: {
            ...content,
...
// ── 话术模板（v12）：纯函数与共用校验 ────────────────────────────────
function toGreetingTemplateDto(record) {
    return {
        id: record.id,
        resumeId: record.resumeId,
        name: record.name,
...
function requireResumeRecord(store, resumeId) {
...
function assertTemplateQuota(store, resumeId) {
    const count = store.pipeline.listTemplates({ resumeId }).length;
    if (count >= MAX_RESUME_GREETING_TEMPLATES) {
        throw new DomainError('INVALID_INPUT', `这版简历的话术模板已有 ${String(count)} 条（上限 ${String(MAX_RESUME_GREETING_TEMPLATES)} 条）`, { hint: '每个方向留几条不同语气的开场就够 —— 先删掉不用的，或编辑现有的。' });
    }
}
function upsertGreetingTemplate(store, resumeId, seed, via, now) {
...
/** 从正文里提取 `{占位符}` 名列表（去重保序）。 */
export function templateVarsOf(body) {
    return [...new Set([...body.matchAll(/\{([^{}\s]{1,12})\}/g)].map((match) => match[1]))];
}
/**
 * 模板正文校验：占位符、联系方式/长度（复用发送侧同一条 `validateGreetingText`）、
 * 技术词白名单（简历没有的技术词 = 编造，R8 红线）。
 */
export function validateTemplateBody(allowed, body) {
    if (!body.includes('{岗位}')) {
 ...
 * "什么都不加"在结构上就不可能编造。
 */
export function buildResumeGreetingTemplate(content, tone) {
    const title = content.basics.title === '' ? '这个方向' : content.basics.title;
    const years = typeof content.basics.years === 'number' && content.basics.years > 0 ? `（${String(content.basics.years)} 年经验）` : '';
 ...
 */
export function stripContacts(content) {
 ...
 */
function ungroundedNotes(content, source) {
...
function leafValues(value) {
    ...
    if (Array.isArray(value))
        return value.flatMap((item) => leafValues(item));
 ...
 */
function storedNameOf(displayName, format) {
...
function parseUploadFormat(fileName, declared) {
    ...
    if (UPLOAD_FORMATS.includes(candidate))
        return candidate;
    ...
    throw new DomainError('INVALID_INPUT', `不支持的附件格式：${candidate || '（没有扩展名）'}`, {
        hint: '只收 PDF 与 DOCX。老版 .doc 请先用 Word 另存为 .docx；' +
 ...
 */
function assertAttachmentKind(format, bytes) {
