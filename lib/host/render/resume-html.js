 ...
 */
const ICONS = {
    phone: '<svg class="ico" viewBox="0 0 16 16" aria-hidden="true"><path d="M3.6 1.8h2.1l1 2.6-1.4.9a8.2 8.2 0 0 0 4.4 4.4l.9-1.4 2.6 1v2.1c0 .8-.7 1.5-1.5 1.5A10 10 0 0 1 2.1 3.3c0-.8.7-1.5 1.5-1.5z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/></svg>',
    mail: '<svg class="ico" viewBox="0 0 16 16" aria-hidden="true"><rect x="2" y="3.6" width="12" height="8.8" rx="1.6" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M2.8 4.8 8 8.5l5.2-3.7" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>',
    pin: '<svg class="ico" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 14s4.1-3.9 4.1-6.9A4.1 4.1 0 0 0 3.9 7.1C3.9 10.1 8 14 8 14z" fill="none" stroke="currentColor" stroke-width="1.2"/><circle cx="8" cy="7" r="1.5" fill="none" stroke="currentColor" stroke-width="1.2"/></svg>',
};
/**
 * 页眉。
 *
 ...
 */
function renderBasics(basics, showOptional) {
    const out = [];
    ...
    if (name !== '' || role !== '')
        out.push(`      <div class="resume-headline">${name}${role}</div>`);
    // 联系方式行的顺序固定：手机 → 邮箱 → 城市 →（开关打开时）年限 → 链接。
    // HR 找手机号时手是有肌肉记忆的。
    const contacts = [];
    if (has(basics.phone))
        contacts.push(`<span>${ICONS.phone}${escapeHtml(basics.phone)}</span>`);
    if (has(basics.email))
        contacts.push(`<span>${ICONS.mail}${escapeHtml(basics.email)}</span>`);
    if (has(basics.city))
        contacts.push(`<span>${ICONS.pin}${escapeHtml(basics.city)}</span>`);
...
function renderSkill(skill) {
    const bits = [`<b class="skill-name">${escapeHtml(skill.name)}</b>`];
    if (has(skill.level))
        bits.push(`<span class="skill-level">${escapeHtml(skill.level)}</span>`);
    if (typeof skill.years === 'number')
        bits.push(`<span class="skill-level">${String(skill.years)} 年</span>`);
    if (has(skill.evidence))
        bits.push(`<span class="skill-note">${escapeHtml(skill.evidence)}</span>`);
...
function renderExperience(experience) {
    ...
    const tail = [dateRange(experience.start, experience.end), has(experience.city) ? escapeHtml(experience.city) : '']
        .filter((part) => part !== '')
        .join(' | ');
    const lines = [`        <div class="exp-head">`];
    lines.push(`            ${heading}`);
    if (tail !== '')
        lines.push(`            <span class="exp-period">${tail}</span>`);
    lines.push(`          </div>`);
...
function renderEducation(education) {
    ...
    return `          <li class="edu-item"><span class="edu-main">${escapeHtml(education.school)}${extra === '' ? '' : `<span class="edu-extra"> · ${extra}</span>`}</span>${tail === '' ? '' : `<span class="exp-period">${tail}</span>`}</li>`;
}
// ─────────────────────────────────────────────────────────────────────
// 组装
// ─────────────────────────────────────────────────────────────────────
...
function renderBody(content, options) {
    ...
    if (extras.length > 0) {
        const rows = extras
            .map((extra) => `        <li class="extra-item"><span class="extra-label">${escapeHtml(extra.label)}</span>` +
            `<span class="extra-text">${escapeHtml(extra.text)}</span></li>`)
            .join('\n');
        blocks.push(`    <section class="resume-section" data-section="extras">\n` +
...
const CJK_FONT_STACK = '"Microsoft YaHei", "PingFang SC", "Hiragino Sans GB", "Source Han Sans SC", "Noto Sans CJK SC", SimSun, sans-serif';
/**
 * 打印样式表。
 *
...
export function renderResumeHtml(content, options = {}) {
...
//# sourceMappingURL=resume-html.js.map