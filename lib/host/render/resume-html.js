...
function skillNote(skill) {
    const parts = [];
    if (has(skill.level))
        parts.push(escapeHtml(skill.level));
    if (typeof skill.years === 'number')
        parts.push(`${String(skill.years)} 年`);
    if (has(skill.evidence))
        parts.push(escapeHtml(skill.evidence));
    return parts.join(' · ');
}
// ─────────────────────────────────────────────────────────────────────
// 页眉
...
function renderBasics(basics, showOptional) {
    const out = [];
    if (has(basics.name))
        out.push(`      <h1 class="resume-name">${escapeHtml(basics.name)}</h1>`);
    if (has(basics.title))
        out.push(`      <p class="resume-job">${escapeHtml(basics.title)}</p>`);
    // 联系方式行的顺序固定：手机 → 邮箱 → 城市 → 链接。HR 找手机号时手是有肌肉记忆的。
    const contacts = [];
    if (has(basics.phone))
        contacts.push(`<span>${escapeHtml(basics.phone)}</span>`);
    if (has(basics.email))
        contacts.push(`<span>${escapeHtml(basics.email)}</span>`);
    if (has(basics.city))
        contacts.push(`<span>${escapeHtml(basics.city)}</span>`);
...
function renderProject(project) {
    ...
    if (has(project.role))
        headingBits.push(escapeHtml(project.role));
...
function renderEducation(education) {
    ...
    if (has(education.major))
        parts.push(escapeHtml(education.major));
    if (has(education.degree))
        parts.push(escapeHtml(education.degree));
    const tail = dateRange(education.start, education.end);
    return `          <li class="edu-item"><span class="edu-main">${parts.join(' · ')}</span>${tail === '' ? '' : `<span class="exp-period">${tail}</span>`}</li>`;
}
// ─────────────────────────────────────────────────────────────────────
// 组装
// ─────────────────────────────────────────────────────────────────────
