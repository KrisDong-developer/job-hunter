...
import { EXPORTS_DIR_NAME, RETENTION_AUTO_CLEAN_DEFAULT, RETENTION_DEFAULTS, RETENTION_KEY, RETENTION_MAX_DAYS, RETENTION_MIN_DAYS, } from '../../shared/constants.js';
import { systemClock } from '../util/time.js';
import { asInt, asText } from './row.js';
const DAY_KEYS = [
    'crawlRunsDays',
    'auditLogDays',
    'llmCallsDays',
...
export function normalizeRetentionPolicy(input) {
    ...
    if (typeof raw['autoCleanEnabled'] === 'boolean')
        out.autoCleanEnabled = raw['autoCleanEnabled'];
    return out;
}
export function readRetentionPolicy(store) {
    return normalizeRetentionPolicy(store.setting.get(RETENTION_KEY, 'global', ''));
}
export function writeRetentionPolicy(store, patch, now) {
...
const SPECS = [
    ...
    },
    {
        ...
        table: 'job',
        mode: 'clear-column',
        describe: '这是**体积大头**。只清 `jd_text` / `jd_summary` 两个字段，**不删岗位行** —— ' +
    ...
    },
    {
        ...
        table: 'job',
        mode: 'delete',
        describe: '只删「你没收藏、也没投递 / 打招呼 / 聊过 / 约面 / 定制过」的岗位。' +
...
function tableBytesOf(store) {
    const bytes = new Map();
    let other = 0;
    try {
        const rows = store.db.prepare('SELECT name, sum(pgsize) AS n FROM dbstat GROUP BY name').all();
        const userTables = new Set(store.db
            .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%'")
            .all().map((row) => asText(row['name'])));
        for (const row of rows) {
            const name = asText(row['name']);
            const size = asInt(row['n']);
            if (userTables.has(name))
                bytes.set(name, size);
            else
                other += size;
        }
...
function countOf(store, sql, ...params) {
    const row = store.db.prepare(sql).get(...params);
    return asInt(row?.['n']);
}
/** 该表当前的真实占用 ÷ 行数（`delete` 类类型的字节估算基准）。 */
function bytesPerRow(store, tableBytes, table) {
...
export function storageUsageOf(store, dataDir, clock = systemClock) {
    ...
    if (raw.other > 0)
        tables.push({ table: '（索引与内部结构）', bytes: raw.other, rows: 0 });
    const types = [
        ...SPECS.map((spec) => {
            ...
            const bytes = spec.mode === 'clear-column'
                ? countOf(store, 'SELECT coalesce(sum(length(jd_text)), 0) + coalesce(sum(length(jd_summary)), 0) AS n FROM job WHERE (jd_text IS NOT NULL OR jd_summary IS NOT NULL) AND last_seen_at < ?', cutoff === '' ? '0000-01-01T00:00:00.000Z' : cutoff)
...
export function cleanupPlanOf(store, clock = systemClock) {
    const policy = readRetentionPolicy(store);
    const now = clock();
    const raw = tableBytesOf(store);
    const items = SPECS.map((spec) => {
        ...
        const bytes = spec.mode === 'clear-column'
            ? rows === 0
                ? 0
                : countOf(store, 'SELECT coalesce(sum(length(jd_text)), 0) + coalesce(sum(length(jd_summary)), 0) AS n FROM job WHERE (jd_text IS NOT NULL OR jd_summary IS NOT NULL) AND last_seen_at < ?', cutoff)
            : Math.round(rows * bytesPerRow(store, raw, spec.table));
        return {
            id: spec.id,
            label: spec.label,
    ...
    return {
        ...
        note: '删除**不会立刻缩小文件** —— SQLite 只是把页标成空闲。执行清理时会跑一次 VACUUM，' +
 ...
 */
export function runCleanupOf(store, options = {}, clock = systemClock) {
    const policy = readRetentionPolicy(store);
    const now = clock();
    ...
    for (const spec of SPECS) {
        ...
        if (item === undefined || !item.willRun)
            continue;
        if (wanted !== null && !wanted.has(spec.id))
            continue;
        const cutoff = cutoffIso(policy[spec.daysKey], now);
        if (cutoff === '')
            continue;
        const changes = spec.mode === 'clear-column'
        ...
        if (rows > 0)
            applied.push({ id: spec.id, label: spec.label, rows, bytes: item.bytes });
    }
 ...
 */
export function maybeAutoClean(store, clock = systemClock) {
    const policy = readRetentionPolicy(store);
    if (!policy.autoCleanEnabled)
        return null;
