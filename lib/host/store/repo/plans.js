 ...
 */
export function keywordsOfPlan(plan) {
...
function toDto(row) {
    ...
    return {
        ...
        keywords: normalizeKeywords(asJson(row['keywords_json'], [])),
...
export function createPlanRepo(db) {
    ...
    return {
        create(input, now) {
            ...
            const result = insert.run(input.name, JSON.stringify(input.platforms), JSON.stringify(normalizePlatformOverrides(input.platformOverrides, input.platforms)), JSON.stringify(input.criteria ?? {}), JSON.stringify(normalizeKeywords(input.keywords)), JSON.stringify(schedule), input.enabled === false ? 0 : 1, detectTimezone(), JSON.stringify(postProcess), now);
        ...
        update(id, patch, _now) {
            ...
            updateStmt.run(patch.name ?? current.name, JSON.stringify(platforms), JSON.stringify(overrides), JSON.stringify(patch.criteria ?? current.criteria), JSON.stringify(normalizeKeywords(patch.keywords ?? current.keywords)), JSON.stringify(schedule), (patch.enabled ?? current.enabled) ? 1 : 0, current.timezone, JSON.stringify(postProcess), id);
            return require(id);
        },
        get(id) {
            const row = selectById.get(id);
