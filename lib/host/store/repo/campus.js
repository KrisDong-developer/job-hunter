...
export function createBranchRepo(db) {
    ...
    const updateCampusStmt = db.prepare(`UPDATE campus_application SET stage = ?, stage_at = ?, batch = ?, apply_open_at = ?, apply_close_at = ?, note = ?, updated_at = ?
     WHERE id = ?`);
    const updateAssessmentStmt = db.prepare('UPDATE assessment SET state = ?, result = ?, due_at = ?, at = ?, updated_at = ? WHERE id = ?');
    const updateTripartiteStmt = db.prepare('UPDATE tripartite SET state = ?, sign_deadline = ?, penalty_summary = ?, updated_at = ? WHERE id = ?');
    return {
        ...
        updateCampusApplication(id, patch, now) {
            ...
            updateCampusStmt.run(stage, stage === record.stage ? record.stageAt : now, patch.batch ?? record.batch, patch.applyOpenAt === undefined ? record.applyOpenAt : patch.applyOpenAt, patch.applyCloseAt === undefined ? record.applyCloseAt : patch.applyCloseAt, patch.note === undefined ? record.note : patch.note, now, id);
        ...
        updateAssessment(id, patch, now) {
            ...
            updateAssessmentStmt.run(patch.state ?? record.state, patch.result === undefined ? record.result : patch.result, patch.dueAt === undefined ? record.dueAt : patch.dueAt, patch.at === undefined ? record.at : patch.at, now, id);
        ...
        createTalkSession(input, now) {
            ...
            return toTalk(selectTalkById.get(asId(result.lastInsertRowid)));
        },
        ...
        updateTripartite(id, patch, now) {
            ...
            updateTripartiteStmt.run(patch.state ?? record.state, patch.signDeadline === undefined ? record.signDeadline : patch.signDeadline, patch.penaltySummary === undefined ? record.penaltySummary : patch.penaltySummary, now, id);
