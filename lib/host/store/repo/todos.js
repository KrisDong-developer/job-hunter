...
export function createTodoRepo(db) {
    ...
    const listOpenFiltered = db.prepare("SELECT id, kind, level, title, ref, detail_json, due_at, state, created_at FROM todo WHERE state = 'open'" +
        ' AND kind = ? AND level = ?' +
        " ORDER BY CASE level WHEN 'urgent' THEN 0 WHEN 'warn' THEN 1 ELSE 2 END, id DESC LIMIT ?");
    const listOpenKind = db.prepare("SELECT id, kind, level, title, ref, detail_json, due_at, state, created_at FROM todo WHERE state = 'open'" +
        ' AND kind = ?' +
        " ORDER BY CASE level WHEN 'urgent' THEN 0 WHEN 'warn' THEN 1 ELSE 2 END, id DESC LIMIT ?");
    const listOpenLevel = db.prepare("SELECT id, kind, level, title, ref, detail_json, due_at, state, created_at FROM todo WHERE state = 'open'" +
        ' AND level = ?' +
        " ORDER BY CASE level WHEN 'urgent' THEN 0 WHEN 'warn' THEN 1 ELSE 2 END, id DESC LIMIT ?");
    ...
    return {
        ...
        listOpen(filters) {
            if (typeof filters === 'number') {
                return selectOpen.all(filters).map(toRecord);
            }
            const options = filters ?? {};
            const limit = options.limit ?? 50;
            if (options.kind !== undefined && options.level !== undefined) {
                return listOpenFiltered.all(options.kind, options.level, limit).map(toRecord);
            }
            if (options.kind !== undefined) {
                return listOpenKind.all(options.kind, limit).map(toRecord);
            }
            if (options.level !== undefined) {
                return listOpenLevel.all(options.level, limit).map(toRecord);
            }
