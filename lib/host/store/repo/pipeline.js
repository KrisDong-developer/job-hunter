...
export function createPipelineRepo(db) {
    const insertTemplate = db.prepare(`INSERT INTO greeting_template (name, body, vars_json, scene, uses, replies, created_at, updated_at)
     VALUES (?, ?, ?, ?, 0, 0, ?, ?)`);
    const updateTemplate = db.prepare('UPDATE greeting_template SET name = ?, body = ?, vars_json = ?, scene = ?, updated_at = ? WHERE id = ?');
    ...
    const insertQuestionNote = db.prepare(`INSERT INTO question_note (question, my_answer, better_answer, topic, interview_id, times, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 1, ?, ?)`);
    const selectQuestionByText = db.prepare('SELECT * FROM question_note WHERE question = ? AND topic = ? ORDER BY id DESC LIMIT 1');
    const bumpQuestion = db.prepare('UPDATE question_note SET times = times + 1, my_answer = ?, better_answer = ?, updated_at = ? WHERE id = ?');
    ...
    return {
        // ── 模板 ──────────────────────────────────────────────────────
        listTemplates() {
            return selectTemplates.all().map(toTemplate);
        },
        upsertTemplate(input, now) {
            if (input.id === undefined) {
                const result = insertTemplate.run(input.name, input.body, JSON.stringify(input.vars ?? []), input.scene ?? '', now, now);
        ...
        createGreeting(input, now) {
            const result = insertGreeting.run(input.jobId, input.platformId, input.templateId ?? null, input.content, now, input.channel ?? 'platform', input.actor, 'greeted', now, now);
        ...
        upsertQuestionNote(input, now) {
            ...
            const result = insertQuestionNote.run(input.question, input.myAnswer ?? '', input.betterAnswer ?? '', topic, input.interviewId ?? null, now, now);
