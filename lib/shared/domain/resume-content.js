export const SKILL_LEVELS = ['了解', '熟悉', '熟练', '精通'];
/** 一份空简历：新建版本时的起点，也是"结构合法"的基准。 */
export function emptyResumeContent(direction = '') {
    return {
...
function stringListOrUndefined(value) {
...
export function normalizeResumeContent(input) {
    ...
    return {
        ...
        skills: (Array.isArray(raw['skills']) ? raw['skills'] : []).map((item) => {
            ...
            return withOptional({ name: text(record['name']).trim() }, {
                level: SKILL_LEVELS.includes(level) ? level : undefined,
        ...
        projects: (Array.isArray(raw['projects']) ? raw['projects'] : []).map((item) => {
            const record = asRecord(item);
            return withOptional({ name: text(record['name']).trim(), highlights: stringList(record['highlights']) }, {
        ...
        education: (Array.isArray(raw['education']) ? raw['education'] : []).map((item) => {
            const record = asRecord(item);
            return withOptional({ school: text(record['school']).trim() }, {
...
export function isResumeContentUsable(content) {
    return (content.basics.name.trim() !== '' &&
        (content.experiences.length > 0 || content.projects.length > 0 || content.skills.length > 0));
}
export function inspectResume(content) {
    const issues = [];
...
function collectTech(text, into) {
...
export function flattenResumeText(content) {
    const parts = [content.basics.name, content.basics.title, content.summary];
    for (const skill of content.skills)
        parts.push(skill.name, skill.evidence ?? '');
    ...
    for (const education of content.education)
        parts.push(education.school, education.major ?? '', education.degree ?? '');
    for (const extra of content.extras)
        parts.push(extra.label, extra.text);
    return parts.join('\n');
}
/**
 * 走遍所有会产生"事实原子"的字段。
 ...
 */
function walkTextAtoms(content, tech, numbers) {
    collectTech(content.basics.name, tech);
    collectTech(content.basics.title, tech);
    if (typeof content.basics.years === 'number')
        collectNumbers(String(content.basics.years), numbers);
    ...
    for (const experience of content.experiences) {
        ...
        for (const stack of experience.stack ?? [])
            collectTech(stack, tech);
...
function walkOrgs(content, orgs) {
    for (const experience of content.experiences)
        orgs.add(experience.company.trim());
    for (const project of content.projects)
        orgs.add(project.name.trim());
    for (const education of content.education)
        orgs.add(education.school.trim());
    orgs.delete('');
}
export function factAtoms(content) {
    const tech = new Set();
 ...
 */
function structuralAdditions(original, candidate) {
    ...
    for (const experience of original.experiences) {
        ...
        if (company !== '')
            beforeOrgs.add(key(company));
        if (company !== '' || title !== '')
            beforePairs.add(key(`${company}@${title}`));
    }
    for (const project of original.projects)
        if (project.name.trim() !== '')
            beforeOrgs.add(key(project.name));
    for (const education of original.education)
        if (education.school.trim() !== '')
            beforeOrgs.add(key(education.school));
    ...
    for (const experience of candidate.experiences) {
        ...
        if (!beforePairs.has(pair))
            orgs.push(`${experience.company.trim()}｜${experience.title.trim()}`);
    }
    for (const project of candidate.projects) {
        if (project.name.trim() !== '' && !beforeOrgs.has(key(project.name)))
            orgs.push(project.name.trim());
    }
    for (const education of candidate.education) {
        if (education.school.trim() !== '' && !beforeOrgs.has(key(education.school)))
            orgs.push(education.school.trim());
    }
    return { skills, orgs };
}
/**
 ...
 */
export function checkNoFabrication(original, candidate, options = {}) {
    ...
    const reasons = [];
    const addedOrgs = [...new Set([...after.orgs, ...structure.orgs].map((name) => name.trim()))].filter((name) => name !== '' && !before.orgs.has(name));
...
export function techTokensOf(text) {
    const set = new Set();
...
export function resumeFileName(content, format) {
    const parts = [content.basics.name, content.basics.title];
    if (typeof content.basics.years === 'number')
        parts.push(`${String(content.basics.years)}年`);
...
// DTO（给界面与工具）
// ─────────────────────────────────────────────────────────────────────
//# sourceMappingURL=resume-content.js.map