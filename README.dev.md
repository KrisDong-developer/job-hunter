...
## P21 数据保留、清理与可携带性（2026-09-19）

§18（数据清理）+ J8（数据可携带）此前**一条都没实现**：文档里有 `store/cleanup.ts` 与
`POST /maintenance/cleanup` 的规划，代码里两者都不存在；数据一旦想搬走只能手抄。
这一批把它们落地，判据同样是"链路真的通"而不是"接口存在"。

### 1. 先调研出来的四个事实（决定了实现形状）

| 事实 | 怎么查的 | 后果 |
|---|---|---|
| **`dbstat` 可用** | `node:sqlite` 的 SQLite 3.49，`SELECT name, sum(pgsize) FROM dbstat GROUP BY name` 实测返回 | 磁盘占用能给**真实分页字节数**，不必估算；拿不到时如实标"不可用"，不编数字 |
| **`docx.ts` 里已有自建 ZIP** | 读代码 | 抽成 `render/zip.ts` 复用做导出归档（零依赖不变） |
| **请求体上限 64KB** | `http.ts` 的 `readBody` | 导入必须**按行分批**，不抬这个安全常量 |
| **`job` 的子表外键分两类** | `schema.ts` 的 `REFERENCES job(id)` | `job_flag`/`visa`/`cover_letter` 是 CASCADE（派生，可随删）；`application`/`greeting`/`message`/`interview`/`tailoring`/`campus` 是 **SET NULL** —— 删父行会把用户资产变成**孤儿**，必须显式排除 |

### 2. §18：保留策略 + 占用 + 预览 + 清理

`store/cleanup.ts` 一处收口：策略读写、占用统计、预览、执行。
**类型表**是唯一来源（标签、保留键、所在表、条件、清它会失去什么），计数与删除共用同一个 `where`。

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/maintenance/storage` | 文件/WAL 真实大小 + 附件目录 + 按表 dbstat + 按类型行数（长期保留的也在列） |
| POST | `/maintenance/cleanup/preview` | **只读无副作用**，给出每类将删多少行 / 预计释放多少 |
| POST | `/maintenance/cleanup` | 需 `confirm: true`；执行后 `VACUUM`；要求持有租约 |

三条落进代码的铁律：

* **删除不等于释放（R16）**：执行完必须 `VACUUM`，结果里给 **VACUUM 前后的真实文件大小**；
  预览里明说"删了不会立刻变小，这里给的是估算"。
* **用户资产不自动删**：投递 / 打招呼 / 消息 / 面试 / 简历与附件**连保留期配置都没有**。
  占用页把它们列为「长期保留」—— 用户问"会不会删我的投递记录"时，界面能指着那一屏回答。
* **删父行会制造孤儿**：岗位清理带整组 `NOT EXISTS`（有投递/打招呼/消息/面试/定制/校招关联的、
  收藏过的、`jd_text` 还在的，一律不删）。所以顺序是"先清 JD 字段、再删空壳行"，两步不重叠。

**与 §18.2 的两处有意偏差**（都写进了 ARCHITECTURE）：

* 没有 `snapshot_ref` / 截图这两类 —— 实现里**不存原始 HTML 与截图**（§18.1 的"几十 GB"估算
  在实现里不成立），体积大头其实是 `jd_text`，它单独成一类（默认 180 天）。
* **定时清理默认关闭**（§18.3 只要求"可关闭"）。删除不可逆，所以让用户先手动跑一次预览再打开；
  开启后只在**启动时**跑一次 —— `VACUUM` 可能阻塞几百毫秒到几秒，塞进心跳迟早出事。

### 3. J8：导出三种格式、导入一种

| 格式 | 内容 | 读者 |
|---|---|---|
| `json` | 全部业务表的原始行 + `meta`（版本/时间/计数） | **真备份**：换机器、换版本都还能读 |
| `csv` | `csv/*.csv` 打成一个 zip（jobs / companies / applications / greetings / messages / interviews） | 人 / Excel |
| `archive` | `data.json` + `csv/*` + `files/*`（简历附件原文件）+ `README.txt` | 搬家 |

* **CSV 必须带 UTF-8 BOM**：不带的话中文 Excel 按 ANSI 码页解码，中文全乱 → 用户会认为导出是坏的。
* 导入只做**岗位**，且**幂等**（键 = 来源链接；没有链接时用「标题|公司|城市」）；
  导入的行挂在一个虚拟平台「手动导入」下（`platform` 表登记一行，于是 `JOBS` 列表里能看出处），
  并立刻走一遍 `intel.evaluateJob` —— 否则用户在列表里看到一批"没有分"的岗位会以为导入失败了。
* **三类输入明确拒绝**（不假装支持）：`.xlsx` 二进制（zip 魔数）、非 UTF-8（出现替换字符，中文 Excel 默认 GBK）、
  表头里没有"岗位标题"列。理由一样：脏数据进了岗位库，比报错糟得多。

### 4. 工具面与界面

* 工具 `data_transfer`（`storage` / `cleanup_preview` / `export` / `import`），导出**落盘**到
  `<dataDir>/exports/` 并返回路径，导入**只接受该目录下的文件名**（`basename` 之外一律拒 —— 与
  `system/reveal` 同一条不接受任意路径的纪律）。
* **刻意不给"执行清理"这个工具**：删除不可逆，界面上的执行有三步（预览 → 勾选 → 弹窗确认），
  模型这边只给只读的预览。比 §22.4 的"过审批"更保守一点，理由写在 `tools/data.ts` 文件头。
* 界面：设置页新增第三个分区「数据与存储」（保留期表单 + 占用 + 预览/执行 + 导出下载 + 导入）。
  导入按行分批（300 行/次）；**内容里有引号时不分批** —— 引号字段里可能有换行，
  按行切会把一行切成两半，那比拒绝导入糟得多。

### 出口标准怎么验的

`test/http/data-routes.test.ts`（19 条），断言的是行为而不是状态码：

* **预览无副作用**：连调两次预览，行数与数据都不变；
* **用户资产一条没动**：造一条 700 天前的投递 + 打招呼（挂在岗位上）与一个"从没被碰过"的老岗位，
  清理后前者与它岗位的 JD 都在、后者按规则处理；
* **VACUUM 真的跑了**：`vacuumed === true` 且 `dbBytesAfter <= dbBytesBefore`；
* **CSV 往返**：BOM、逗号、引号、**字段内换行**逐项还原（手写解析器最容易错的三处）；
* **导入幂等**：同一份表导两次 → 第二次 `inserted = 0 / updated = 2`；
* **拒绝三类脏输入**，且拒绝之后岗位库仍是空的；
* **只读实例**：伪造一份"别的进程刚心跳过"的租约文件，断言清理/导入 409、而**导出仍然可用**
  （备份不该被租约挡住）。

**typecheck / build / verify 19/19 / test 945 通过。**

### 仍未做（如实记账）

| 项 | 为什么还没做 |
|---|---|
| **`.xlsx` 直接导入** | 需要解包 + 解析共享字符串表 + 单元格引用，做错会把脏数据写进岗位库。J8 的"从 Excel 导入"目前走 CSV（Excel 的通用交换格式），拒绝时给的是可执行的下一步 |
| **整库覆盖式恢复** | 会用备份里的旧数据盖掉当前数据（包括你刚投的岗）。真要整库搬走，直接换 `data.db` 更直接（路径在 README 里） |
| **清理前自动导出归档**（§18.3 P4） | 需先把"归档"接到清理流程里并决定放哪、留几份；本批未做 |
| **硬上限保护**（§18.3 P5） | 需要先有"磁盘阈值"这一个用户设置与触发时机（启动？每轮采集后？），属独立一次改动 |
| **单个岗位永久保留快照**（§18.3 P6） | 依赖"存快照"这件事本身（当前不存） |
| **一键清空全部数据**（§18.3 P7） | 它要删的正是"只由用户显式删除"的那些资产（简历与附件），与 §18.2 的取向相反。目前的诚实替代是：占用页给出**数据目录路径**，README 也写着"要清就自己删这个文件夹"。若要做，应做成"归档一份 → 再清"，而不是一个能一键抹掉的按钮 |
| **`/diagnostics/export` 诊断包** | 与"数据导出"不是一回事（那个含日志与错误证据），单独一批 |

> 已完成但值得点名的两条：**定时清理**（§18.3 P1，默认关、可在设置里打开）与
> **清理前预览**（§18.3 P2，**P0**）。

---

## P22 批量打招呼：D3 从"开关形同虚设"到全链路（2026-09-19）

D3（批量打招呼，P1）此前只有**半条链**：闸门里的 `checkBatch` 已经会按 `payload.jobIds` / `payload.count`
判上限，但**没有任何批量发送入口** —— 唯一生产 `jobIds` 的是 `job_match_explain` 工具，
所以那个上限永远不会被触发，L3 开关也就没什么可管的（对比 `POST /jobs/batch/mark` 是有的）。
这一批把它接通，判据同样是"链路真的通"。

### 1. 先调研出来的三个事实（决定了实现形状）

| 事实 | 怎么查的 | 后果 |
|---|---|---|
| **只有 BOSS（zhipin）实现了 `actions.sayHello`** | 读 `platform/adapters/*.ts`：`zhipin.ts` 有实现，`liepin`/`lagou`/`guopin`/`sinojobs`/`waiqi-job`/`indeed` 都写着"刻意不实现"；`zhaopin` 是"IM 走网易云信私有 WS，没有独立的打招呼动作" | 跨平台勾一批的真实结果是"能发几条、其余发不了" → 预览必须**逐条**说清为什么，否则用户会以为批量坏了 |
| **`checkBatch` 只在 `actor === 'model'` 时生效** | 读 `guard/rules.ts` | 界面不需要它兜（界面自己按 `BATCH_MAX_ITEMS` 分批）；**模型必须兜**，且必须真的兜住 |
| **批量是逐条调 `sendGreeting`** | 读 `runtime/actions.ts` | 每条只有 1 个岗位 → `payload.count` 缺失 → `checkBatch` 永远不触发。"逐条过闸门"会变成"逐条绕过批量上限"，所以 `sendOne` 要带上 `batchSize` |

### 2. 两个上限，两条理由（都不是洁癖）

`BATCH_MAX_ITEMS = 5`、`BATCH_ITEM_INTERVAL_MS = { min: 3000, max: 9000 }`
（原名 `GREETING_BATCH_MAX` / `GREETING_BATCH_INTERVAL_MS`，L4 批量投递落地时收口成这两个名字）：

* **5 条**来自两个互相独立的约束：**请求时长**（每条要开页面、逐字符输入、校验送达，实测秒级；
  再加条间 3–9 秒，一次塞 20 条就是十几分钟不返回）与**审批粒度**（§4.4.2 要求确认时看到正文全文，
  一次问 20 条人只会无脑点确认，那等于没有审批）。
* **间隔放在宿主**而不是界面：模型工具与（将来的）定时任务走同一条路径，绕不过去。
  两次独立请求之间的空档宿主管不到 —— 那一段由界面在分批之间补上（见 §6）。

### 3. 预览：预测必须准，否则它就是装饰

`runtime/greeting-batch.ts` 的 `previewGreetingBatch` 是五步，**顺序是刻意的**：

1. **平台能力 + 登录态**（`canSend`）—— 不满足时连话术都不生成。**能力先于登录**：
   "平台不支持"比"未登录"更接近事实，先说后者会让人白登录一次；
2. **闸门预检**（`guard.preview`）—— 只读、不写审计；
3. **批内同公司去重**（`duplicate_company`）；
4. **批内额度预占**（`quota_exhausted`）；
5. 到这一步**才**生成话术（`draft_failed`）。

第 3、4 步不是查表，是照 `checkCooldown` / `checkQuota` 的口径**真实模拟一遍**：同公司第一条通过
闸门的先把名额占掉；额度余量直接读 `guardUsageOf`（与判定**同源**）。各算一份必然漂移，
而漂移的表现恰恰是"预览说能发 3 条、点下去第 2 条就被拒" —— 那比没有预览更恼火。

### 4. 执行：逐条回执，永不整批失败

* 每条独立 try/catch → `receipts` 与传入顺序**一一对应**。批量最糟的形态是"第 3 条挂了，
  剩下 17 条不知道发没发"；
* 间隔插在**条与条之间**：第一条不等（用户刚点完确认），最后一条后面也不等；
* `sendOne` 带 `batchSize` → 透传为 `payload.count`（否则见 §1 的第三个事实）。

### 5. 抽出一份共用判据

`greetingPlatformBlocker` / `greetingReadinessOf`（`guard/actions/greeting.ts`）——
单条发送与批量预览**共用同一份**"这个平台现在能不能发"。两张皮迟早漂移，
而漂移正是"预览说能发、点下去才失败"。

### 5.5 一个必须写清楚的语义：选了附件 ≠ 平台会收到它（2026-09-19 补）

这一格**返工过一次**，记下来免得再绕：

* **第一版**把 `resumeFileId` 当成"要上传的文件"：平台只吃平台内简历时**逐条拦下**
  （`local_resume_unsupported`），界面上干脆不给选择器，理由是"能选但必然失败的下拉比不做更糟"。
* **但那样这个参数就等于废掉了** —— 而"这次投递对应哪一版简历"恰恰是 R6/归因**真的需要**的信息
  （`application.resume_id` 就是干这个的；"哪版回复率高"要看它）。缺了它，选择器与这个参数都没意义。
* **现在的口径**：把两件事分开 —— **登记**（写进记录）与**上传**（真的交给平台）：

  | 平台事实 | 实际行为 |
  |---|---|
  | `resumeSource: 'local'` | 把解析出的绝对路径交给适配器，**真的上传**（今天还没有这样的平台） |
  | 其余（`'platform-only'`，zhipin / zhaopin 实测） | **不传路径**（传了适配器只会 fail-closed，把一次本来能成的投递变成失败），照投 —— 平台用它自己那份；那份简历只作**本地登记** |

* 所以"选了附件"**不是**拦截条件，而是一句必须说清的话。三处都说了：
  预览的 `uploadsResumeFile`（结构化 —— 界面据此显示醒目的一句，而不是让人去读散文）、
  审批文案的「使用简历版本：X（平台只吃它自己那份简历 —— 这份文件不会上传，只作本地登记）」、
  以及选择器的 `title` 与 `?`。
  **说法只有一份**（`resumeVersionTextOf`），单条审批与批量预览的闸门预检共用它 ——
  两处各写一句的话，用户在预览里看到的与在确认框里看到的迟早不一样。
* 顺带修掉一个归因缺口：投递成功后落库以前**不回填** `resumeId`/`resumeFileId`
  （`recordApplicationSent` 会退回"当前默认简历"）。现在**不认识就记 null**，
  而用户显式选的那一版如实记进去 —— 这正是选择器存在的意义。

### 6. 入口

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/greeting/send-batch/preview` | 只读无副作用：逐条给出"能不能发 + 为什么 + 将发出什么" |
| POST | `/greeting/send-batch` | 必须 `confirm: true`（否则 400，`hint` 指向预览）；逐条过闸门、逐条回执 |

* **工具面**：扩的是 `greeting_send` 的 `jobIds`（**不新开工具** —— 同类操作合并）。
  run 里先在工具层挡一道 `guard.batchLimit`：闸门里每条也会挡，但那样用户要等完
  N 个 3–9 秒的间隔、最后拿到一屏"超过上限"。模型侧**每条单独问一次审批**（`actor='model'`
  时由 `guard.run` 内联 `approval.ask`），这是刻意的：一次问 5 条会让人看不清自己在批准什么。
* **界面**：岗位库列表加勾选列（含"选中本页"）+ 工具条（已选 N 条 / 批量打招呼 / 清除选择）；
  `batch-greeting-modal.tsx` 打开即拉预览 → 逐条可勾选、可改正文、可单条重新生成 →
  **一次确认**（同一屏已看过全部正文）→ 按 `batchMax` 串行分批 → 逐条回执。
  发送中**禁用关闭**：关掉不会撤回已经发出的那几条，用户会以为"关了等于没发"。

### 出口标准怎么验的

* `test/host/greeting-batch.test.ts`（**8 条**）：注入依赖测纯逻辑 —— 只为能发的项生成话术、
  两种批内预测、逐条回执顺序与"中间失败不影响后面"、间隔只在条与条之间（3 条 = 2 次、单条 0 次）、
  `batchSize` 透传（断言 `[2, 2]`）、上限与空列表直接拒绝；
* `test/http/greeting-batch.test.ts`（**9 条**）：真实 runtime + 路由。`liepin → platform_unsupported`
  这一条断言同时**钉住了"只有 BOSS 实现了 sayHello"这个平台事实**（它一旦变化，这里会先响）；
  预览只读（打招呼行数与审计都没变）；跨站 403；字面量优先（`send-batch` 不会被 `/greeting/send` 吃掉）。

**typecheck / build / verify 19/19 / test 全绿（962 通过）。**

### ⚠️ 写测试时踩到的两件事（写给下一个改这里的人）

1. **要走到"发送"那一步，先得让闸门放行。** `greeting.send` 是 `danger: 'high'`，
   而检查链第一梯队里就有 `checkStealth` —— 它要求"已确认对当前雇主隐藏"。
   我最初写的 http 用例没做这一步，拿到的是 `GUARD_DENIED`（未确认隐身），
   而断言写的是 `NOT_LOGGED_IN`。**这次是断言错了，不是代码错了**：闸门在前是有意的，
   `p5-routes.test.ts` 有一条"发送打招呼：隐身没过就先被闸门拒（不会白问用户一次）"专门守着这个顺序。
   顺带发现这类用例还**必须关掉 `sendWindow` 与 `dayOffProbability`** —— 它们按本地墙钟判定
   （默认窗口 `09:00-16:00`），晚上跑测试会得到"现在不在发送窗口内"，那是典型的定时炸弹。
2. **`company.ensure` 要 `nameNorm`**（身份键是归一化后的名字），测试数据直接传原名即可。

### 仍未做（如实记账）

| 项 | 为什么还没做 |
|---|---|
| **批量投递**（L4，与 D3 同族） | 它要选简历、要决定"每岗是否定制版本"，审批文案（§4.4.2 的"用了哪版简历"）与打招呼不是一套。`POST /jobs/batch/mark` 那种"批量改状态"不对外发送，不能照搬 |
| **单条"发送打招呼"按钮** | 至今只有 `greeting_draft` 生成话术，界面上仍没有单条发送入口（批量入口这一批补齐了）。缺的是一个"发送前可编辑正文 + 两段式确认"的轻量弹窗，与本批的批量弹窗是两份不同交互 |
| **登录后自动补发** | 预览里那些 `not_logged_in` 的条登录后确实能发了，但"自动补发一批"是一种**自动对外发送**，与 L3 默认关闭的取向冲突。当前做法是登录后重新打开弹窗、重新预览 |
| **"每平台今天还剩几条"的独立面板** | D3 原文的四项里，限速与随机间隔落在宿主、每日上限仍走闸门的两层取小（`guardUsageOf`）并在预览里给了余量数字；但没有做成一张按平台列出的面板（现在的落点是「今日」页的额度卡 + 预览里的数字） |

---

## P23 L4 批量投递：从"只是一个开关"到全链路（2026-09-19）

L4（批量投递简历）此前**只有开关**：闸门里的 `application.send` 与单条动作都在，
但**界面上没有任何真投递的入口**（客户端 `deliverApplication` 零调用点），也没有批量入口 ——
于是 §22.5 的"GUI 与工具对等"在投递这一格是空的，D-3 的 L4 开关也没什么可管的
（与 P22 的 D3 一模一样的病）。
...
| 事实 | 怎么查的 | 后果 |
|---|---|---|
| **只有 `zhipin` 与 `zhaopin` 实现了 `actions.sendResume`** | 读 `platform/adapters/*.ts`（其余都写着"刻意不实现"） | 跨平台勾一批的真实结果是"能投的没几个"→ 预览必须逐条说清 |
| **这两个实现对非空 `filePath` 一律 fail-closed** | 读两个 `sendResume` 实现 | "上传本地简历"今天**一个平台都走不通**→ 不能让用户以为选了就能用 |
| **`POST /applications/deliver` 收的是任意 `filePath` 字符串，HTTP 层零校验** | 读 `routes/applications.ts` | 既不合"不接受任意路径"的纪律（`readExportFile` 的先例），又让 §4.4.2 的"用了哪版简历"退化成一句路径 |
| **投递成功后落库不回填 `resumeId`/`resumeFileId`** | 读 `runtime/actions.ts` 的 `record` 回调与 `pipeline.recordApplicationSent` | R6 的"这版投了哪个岗"在适配器投递路径上**不准**（会退回"当前默认简历"） |
| **BOSS 的「发简历」要双方回复之后才可用** | `OPTIMIZATION-PLAN` §7 / `platform-facts` | 能投的实际只有 zhaopin；BOSS 那侧会如实失败（平台侧条件，不是工具缺陷） |

### 2. 顺手修掉的三处既有缺陷（不修的话 L4 说不了真话）

1. **`filePath` → `resumeFileId`**：外部只给**附件 id**，路径由宿主自己从库里查
   （`join(filesDir, path)`）。HTTP 与工具层对非整数一律 400 —— 测试里直接钉住
   "body 里塞一个 `C:\...\resume.pdf`，确认文案里**不出现任何绝对路径**"。
   审批文案改成"简历名 · 附件名"：§4.4.2 要的是**版本信息**，不是本机目录结构。
2. **归因不再猜**：`recordApplicationSent` 以前会退回"当前默认简历"，而适配器投出去的是
   **平台上那一份** —— 这会让"按简历版本看转化率"把结果算到一份可能根本没投出去的简历上。
   现在**不认识就记 null**。这个函数的注释本来就是这么写的（"如实记 null 比编一个版本号好"），
   是实现没跟上（又一处"注释描述了正确意图、实现没跟上"，同坑 15）。
3. **一句陈旧提示**：`ADAPTER_BROKEN` 的 hint 以前写"目前只有 zhipin 实现了投递"，而 zhaopin 也实现了。

### 3. 与批量打招呼共用一套机制

新增 `runtime/batch.ts`：**批内同公司去重 / 批内额度预占 / 条间随机间隔 / 条数上限校验 / 失败翻译**。
这几件判定最怕两处各写一份（漂移的表现就是"预览说能投、点下去就被拒"），所以收口；
而"这一个是什么、怎么判能不能发、回执长什么样"留在各自的编排模块里 —— 那才是真正不同的部分。

同时把 `GREETING_BATCH_MAX` / `GREETING_BATCH_INTERVAL_MS` 收口成
`BATCH_MAX_ITEMS` / `BATCH_ITEM_INTERVAL_MS`（两个动作用同一个数与同一段间隔，理由同源；
分两个名字只会给"改了一个忘了另一个"留机会）。

### 4. 预览：五步，顺序是刻意的

**① 平台能力 + 登录态**（能力先于登录）→ **② 简历来源**（平台收不收本地附件）→
**③ 闸门预检** → **④ 批内同公司** → **⑤ 批内额度预占**。

第 ② 步是投递独有的，也是这一批最要紧的一格：`resumeSource: 'platform-only'` 的平台
（zhipin / zhaopin，**实测**）在"指定了本地简历附件"时被逐条标成 `local_resume_unsupported`，
并给出**可执行的下一步**（把关联简历清成"平台内简历"即可，或先去平台上换）。

这一格来自**新的平台事实** `PlatformFacts.resumeSource`，不是写死在编排里的 ——
将来有平台支持上传时改一处事实就通了。而"光知道适配器实现了 `sendResume` 不够"
这条，是两个适配器对非空路径 fail-closed 的实测结论换来的。

### 5. 执行：回执里多一格送达状态

投递与打招呼最大的区别是**不可逆**，所以回执带 `delivery`：
`pending`（已发出·未确认）必须与 `failed` 分开说 —— 前者要先上平台核对，直接重投可能投两遍。
批级 `note` 会把"其中 N 条只到已发出·未确认"单独拎出来，界面上它走 `jh-warn` 而不是 `jh-ok`。

顺带把 `DeliveryState` 从 `host/platform/types.ts` 挪进 `shared/enums.ts`（补上
`DELIVERY_STATE_LABEL` / `DELIVERY_STATE_TONE`）：界面在此之前**根本看不到送达状态**——
投递与打招呼的结果只有"成功"或"异常"，而"点了按钮"与"真的进了对方会话"是两件事（§12.2）。

### 6. 入口

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/applications/deliver` | 单条真投递（两段式确认）；收 `resumeFileId`（**不接受路径**） |
| POST | `/applications/deliver-batch/preview` | 只读预览：逐条"能不能投 + 为什么 + 用哪份简历" |
| POST | `/applications/deliver-batch` | 需 `confirm: true`；逐条过闸门、逐条回执 |

* **工具面**：扩的是 `application_deliver` 的 `jobIds` + `resumeFileId`（**不新开工具**）。
  run 里先挡 `guard.batchLimit`（否则用户要等完 N 个 3–9 秒再拿到一屏"超上限"），
  再逐条投；模型侧每条各自过审批（`actor='model'` 时由 `guard.run` 内联 `approval.ask`）。
  回执文本逐条给送达状态，并明写"写「已发出·未确认」的那些先去核对、不要重投"。
* **界面**：
  * 岗位库勾选后新增「**批量投递**」→ 弹窗（选简历 → 预览 → 逐条可跳过 → 一次确认 → 按 5 条分批 → 逐条回执）；
  * 岗位详情新增「**投递简历**」→ 两步弹窗（先选简历，再看宿主的确认文案），
    **确认文案原样来自宿主**（含平台 / 岗位 / 用了哪版简历 / 这份文件传不传得上去 /
    平台会顺带做什么），多行文本保留换行（`.jh-approval`）。
    在此之前 `deliverApplication` 是死代码，§22.5 的 GUI/工具对等就在这一格空着。
  * 两处共用 `screens/resume-file-picker.tsx`：选项是「平台内简历（不登记版本）」+ 各版导出的附件
    （`resume_file.id`）。选项落在**附件**上，因为投递这条链路的输入就是附件 id，
    而服务端由它反推出所属版本写进 `application.resume_id`（R6 的归因靠这一格）。
    为此 `ResumeSummaryDto` 补了 `files`（**不含磁盘路径**，只有 id/文件名/字节/时间）——
    列表不带 id 的话，"选哪份简历"就只能靠"每版再请求一次"拼出来。

### 出口标准怎么验的

* `test/host/application-batch.test.ts`（**10 条**）：注入依赖测纯逻辑 ——
  `uploadsResumeFile` 的三态（指定了附件 + 平台只吃自己那份 ⇒ `false` 且**照投** /
  平台接受上传 ⇒ `true` / 不指定 ⇒ `null`）、两种批内预测、逐条回执顺序与"中间失败不影响后面"、
  `pending` 被单独说出来、间隔只在条与条之间（3 条 = 2 次、单条 0 次）、
  `batchSize` 透传（断言 `[2, 2]`）、上限与空列表。
* `test/http/application-batch.test.ts`（**13 条**）：真实 runtime + 路由。
  最要紧的三条：① `POST /applications/deliver` 的 body 里塞一个绝对路径，断言**确认文案里不出现它**
  —— 这条钉住"路径不再是输入"；② 指定附件时确认文案**必须写出"这份文件不会上传"**；
  ③ `51job → platform_unsupported` 与 `zhaopin → not_logged_in`
  两条各自成回执而不整批失败（**同时钉住了"只有 zhipin/zhaopin 实现了投递"这个平台事实**）。
  另有：预览只读（不写投递记录、不写审计）、跨站 403、字面量优先。
* `test/domain/resumes.test.ts` 补一条：列表带上附件 id，**但不带磁盘路径**（`path` 不出 DTO 边界）。

**typecheck / build / verify 19/19 / test 全绿（994 通过）。**

### 仍未做（如实记账）

| 项 | 为什么还没做 |
|---|---|
| **本地附件真的传上去** | 现在**没有任何平台**接受本地附件（zhipin / zhaopin 实测都没有"把本地文件发给 HR"的入口）⇒ 选了附件只会作**本地登记**（见 §5.5）。链路已经就位（id → 宿主解析路径 → 适配器），等真有平台支持上传，改一处平台事实 `resumeSource` 就通了 |
| **按简历版本的 A/B 归因** | 归因表本来就在（`/analytics/attribution`），`application.resume_id` 现在也能被用户显式选出来；但"平台侧到底用了哪一版"仍只有用户自己知道 —— 要更硬的口径得等平台允许指定投哪一版 |
| **BOSS 上实际投不出去** | 平台要求"双方回复后"才开放「发简历」（按钮带 `unable` + aria-label 明写），而当前账号的会话都是我们单方面发出。这是平台条件，不是工具缺陷；预览不会假装能投 |
| **"无人值守"的自动投递** | D-19 明确"定时只做准备，不发送、不投递、不回复"。所以 L4 的"自动"= **批量 + 一次确认**，不是"后台自己投"。真要无人值守，那是另一条产品线，且与 D-3 的取向冲突 |
| **单个岗位"再投一次"的显式出口** | 冷却期（默认 24 小时）与每日额度会拦住重复投递，界面没给"确认要重投"的按钮。投递不可逆，宁可先不做 |
...
17. **「参数传了」不等于「参数被用上了」**（2026-09-19 真实线上故障抓到）。
    `PageLike` 声明的是 `waitForSelector(selector, timeoutMs: number)`，而真路径透传的是
    Playwright 的 `waitForSelector(selector, options?)` —— 第二个参数是**配置对象**。
    传个数字过去不报错、不告警，`options.state` 与 `options.timeout` **双双取不到**：
    每个适配器自己调的 `waitForListMs` 全部失效（实际是 `visible` + 默认 30s），
    而"可见"这个隐含门槛还会在窗口不给布局时假失败。
    这类偏差 `tsc` 也拦不住 —— 两边的**参数个数一样**，而运行期的对象是从
    Playwright 的类型**断言**（不是赋值）转过来的，结构检查早就绕过了。
    故障现场极具误导性：报的是「连搜索页都没打开成功」，Playwright 却说
    *locator resolved to 42 elements* —— 网络与页面都好好的。
    **处置**：归一化收在中心（`browser.ts` 的 `normalizeWaitForSelector`，等 `attached`、
    超时返回 `false` 不抛错），让真路径与 jsdom 夹具**同形**；
    **验证要用负对照**：`waitForSelector('#不存在', 1500)` 必须在 ~1.5s 返回 `false`
    （修复前会等满 30s 再抛）—— 这条比"能跑到数据"更能证明参数真的生效了。
