...
| 单测基线 | **913 个（912 通过 / 1 跳过）**（`npm test`，离线，绝不访问真实招聘站）。2026-09-19 更新（`runtime.ts` 从 2074 行拆到 1310 行：`runtime/{contract,gate,adapters,views,actions,lifecycle}.ts`；`openDataLayer` 拆成 6 个有序阶段，并把「模型层必须先就绪」写进**阶段签名**；领域规则 `syncDeadlineTodos` / `resumeProfileOf` 下沉到 `domain/{campus,intel}.ts`。另**修掉一个静默接线 bug**：`resumes` 比 `ai` 先建 → 简历定制永远退回规则路径（`via='rule'`，不报错不留痕），现由 `test/host/ai-wiring.test.ts` 钉住「配了模型就必须走 `via='llm'`」。单测增量：平台门判定 +10 条；`city-gate.test.ts` 的"复刻的门"换真实现；+1 条"三个入口对同一份健康事实说法一致"；+2 条 close() 后逐个访问器拒答（**已验证会红**）；+1 条 `/health` 版本号来自包清单（**已验证会红**）；+3 条 `resumeProfileOf`；+2 条模型接线（**已验证会红**）。原记录 911 / 908 / 905 / 904 / 894 / 892 / 889 / 870 / 750）—— 期间落了 zhipin 的打招呼/回复/收件箱/阶段探测、zhaopin 的收件箱/阶段探测与对应护栏 |
