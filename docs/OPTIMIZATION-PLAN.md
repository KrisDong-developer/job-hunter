...
| 单测基线 | **1080 个（1080 通过 / 0 跳过）**（`npm test`，离线，绝不访问真实招聘站）。2026-09-20 更新（BOSS 会话级 + 详情页实测那一轮：+2 条 `sendResume` 用例钉住"弹窗开了不等于发得出去"、+2 条 JD 水印剔除（含"句中正常提到平台名不能误删"的反例）、+1 条 `jdWatermarkTexts` 合并校验；并把 `detectStage`/`readInbox` 夹具的未读徽章换成实测类名 `.notice-badge`）。再上一记录 **1077 个**、**913 个（912 通过 / 1 跳过）**（2026-09-19 更新（`runtime.ts` 从 2074 行拆到 1310 行：`runtime/{contract,gate,adapters,views,actions,lifecycle}.ts`；`openDataLayer` 拆成 6 个有序阶段，并把「模型层必须先就绪」写进**阶段签名**；领域规则 `syncDeadlineTodos` / `resumeProfileOf` 下沉到 `domain/{campus,intel}.ts`。另**修掉一个静默接线 bug**：`resumes` 比 `ai` 先建 → 简历定制永远退回规则路径（`via='rule'`，不报错不留痕），现由 `test/host/ai-wiring.test.ts` 钉住「配了模型就必须走 `via='llm'`」。单测增量：平台门判定 +10 条；`city-gate.test.ts` 的"复刻的门"换真实现；+1 条"三个入口对同一份健康事实说法一致"；+2 条 close() 后逐个访问器拒答（**已验证会红**）；+1 条 `/health` 版本号来自包清单（**已验证会红**）；+3 条 `resumeProfileOf`；+2 条模型接线（**已验证会红**）。原记录 911 / 908 / 905 / 904 / 894 / 892 / 889 / 870 / 750）—— 期间落了 zhipin 的打招呼/回复/收件箱/阶段探测、zhaopin 的收件箱/阶段探测与对应护栏 |
...
| **测试计数变了要改文档** | 当前值**只看 §1.1 的「单测基线」那一行**，不要在本表里再写死一个数字（历史教训：这里写过 705、那边写过 870，两处互相矛盾）。改完 `npm test` 后先 grep 一遍 `个（` 与 `个（.*通过` 核对 |
...
| 项 | 状态 |
|---|---|
| 平台适配：BOSS 直聘 / 猎聘 / 拉勾 | **已过期**（2026-09-18 复核更正）：BOSS（`zhipin`）**已实现**打招呼 / 回复 / 收件箱 / 阶段探测，且经真实账号端到端验证；`zhaopin` 的收件箱 / 阶段探测也已实现（接口化）；猎聘 / 拉勾的适配器**已注册**（列表/详情夹具校准过）但仍无 `actions`。**2026-09-19 复核**：猎聘这轮**补齐三件**（见 ADAPTERS §7.2）：① 修掉一个**静默 bug** —— `readListPage` 的"接口化列表"通道因缺 `x-fscp-*` 一族请求头而长期**静默回退 DOM**（服务端回 `-1400` 但 HTTP 200），修复后在线复验 `42 条 / publishedAt 42 / industry 42`（此前这两个字段恒为空），头集进 `LIEPIN_API_HEADERS`（可 DB 覆盖）+ 用例钉住；② **补上登录检测**（`auth`）—— 此前 `auth===undefined` 让 `platforms.loginStatus` 直接抛错、`account.loggedIn` 恒 false，界面入口永远不亮，现用两份真实快照定出的结构性标记实现；③ **定谳 `authRequirement` = `none`/`none`/`required`** —— `detail` 此前写 `required` 与自家未登录夹具矛盾（夹具带未登录标记而 JD 完整、薪资明文）。**仍未完成**：`readInbox`/`detectStage`/`sayHello`/`sendResume` 不实现（`direction` 语义没定论 + 猎聘 IM 是"人"维度）。**城市码表已补齐**（2026-09-19 逐省点开页面自己的「请选择城市」弹窗采样，**370 个城市码**，原始数据 `test/fixtures/liepin-city-codes.json`；并把跨平台 `CITY_DIRECTORY` 从 52 扩到 373、把猎聘纳入"码表不许与目录脱节"检查）。**2026-09-20 复核**：猎聘再补三件 —— ① **`readInbox` 落地**（`get-contact-list` 接口通道，`supportsInbox` 随之改为 true）：`direction` 字段**不作判据**（八行真实样本下它与"最后一条的方向""谁先发起"两种解释都吻合），改用 `unReadCnt>0 ⇒ hr`（8/8 样本）与 `extType 200（平台替我生成的招呼语建议）⇒ me`（1/1 样本）两条有样本的规则、其余按 `hr` 记（与 zhipin 同口径：漏报比误报贵）；② **修掉 `platformJobId` 双通道不一致的真 bug** —— 接口给 8 位数字 `job.jobId`、DOM 给 10 位 URL 路径 id，同一批 42 条里只有 19/42 相等 ⇒ 统一到卡片 href 埋点 `pgRef` 的数字 id（实测该集合与接口 jobId **42/42 完全相等**），否则两轮之间换通道会把同一批岗位写两遍；③ **IM 接口门坎实测**（六项静态头 + 现造遥测才 `flag:1`，且体必须是 urlencoded）—— 与搜索接口同一条结论。**仍未完成**：`detectStage`/`sayHello`/`reply`/`sendResume`（输入面已量到，但"点了 ≠ 发出去了"那条桥缺失：`open-chat` 的应答是页面自己发的请求，`PageLike` 看不到；按钮文案被拒也会翻成「继续聊」）。**同日再复核（发送实验）**：上一句的"桥缺失"被推翻 —— **桥在页面输入面自己身上**（与 zhipin `#chat-input` 同构）。`LIEPIN_ALLOW_SEND=1` 的探针在真实会话里真键盘打话术 + 回车，**三重证据闭环**（textarea value 上屏 → `.im-ui-message-item-send` 出现同文本且 loading 图标归 hide → `get-contact-list` 第一行 lastPayload 即这句话）⇒ **`sayHello`/`reply` 落地**（`supportsGreeting` 置 true；`greetingSideEffect` = "插入建议卡片、不代发"），实现带三条猎聘特有护栏：① 输入前必校验 `textarea.value`、失败再聚焦并回退真键盘路径、两路都不上屏**绝不按回车**（第一次实验实测：点击落在动画中的弹窗上、焦点没进输入框时 insertText 全部落空）；② 受理/被拒按 DOM 后果判（被拒弹 `.complete-resume-modal` code 30011、受理则输入框出现）；③ `reply` 固定从搜索页进抽屉（详情页上 `#im-c-entry` 内层 `.im-ui-basic-entry` 懒加载不保证渲染，快照实测 0/1）。**仍未完成**：`sendResume`（投递二级确认未实测）、`detectStage`（人/岗维度对不上）；`supportsReadReceipt` 保持 false（`oppositeRead` 实测恒 "1" 无反向样本） |
...
| BOSS 列表薪资（F1） | **已定案并落地**（2026-09-19 收尾）：数字是**字体混淆**的私有区码点（U+E031–U+E03A 一码一数字）⇒ DOM 通道一律置空 + 记 `salary:obfuscated`，绝不把乱码写库。明文来源已接通：`POST wapi/zpgeek/search/joblist.json`（**表单体**）的 `salaryDesc`，连接键 `encryptJobId ↔ 卡片 href id` 实测 **15/15** 命中；适配器走"DOM 定列表、接口只补薪资"，且**只要 cookie + content-type 就调得通**（不需要 `zp_token`）。另纠正一条旧结论：**接口的 `page` 参数有效**，旧说法指的是搜索页 URL 的 `&page=` 被 SPA 忽略 |
| BOSS 登录检测 | **已补**（2026-09-19）：此前 `zhipin` 没有 `auth` ⇒ `platforms.loginStatus` 直接抛「没有声明登录入口」、`account.loggedIn` 恒 false ⇒ 打招呼/收件箱/回复都实现了、**界面入口却永远不亮**。锚点为页头 `a[ka="header-username"]` / `a[ka="header-login"]`（两份真实快照对比定案） |
| BOSS `.choose-resume-dialog`（F2） | **已实测**（2026-09-20，`npm run probe:zhipin-chat ZHIPIN_PROBE_RESUME=1`）：等到了 HR 回复 ⇒ 「发简历」的 `unable` 消失 ⇒ 用真鼠标点开弹窗并采下原文（`.probe-zhipin-capture/resume-dialog-2026-09-20.html`）。结构：外壳/内层**同名** `.choose-resume-dialog`（命中 2 个节点）→ 条目容器 `.resume-choose-container` → 确认按钮 `button.btn-v2.btn-sure-v2.btn-confirm`。**两条影响行为的事实**：① 没有可选简历时弹窗照样打开（空态 `.resume-top-tip`「未上传简历」）；② 未选中时确认按钮带 `disabled`。⇒ `sendResume` 改为先读弹窗状态（新增 `resumeDialogStateInPage`），条目空/按钮 disabled 时如实报 `missing`，**修掉了旧代码会把它讲成 `delivered` 的那条撒谎路径**。⚠️ 仍缺样本：条目本身的类名（账号无附件简历）、发送成功后的 `.resume-card` |
| BOSS 会话行未读/已读（F2 同批） | **已实测**（2026-09-20，2 条真实会话）：未读徽章 = **`.notice-badge`**（文本即未读数，同行为 HR 主动发来的、且**没有** `.message-status`）；**`status-read`（`[已读]`）拿到样本** ⇒ `detectStage` 的 `read` 一档不再是"代码支持但没见过" |
| BOSS 详情页 JD **水印注入** | **已修**（2026-09-20 实测发现）：`.job-sec-text` 里被塞了**随机类名**的 `<span>`（实测 `TkBBeZbHdGjN` / `pyKakWzEQwNK`），内容就是 `BOSS直聘` / `来自BOSS直聘`，落点在正文**任意位置** —— 实测那句把「后端业务系统」劈成了「后 + 水印 + 端业务系统」。因为 `jdText` 会原样写库（`store.job.setJdText`）并喂给打分/技能差距分析，脏文本一路带下去。修法：`extractDetailInPage` **逐节点走、跳过"整个元素恰好等于水印串"的节点**，名单进 `ZhipinDetailSelectors.jdWatermarkTexts`（可 DB 覆盖；类名随机所以**只能按文本判**）。在线复验：同一份真实快照 `jdText` 297 → **283** 字、`直聘` 0 处、`后端业务系统` 复原。📌 列表页查过**没有**这种注入 |
| 页面抽象层 `PageLike.waitForSelector` 签名错配 | **已修**（2026-09-19 真实线上故障）：`PageLike` 声明的是 `(selector, timeoutMs: number)`，而真路径透传 Playwright 的 `(selector, options?)` ⇒ 传数字让 `state`/`timeout` **双双失效**（实际 `visible` + 默认 30s），**全部 11 处调用点**（所有适配器）的 `waitForListMs` 都形同虚设。现场极具误导性：报的是「连搜索页都没打开成功」，而 Playwright 日志写着 *locator resolved to 42 elements* —— 网络与页面都没问题。修复收在中心：`browser.ts` 的 `normalizeWaitForSelector`（等 `attached`、超时返回 `false` 不抛错，与 jsdom 夹具同形），页池 `newPage`/`seed` 两处接线 + 3 条用例钉住。在线复验（生产链路 `createBrowserManager` → 猎聘适配器）：负对照 `waitForSelector('#不存在', 1500)` → `false` 用时 **1504ms**（修复前会等满 30s 再抛）、`gotoSearch` **1.9s**、`detectBlock=null`（站点未被拦）、`readListPage` **42 条**（`publishedAt` 42 ⇒ 接口通道正常）、`hasNextPage=true`。**仍待做**：各 `test/tools/probe-*.ts` 自己拿的是**未经归一化**的原始 Playwright 页 ⇒ 校准探针的等待语义与生产不一致（本次故障正是这种"探针里没复现"的形态），需要给探针套同一个归一化 |
