...
| 单测基线 | **679 个（678 通过 / 1 跳过）**（`npm test`，离线，绝不访问真实招聘站）。P11 适配器契约三轴后 |
...
npm test                     # 基线 679；新增测试后要更新文档里的计数
```
...
| **测试计数变了要改文档** | 当前是 **679**（P11 之后）。这个数在 `README.dev.md`、`docs/OPTIMIZATION-PLAN.md` 里出现过 —— 改完先 grep 一遍计数 |
