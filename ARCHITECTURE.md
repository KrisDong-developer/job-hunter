...
**U9 平台与适配器**（P0）、**U10 设置**（P1）、**U11 日志与诊断**（P1），**三屏至今未实现**，
它们的内容（平台与登录、抓取按钮、设置可读、审计/LLM 记录）现在全部挤在 U0 今日里。
这就是今日屏拥挤、且"配置类内容混在日常动线里"的根因。因此：

- **U9 就是"数据采集"该在的地方**（tab 显示「采集」，页面标题「数据采集」）：采集方案配置（平台集合 / 筛选条件 / 抓取深度 / 偏好时段）、
  平台状态（登录态、健康、**筛选维度发现与人工覆盖**）、触发（立即 / 补跑 / **一键暂停**）、下次运行与**"为什么没跑"**；
- **今日（U0）只留摘要 + 一次动作**：新鲜度徽章 + 下次运行（本地时间）+「立即采集」+「去配置」，健康只留一行只读；
- U10/U11 维持原优先级（P1），不因为这次需求提前。
...
| U9 平台与适配器（tab「采集」，页标题「数据采集」） | **P0 · 未实现** | 登录态、健康、动态筛选项发现结果与人工覆盖；**并承载采集方案的配置与触发**（平台集合 / 筛选条件 / 抓取深度 / 偏好时段 / 一键暂停 / 立即与补跑 / 下次运行与"为什么没跑"）—— 见 §4.6.1 |
| U10 设置 | P1 · 未实现 | 抓取、额度、隐私、保留、风险开关（现行内容散落在 `/settings`、`job_settings` 工具与 U0 里） |
| U11 日志与诊断 | P1 · 未实现 | 运行历史、错误、LLM 调用记录（现行内容只有 HTTP + `audit_log`/`llm_call` 两张表） |
