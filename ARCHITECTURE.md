...
### 4.6.1 定时任务的需求基线（SR-1…SR-37，2026-09-17 定稿）
...
- **U9 就是"数据采集"该在的地方**（tab 显示「采集」，页面标题「数据采集」）：采集方案配置（平台集合 / 筛选条件 / 抓取深度 / 偏好时段）、
  平台状态（登录态、健康、**逐字段健康**）、触发（立即 / 补跑 / **一键暂停**）、下次运行与**"为什么没跑"**；
...
| **P14 量级基线告警** | 新增 `yield-drop`（warn，恢复即关闭）：基线 = 该平台最近 30 轮里**只取 `state='ok'`** 的 `found` 中位数；样本 ≥5、基线 ≥5、低于基线 30% 才告警；**本轮自己排除在基线外**；`/platforms` 的 `yield` 字段 + 采集页展示 | 见 §4.6.1「量级基线告警」与 README.dev.md 的 P14<br>**✅ 达成**：单测 700（699 通过 / 1 跳过），含一条**走真实 `runCrawl` 主链**的端到端断言（5 轮常态 → 一轮空结果 → 告警 → 恢复关闭）<br>**仍未做**：批次 5 的**单轮预算 + 到点中止**、平台总览矩阵屏（按新鲜度排序与两层额度已在 P16 做了） |
