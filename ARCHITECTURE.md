...
>
> **v3 修订（外部评审吸收）**：
> ① **字段级断言 + 脏数据不入库**，提到 P0（§4.2.4）；② 反爬边界明确化，不做指纹伪造 / UA 轮换 / 代理池（§4.2.5，D-17；**2026-09-18 D-17a 修订**：允许并默认启用环境一致性手段——patchright / stealth 注入 / 端口守卫）；
...
| 可修复性 | `pending_repair` 保留**失败字段与当时解析出的标量**（*不*存原始 HTML —— 见下），供 UI 内看清坏在哪、并配合"改选择器覆盖 → 重跑一轮"修复 |
| 修法与入口 | 改这个平台的选择器覆盖（`PUT /platforms/:id/adapter-config`，**热生效**）→ 重跑一轮抓取 → 确认数据正常 → 清空该平台队列（`POST /repairs/clear`）。**没有"离线重放"这一步**：`pending_repair.raw_html` 列存在，但入队时不写入（原始页面是体积杀手，§18.1 默认不存），没有 HTML 就没有可重放的东西 |

#### 4.2.5 反爬对抗策略（v3 新增，边界明确）

**采用的**：真实浏览器 + 持久化 profile + 用户真实登录态；`--disable-blink-features=AutomationControlled`；**单 IP**；低频串行；请求间随机延时（高斯分布 + 犹豫 + 突发惩罚）；尊重平台额度（含"配额耗尽"判墙）；**抓取请求一律由 Chromium 网络栈发出**（因此 TLS/JA3 与真实 Chrome 一致，这不是我们的弱点——禁止用 `page.request` 绕成 Node 侧请求）。

**环境一致性（D-17a，2026-09-18 修订后允许并默认启用）**：

| 手段 | 做法 | 为什么不算"指纹伪造" |
|---|---|---|
| patchright 引擎 | `playwright-core` 的反检测分支启动同一个真实 Chrome | 它隐藏的是 **CDP 协议自身的自动化痕迹**（`navigator.webdriver`、`Runtime.enable` 注入的 console API 等），让"自动化运行的真实浏览器"与"真人手动的真实浏览器"环境一致。不新增、不修改任何身份信号 |
| stealth 注入（`platform/stealth.ts`） | `addInitScript` 在每个页面脚本运行前恢复"原生环境"的样子：被包装函数的 `toString` 仍返回 `[native code]`、console 方法收到的对象参数替换为 `{}` 防探测展开 | 它隐藏的是**我们自己对环境的改动痕迹**，不是编造不存在的环境 |
| 调试端口守卫（`platform/cdp-guard.ts`） | 仅当启动参数带 `--remote-debugging-port` 时，用 CDP `Fetch` 域把页面发往 `127.0.0.1:<port>` 的探测请求伪造为 `ConnectionRefused` | 真人手动浏览时没有调试端口；守卫让"开着调试端口的浏览器"在网络层看起来和"没开"一样 |

**明确不做的**（需求 §5.2，**D-17 / D-17a**）：

| 不做 | 理由 |
|---|---|
| UA 轮换 / Canvas / WebGL 指纹伪造 | UA 与指纹必须与浏览器版本、与已沉淀的设备指纹一致。已登录账号每天换 UA 是**典型的自动化特征**，反而提高风控命中率。伪造一个"不存在的身份"与"隐藏自动化痕迹"有本质区别，前者仍禁止 |
...
### 4.6.1 定时任务的需求基线（SR-1…SR-46，2026-09-17 定稿，2026-09-18 追加 J/K 段）

> 依据 `REQUIREMENTS.md` 的 **D-19**（触发模型四项决定）。这一节是**实现级需求**：
> 细化 C9 / R20 / D-5 / §14，并把"固定单点"改成"窗口内随机 + 在场触发"。
> 编号用 `SR-x`，**不占用**本文档 §12 的 R1–R21。

**前提（决定其余一切）**：**插件不是守护进程**。DSH 关着的时候没有调度。
所以目标不是"09:30 准时触发"，而是**"用户下一次坐下来的时候，手上是新鲜的"**。

#### 触发模型（三类触发器 + 抑制规则）

| 触发器 | 语义 | 理由 |
|---|---|---|
| **T1 窗口触发**（首选） | 配置"偏好时段"（如 09:00–11:00），窗口打开后在窗口内**随机**选点，窗口内最多一次 | 固定时刻 + 每天同一分钟是最容易被识别的模式（R18 / D-17） |
| **T2 在场触发** | DSH 启动 / 用户打开面板时，若新鲜度为 stale/cold → **提示**一次刷新，不自动跑 | 插件只在用户在场时活着，那就利用在场，而不是盲猜时刻 |
| **T3 人工触发** | 立即运行 / 补跑 | 永远保留；且要区分"这次算不算刷新"（SR-7） |

抑制规则（三类共用）：同平台最小间隔 · 每日上限 · 窗口外不跑 · 退避中不跑 · 只读实例不跑。
...
| # | 需求 | 验收 |
|---|---|---|
| SR-1 | 窗口内随机触发，不是固定单点 | 连续 5 天触发时刻互不相同，且都在窗口内 |
| SR-2 | 在场触发（T2）：陈旧时提示，不自动跑 | 启动后首屏可见提示；不点就不跑 |
| SR-3 | 抑制：最小间隔 / 每日上限 / 窗口外 | 到上限后再到点记 `skipped(quota_reached)` |
| SR-4 | 多方案各自排程；同时到期则**串行** | 两方案同时到期 → 各一条 `crawl_run`，无并发 |
| SR-5 | 时区**跟着人走**：存本地墙钟 + 时区快照 | 改系统时区后，"下次运行"按当地 09:00 算 |
| SR-6 | 时钟跳变 / 休眠唤醒：不补偿、只重算 | 系统时间 +3h 不会导致连跑两次 |

**B 新鲜度与欠账**

| # | 需求 | 验收 |
|---|---|---|
| SR-7 | 区分 `last_attempt_at` 与 `last_success_at`（只有 ok 推进后者） | 失败不推进 `last_success_at`，但推进冷却 |
| SR-8 | 新鲜度三级：fresh(<18h) / stale(18–48h) / cold(>48h)，阈值随计划频率 | 改 `last_success_at` 后三级提示与待办随之出现 |
| SR-9 | 欠账只提示不自动跑（C9）；待办按 ref 去重 | 停机 3 天回来只有 1 条待办，且不自动跑 |
| SR-10 | 手动/补跑成功后新鲜度推进，并关闭 cold 待办 | 补跑后待办消失 |

**C 执行、互斥与租约**

| # | 需求 | 验收 |
|---|---|---|
| SR-11 | 全机同时只有一个抓取（手动与定时共用一把锁） | 连点两次「立即运行」→ 第二次被拒或排队 |
| SR-12 | 只有租约持有者可执行；只读实例**手动跑也拒绝**并给原因 | 非持有者点运行 → 明确报错 |
| SR-13 | 租约可核验（pid + 心跳 + 相对时间）；**丢失时主动提示** | 杀掉持有者 → 另一实例接管，原实例提示"已交出" |
| SR-14 | 可中止；中止后不写半截数据 | 主表无部分记录，`crawl_run` 记 `aborted` |
| SR-15 | **崩溃安全**：悬挂的 `running` 在下次启动收敛为 `failed/aborted`（现状疑似缺失） | 强杀后重启 → 该 run 不再是 `running` |

**D 前置条件与跳过原因**

| # | 需求 | 验收 |
|---|---|---|
| SR-16 | 每平台独立检查：登录态 / 适配器健康 / 风控暂停 / 离线闸门 / 配额 | 每个条件可单独触发对应原因 |
| SR-17 | `skipped` 必带**枚举原因**，界面显示人话 | 面板能看到"为什么没跑"，不只有"已武装" |
| SR-18 | 一个平台被跳过不影响其他平台 | 51job 未登录时另一平台照常跑 |
| SR-19 | 离线闸门开启时定时直接跳过并说明 | 开启后零真实请求 |

**E 失败、风控与退避**

| # | 需求 | 验收 |
|---|---|---|
| SR-20 | 退避：失败后 15m → 1h → 4h，封顶当天不再试 | 连续失败三次的间隔符合曲线 |
| SR-21 | 连续失败 N 次 → `risk_paused` + urgent 待办 + **人工确认才恢复** | 达阈值后不再尝试该平台 |
| SR-22 | 风控信号（滑块/验证码/连续零结果）单独识别，不计入普通失败 | 触发后写 `risk_paused`，原因可读 |
| SR-23 | 失败不推进 `last_success_at`，但推进冷却 | 见 SR-7 |

**F 通知与待办**

| # | 需求 | 验收 |
|---|---|---|
| SR-24 | 只在下述情况打扰：cold 欠账 / 风控暂停 / 连续失败 / 登录失效 | 正常跑完不产生待办 |
| SR-25 | 待办按 ref 去重，修复后自动关闭 | 连续 3 天 cold 只有 1 条；补跑后关闭 |
| SR-26 | 面板/浮层如实报告"本轮没跑"及原因 | 与 SR-17 同一份枚举 |

**G 可观测与解释**

| # | 需求 | 验收 |
|---|---|---|
| SR-27 | 今日屏显示本地时间（含抖动说明）+ 相对时间 + 状态 + 新鲜度徽章 | 不再是裸 UTC |
| SR-28 | 最近 N 次运行小表：时间/状态/新增/失败原因/触发原因 | 触发原因区分 schedule/manual/catch-up |
| SR-29 | 每次触发落 `crawl_run` 且带 `reason`；保留期限可配（R21） | 表不无限增长 |

**H 配置与控制**

| # | 需求 | 验收 |
|---|---|---|
| SR-30 | **全局一键暂停**（现状只有 per-plan 开关）；**只停定时**，手动永远可用 | 暂停后无定时触发，但「立即运行」仍成功 |
| SR-31 | 可配：窗口 / 工作日 / 每日上限 / 最小间隔 / 退避 | 改完立刻反映到"下次运行" |
| SR-32 | **不提供**"精确到分钟的单点时刻" | 配置项里没有该字段 |
| SR-33 | 首次安装不自动跑，第一次由用户显式触发 | 新装后零网络请求 |

**I 数据一致性**

| # | 需求 | 验收 |
|---|---|---|
| SR-34 | 重抓**不得覆盖**用户处置态（收藏/忽略/归档）——现状待确认 | 已收藏岗位重抓后仍已收藏 |
| SR-35 | 重抓幂等（平台 + `platform_job_id`） | 连跑两次不产生重复行 |
| SR-36 | 只落合格记录，不合格进 `pending_repair` | 破坏选择器后主表零脏数据 |
| SR-37 | 定时跑完后若简历版本变了，分数标 `stale`（R17） | 换启用简历后旧分标过期 |

**J 抓取目标与配置面（2026-09-17 续，D-19 的追加决定）**

| # | 需求 | 验收 |
|---|---|---|
| SR-38 | 抓取目标**方案级可配**：平台集合 + 每平台筛选条件（`criteria`） | 两个方案各配不同城市/关键词，各自独立跑 |
| SR-39 | 平台可选集来自**注册表**；未注册平台不可选，选到不存在的平台报可读错 | 选不存在的平台 → 明确报错，不空跑 |
| SR-40 | **抓取深度进本版配置**：页数上限 / 排序方式 / 发布时间窗 | 上限设 1 页 → 只抓 1 页；平台给不了的条件如实说明 |
| SR-41 | 适配器**声明支持的筛选维度**（本版就做）；界面据此渲染，不支持的维度**禁用而非隐藏**并给出原因 | 未声明"薪资"的平台，薪资筛选是灰的且有说明 |
| SR-42 | 未知筛选键**显式处理**：报错，或忽略并在界面说明"该平台不认识这个条件" | 塞一个不存在的键 → 不静默 |
| SR-43 | 重复抓取**提示而不合并**：同平台 + 同条件的方案给出重复提示 | 两个同条件方案 → 保存时提示"与方案 X 重复" |
| SR-44 | 抓取后处理可配：打分 / 标注 / 去重（默认全开） | 关掉打分后不写 `match_score` |
| SR-45 | 配置三条入口对等（GUI / 模型工具 / HTTP）且共用同一套校验 | 工具与界面各塞非法条件，报错一致 |

| 追加决定 | 决定 | 后果 |
|---|---|---|
| 抓取深度 | **进本版配置** | 适配器要声明页数/排序/时间窗的 URL 映射；`criteria` 从"自由键值对"升级为**按声明校验** |
| 重复抓取 | **提示，不合并** | 保存/运行前给重复提示（SR-43）；不自动合并 —— 合并会替用户抹掉他的两个意图 |
| 筛选维度声明 | **本版就做** | 适配器新增筛选维度声明；`SearchCriteria` 的键受声明约束（SR-42）；与 §5.5「能力驱动的 UI」同一条原则 |

**K 单轮容量与平台总览（2026-09-18，多平台的收尾）**

| # | 需求 | 验收 |
|---|---|---|
| SR-46 | **单轮预算**：一个方案的一次运行有**时长上限**；到点后不再开新平台（如实报 `round_budget`），正在跑的那一轮在**页与页之间**收手（`aborted`），已抓到的照常入库 | 3 个平台、跨过时限 → 只跑第 1 个，另两个显示"本轮已到时限"；多页抓到一半到点 → `pages` 停住、数据入库、`fail_streak` 不动 |

**K 之后的收尾（批次 3 / 批次 4，2026-09-18）**

这两批不是新的定时需求，而是早先自己标下的"未做项"，验收标准写在实现处
（§4.10.1 的「独立触发与按组折叠」与 §4.6.1 的「跨平台城市目录」）：

| # | 项 | 验收 |
|---|---|---|
| 批次 4 · 去重可独立触发 | 全库复核（`POST /dedup/run` / `job_dedup` / 界面按钮），与抓取后处理共用同一份判断 | 库里**早就存在**的跨平台重复不靠"再抓一轮"就被合并；重复跑幂等 |
| 批次 4 · 岗位列表按组折叠 + 跨平台对照 | `GET /jobs?groupDuplicates=1` 折叠成一行、展开看各平台对照 | 折叠后 `total`/分页一致且同一组不跨页重复 |
| 批次 3 · 跨平台城市目录 | 单一城市表 + 每平台声明 `closed` + 并集取值域 + 提示给下一步 | 空表平台（guopin）带城市当场拦下；拉勾填「珠海」不再被误报；`51job`+`zhipin` 能选到「东莞」 |

**界面落点（不新开页面）**：这些配置**不另造一页** —— §5.4 早已规划
**U9 平台与适配器**（P0）、**U10 设置**（P1）、**U11 日志与诊断**（P1）。

> **这三屏已于 2026-09-17 全部落地**（U10/U11 合并成一个「设置」tab）。
> 落地前它们的内容（平台与登录、抓取按钮、设置可读、审计/LLM 记录）全部挤在 U0 今日里，
> 那正是今日屏拥挤、"配置类内容混在日常动线里"的根因；现在各归其位。

- **U9 就是"数据采集"该在的地方**（tab 显示「采集」，页面标题「数据采集」）：采集方案配置（平台集合 / 筛选条件 / 抓取深度 / 偏好时段）、
  平台**总览矩阵**（一屏横向对照：成熟度 / 登录 / 健康 / 今天能不能跑 / 今日额度 / 产量 / 最近一轮）、
  平台明细（登录态、健康、**逐字段健康**、登录与排查动作）、触发（立即 / 补跑 / **一键暂停**）、下次运行与**"为什么没跑"**；
- **今日（U0）只留摘要 + 一次动作**：新鲜度徽章 + 下次运行（本地时间）+「立即采集」+「去配置」，健康只留一行只读；
- **U10/U11 合并成「设置」**：模型用途开关、闸门只读展示、诊断、模型调用留痕、操作审计。

> **一条实现差异要记下来**：§4.6.1 的界面落点里写了"筛选维度**发现与人工覆盖**"，
> 本次只落地了**声明式**的那一半（适配器 `criteriaDimensions` 声明 + 界面据此渲染 + 未声明的键报错）。
> 动态 DOM 遍历发现筛选面板（`platform/discovery.ts`）**仍未实现** —— 51job 的筛选维度是人工建一次的
> URL 映射，这正是 §4.2.2 说的"新增平台的主要工作成本"。

#### 非功能需求

| # | 要求 | 理由 |
|---|---|---|
| NFR-1 | **离线可测**：调度逻辑用假时钟驱动，不依赖真实等待 | §14；`TimerPort` 已具备 |
| NFR-2 | 定时器随 fiber dispose 全部取消（可热重载、不重复武装） | R19 |
| NFR-3 | 不阻塞主进程：开库/迁移/浏览器启动全异步 | §4.9 |
| NFR-4 | 状态全部落 sqlite（重启后上次/下次/欠账不变） | 单进程现实 |
| NFR-5 | 抓取失败不得让 DSH 崩，异常转成可读状态 | fail-closed 不 fail-loud |
| NFR-6 | 日志与审计不落敏感正文（R21） | 隐私 |
| NFR-7 | 单轮资源可控：浏览器句柄关闭、无泄漏 | 长期运行 |

#### 优先级

| 期 | 内容 |
|---|---|
| P0（bug 级） | SR-27（裸 UTC 显示）、「抓取一次」写死条件（属 SR-31 范畴）、SR-15（悬挂 run）、SR-34（处置态） |
| P1（语义修正） | SR-7/8/9/10（新鲜度模型）、SR-17（跳过原因）、SR-30（一键暂停） |
| P2（模型变化，D-19 已授权） | SR-1/2/5/6（窗口随机 + 在场触发 + 时区 + 时钟）、SR-20/21/22（退避与风控） |
| P3（可观测与运维） | SR-28/29/31、SR-18（每平台独立冷却） |

#### 落地状态（2026-09-17）

| SR | 状态 | 落地位置 / 说明 |
|---|---|---|
| SR-1 窗口随机 | ✅ | `scheduler/schedule.ts`：窗口内触发点由 `hash(YYYY-MM-DD#planId)` 派生（**不是** `Math.random()` —— 同一个窗口必须恒定，否则 `next_run_at` 落库没意义、定时器还会自旋） |
| SR-2 在场触发 | ✅ | `SchedulerStatusDto.refreshSuggested/refreshHint`；今日与采集页都只**提示**，不自动跑 |
| SR-3 抑制 | ✅ | 窗口外 `outside_window`、每日上限 `quota_reached`、每平台冷却 `backoff` |
| SR-4 多方案串行 | ✅ | `tick()` 顺序 `await`，全机只有一把 `mutex` |
| SR-5 时区 | ✅ | `plan.timezone` 记写入时的时区快照；排程按本地墙钟 |
| SR-6 时钟跳变 | ✅ | `refreshNextRuns` 只重算不补偿；跨窗口用窗口标识去重 |
| SR-7 attempt/success | ✅ | `plan.last_attempt_at` / `last_success_at`（schema v7） |
| SR-8 三级新鲜度 | ✅ | `freshnessOf()`，阈值随计划频率（`freshThresholdsFor`） |
| SR-9/10 欠账只提示 | ✅ | `catch-up` 待办按 ref 去重；**全新安装不产生欠账待办**（SR-24/33） |
| SR-11/12 互斥与租约 | ✅ | 复用 `mutex` 与 lease；非持有者手动也拒绝 |
...
| SR-27 今日屏本地时间 | ✅ | `shared/time-format.ts` + `SchedulerStatusDto.triggers` |
| SR-28 最近 N 次运行 | ✅ | `SchedulerStatusDto.recentRuns`（带 `reason`/`skipReason`），采集页小表 |
| SR-29 每次触发落 reason | ✅ | `crawl_run.reason` / `skip_reason`（schema v7） |
| SR-30 一键暂停 | ✅ | `setPaused`；**只停定时**，手动永远可用（行为测试钉住） |
| SR-31 可配 | ✅ | 窗口/工作日/每日上限/最小间隔/退避都在方案里或常量里 |
| SR-32 不提供单点时刻 | ✅ | `PlanSchedule` 里没有 `hour/minute`；HTTP 收到它们**显式报错**并指向窗口字段 |
| SR-33 首次安装不自动跑 | ✅ | `ensureDefault` 只建方案；启动不抓取；也不产生欠账待办 |
| SR-34 处置态不被覆盖 | ✅ | **已核实**：`job.upsert` 的更新分支不碰 `state`/`first_seen_at`；四种处置态都有断言 |
| SR-35 重抓幂等 | ✅ | 沿用 `UNIQUE(platform_id, platform_job_id)` |
| SR-36 只落合格记录 | ✅ | 沿用字段断言 + `pending_repair` |
| SR-37 分数随简历失效 | ✅ | 沿用 `score_rev` / `scoreStale` |
| SR-38 方案级可配 | ✅ | 平台集合 + `criteria` + 抓取深度 + 偏好时段 |
| SR-39 平台来自注册表 | ✅ | `validatePlanConfig`；选不存在的平台给可读错 |
| SR-40 抓取深度 | ✅ | `maxPages` / `sort` / `postedWithinDays` → 51job 的 URL 映射 |
| SR-41 维度声明 | ✅ | `SiteAdapter.criteriaDimensions`；界面**禁用而非隐藏**并给原因 |
| SR-42 未知键显式处理 | ✅ | 未声明的键**报错**（附上该平台支持的维度清单），不静默丢弃 |
...
| SR-46 单轮预算与到点中止 | ✅ | `RoundBudget`（本轮开始 + `ROUND_BUDGET_MS`）；平台之间如实报 `round_budget`、平台之内 `crawl_run.state='aborted'`（`DEADLINE_REACHED`），已抓到的照常入库 |
...
#### 非致命提示通道（notice，2026-09-18）

方案校验现在有**三个出口**，而不是两个：

| 出口 | 语义 | 例子 |
|---|---|---|
| 抛 `INVALID_INPUT` | 配置**非法**，拒绝保存 | 平台未注册 / 键未声明 / 取值越域 |
| `duplicates` | 合法但**与别的方案重复** | 只提示，不合并（SR-43） |
| `notices` | 合法但**大概率不会按你想的跑** | 见下 |

`notices` 存在的理由很窄但很硬：多平台最伤人的失败是**平台安静地返回 0 条**
（城市它不认识、平台还没校准、深度被平台上限截断），而这类失败**从数据里查不出来** ——
`crawl_run` 里"今天没岗位"和"平台坏了"长得一样。同时**报错又太严**：
`workExp` 只有神仙外企认识，为它报错等于禁止多平台。

四种来源：平台成熟度（`experimental`/`disabled`）、抓取登录需求未验证且无登录检测、
方案 `maxPages` 超过平台上限、城市不在该平台的城市表里。
三条入口（GUI / 模型工具 / HTTP）都回传，GUI 在**保存前**就实时显示。

> 顺带补的一个洞：新增 `POST /plans/validate`（**草稿校验**，不需要 id）——
> 在这之前"与谁重复"只有已存在的方案才查得到，而新建时恰恰最需要。

#### 量级基线告警（批次 5）

逐字段健康与 `NO_RECORDS` 之间还有一个空档：**字段都好、`state='ok'`、
`quarantined=0`，但条目数掉了一个数量级**（懒加载没触发 / 翻页静默失效 /
卡片容器只匹配到一部分）。这种轮次在 `crawl_run` 里**与正常轮次无法区分**，
所以必须跟该平台**自己的历史**比：最近 30 轮里只取 `state='ok'` 的 `found` 的**中位数**
作基线，样本 ≥5、基线 ≥5、且低于基线 30% 才告警（`yield-drop`，warn，**恢复即关闭**）。
见 `platform/yield-baseline.ts` 与 README.dev.md 的 P14。

#### 单轮预算与到点中止（SR-46）

多平台把"一轮"变长了：`decide` 会依次跑**用户配的任意多个**平台，于是**一轮的时长没有上界**。
一个卡住的页面就能把整轮拖住 —— `running` 一直为真（界面永远显示"正在采集"），
后面到期的方案也只能排队等它。
...
| **平台之内** | `crawl.ts`（每开一页前比同一个终点） | 当前这一轮 `state='aborted'` + `DEADLINE_REACHED`，已解析的记录照常入库 |
...
2. **只在页与页之间停**。请求中途打断会留下一个状态未知的页面，同一浏览器上下文的下一次
   使用行为不可预期；宁可跑完当前这一页。
3. **中止不是失败**。平台正常响应、字段都解析出来了，只是我们自己收手 ——
   所以它不推进 `fail_streak`、不进退避、也不弹风控待办（`state='aborted'` 不等于 `'failed'`）。
   同理它**不进量级基线**：`applyYieldBaseline` 直接跳过、`readYieldSnapshot` 的"最近一轮"
   也跳过 `aborted` 轮 —— 半截的条数对"这个平台通常给多少"没有发言权，
   不跳过的话一次到点收手就会立刻误报一次量级骤降。
4. **被剪掉的平台必须留痕**。`record()` 在开跑**之前**就调用了（SR-26：到点就该有结论），
   而预算是在跑的过程中才用尽的 —— 不重记一次，那几个平台会停在 `reason=null`（= 跑过了），
   而它们一条都没抓。**又一处"静默少一个结果"**：这类 bug 不留痕就根本查不出来。

为什么给 20 分钟这么松：它是**保险丝**，不是节流阀（节流是每日上限与平台额度的事）。
给紧了只会让正常但慢的站点被腰斩，而"慢"比"失控"常见得多。

#### 跨平台城市目录（批次 3，2026-09-18）

城市是本项目里**唯一一个跨平台共享的人的概念**（"深圳"在哪个平台都是深圳），
而它以前是最乱的一处：码表散在 8 个适配器里（字符串码 / 数字码 / 省级码 / 中文名直传 / 空表），
而"用户填的这个城市，我选的这几个平台到底支不支持"**没有任何地方回答得了** ——
界面只拿到**第一个**声明了 city 的平台那张表。

修掉的是三个真实缺陷（同一个成因：**闭不闭，从 `values` 空不空推不出来**）：

| 平台 | `values` | 真实语义 | 旧行为（错） | 现在 |
|---|---|---|---|---|
| `guopin` / `hiredchina` | **空** | 带城市**一律拒绝**（构造不出 URL） | 当成"自由文本"跳过 → 存得下、每次抓取那个平台整轮失败 | `closed: true` → 没有任何平台接受时**硬拒** |
| `lagou` | 非空（20 个建议） | **自由文本**（中文名原样进 URL） | 当成封闭取值域 → 填「珠海」被**假报**警告（甚至拦下） | `closed: false` → 原样接受 |
| 多平台组合 | — | 取值域是**并集** | 只按第一个平台的表判 → `51job`+`zhipin` 时选不到（也存不进）`zhipin` 支持的「东莞」 | 并集 + "部分平台不支持"改为**提示** |

三处配套（都在 `platform/cities.ts`，它不是取值域的权威 —— 权威永远是各平台自己的声明）：

1. **规范城市表** `CITY_DIRECTORY`（52 个，顺序即界面下拉顺序）：只统一"哪些名字算城市、
   按什么顺序"，**不统一码** —— 平台码本来就不同，硬合成一张表只会多一层会漂移的副本；
2. **归一化复用 `normalizeCityForDedupe`**（不是再写一份）：「深圳市」「深圳·福田」
   在去重与城市校验两处必须给出同一个答案。校验时**只有"原样不行、规范名可用"才改写**，
   且改写**进 notice**（不许悄悄改用户填的东西）；
3. **提示要给出下一步**：`51job 不认识「厦门」` 后面直接跟「支持它的是：zhipin」——
   只报"它会返回空"，用户还得自己一个个平台去试。

> 界面上这一条还顺带修了「选不到」：`criteriaDimensionsFor` 对 **city 取并集**并按目录排序，
> 其它维度（排序/时间/平台特有维度）**刻意不并** —— 那些取值是各平台自己的**不透明编码**，
> `51job` 的 `sortType 2` 与别家的 `2` 不是一回事，并起来会拼出一个错误的 URL 且无声无息。

#### 度量（怎么知道做对了）

1. 打开面板时 `now - last_success_at` 的**中位数**（目标 < 24h）
2. `skipped` 各原因占比（`not_logged_in` 高 = 登录引导有问题）
3. 手动触发 / 总触发次数（过高说明定时没起作用）
4. 失败与风控趋势
5. cold 状态平均持续时间
...
**三条硬键各自的"缺值"处理（R25，2026-09-18 修）**——缺值 ≠ 不同，把它们当"不同"会让同一岗位跨平台时**静默地永不合并**：

| 键 | 缺值时 | 为什么 |
|---|---|---|
| 公司 | 归一化后为空 → **不判**（不是"不同"） | 判不出来就别判 |
| 城市 | 比较前**归一到市级**（`normalizeCityForDedupe`） | BOSS 给「深圳·福田区」、别的平台给「深圳」——不归一会判成两个城市 |
| 薪资 | **只在两边都锚定时**才作门槛（`UNKNOWN_SALARY_BUCKET` 是哨兵，不等于任何真实档位） | BOSS 未登录时薪资为空；拿"未知"去比"b3"等于要求有薪资的那边必须是空 |

**合并门槛与"疑似"出口**：标题 bigram 相似度 ≥ `0.9` 才合并；
落在 `[0.75, 0.9)` 之间**不合并**，但报 `candidate: true` + 指向疑似对象 + 人话依据 ——
"没合并"这件事本身必须能被看见（否则用户永远不知道自己少了几个合并，也无从纠正）；
低于 `0.75` 连提都不提（否则这个出口会变成噪音）。合并与疑似同时存在时**合并优先**。

> ⚠️ **仍未做**：这个"疑似"目前只进日志与抓取汇总（`疑似重复待确认 N 条`），
> **没有持久化的待确认队列，也没有确认/否掉的界面**。真正的"级 5 待人工确认分组"
> （本节上表最后一行）仍待落地 —— 见 README.dev.md 的 P15 末节。

**独立触发与按组折叠（批次 4，2026-09-18）**

去重以前**只在抓取的后处理里**发生，于是两个场合用户完全做不到：

| 场合 | 以前 | 现在 |
|---|---|---|
| 刚打开去重开关 / 刚把某个平台加进方案 | 只能等下一轮抓取 —— 而那一轮可能一条新岗位都没有 | `POST /dedup/run`、模型工具 `job_dedup`、采集页的「全库复核一遍」 |
| 去重规则改过（如 R25 修正） | 老数据里"本该合并却漏了"的散着，没有入口重判 | 同一入口：全库重跑一遍 |
| 同一条岗位在 4 个平台各抓一条 | 列表里四张几乎一样的卡片 | `GET /jobs?groupDuplicates=1` 折叠成一行 + 展开「跨平台对照」 |

两条不可让步的实现约束：

1. **复核与抓取后处理共用同一份判断**（`dedupDepsOf` + `applyDedup`）——
   差别只在**输入集合**（这一轮写进去的 vs 全库）。判断有第二份实现就意味着
   "抓取时合并、复核时不合并"这类自相矛盾，而且它不会报错，只会少合并；
2. **折叠走查询层，不走前端**。`total`、分页、排序全部在同一段 WHERE 上算
   （`job.dedup_group_id` + 组内最小 id 作代表）—— 先取 20 条再在浏览器里折叠会让
   "共 40 条 / 只有 12 行"变成一个新谜题，而且**同一组会跨页重复出现**。

> 代表的选取用**组内最小 id**而不是 `dedup_group.primary_job_id`：
> primary 是"第一次合并时那个候选"，折叠留谁不该跟着它走
> （它是可以被拆组、被加的）。最小 id 稳定且与插入顺序一致。

复核**只处理还没有分组的岗位**（已在组里的一律跳过）：`applyDedup` 的语义是
"把这个岗位并进它该在的组"，对已有组的岗位再跑一次要么白跑，要么想把它挪到别的组 ——
而"挪组"不是这套模型支持的（成员是显式 id 列表，挪动等于悄悄改掉用户看过的分组）。
新岗位会通过"候选有组 → 并进那个组"正确挂上去。
...
| 屏 | 优先级 | 说明 |
|---|---|---|
| U0 今日 | P0 | 首屏，回答"今天干什么"。**2026-09-17 减负（D-19 / D6）**：只留 新鲜度徽章 + 下次运行（本地时间 + 相对时间 + 抖动说明）+「立即采集」+「去配置」，平台健康只留一行只读 —— 配置类内容整体搬去 U9 |
| U1 岗位库 | P0 | 核心工作界面；动态筛选器 + 列表 + 风险徽章 |
| U2 岗位详情 | P0 | 抽屉式；匹配解释 + 风险依据 + 黑话解释 |
| U5 流水线 | P1 | 看板 + 列表双视图 |
| U8 数据看板 | P1 | 漏斗 / 渠道 / 简历版本归因 + **箱线图（P25–P75 高亮，口径可切）** + **本地基准对比** + **简历版本 A/B 对比表（每格带样本量）** |
| U9 平台与适配器（tab「采集」，页标题「数据采集」） | **✅ P0（2026-09-17 落地）** | 登录态、健康、**逐字段健康**、登录入口；**并承载采集方案的配置与触发**（平台集合 / 筛选条件 / **抓取深度** / **偏好时段** / **一键暂停** / 立即与补跑 / 下次运行与"**为什么没跑**"）—— 见 §4.6.1。**平台与登录整块从 U0 迁移到这里** |
| U10 设置 | **✅ P1（2026-09-17 落地）** | 模型用途开关（按用途分项）、闸门只读展示（额度/审批/审计**模型改不了**，界面上写清楚）、保留与风险开关 |
| U11 日志与诊断 | **✅ P1（2026-09-17 落地）** | 运行历史（时间/状态/新增/失败原因/**触发原因**）、诊断（版本/路径/计数/工具注册结果/离线标记）、**模型调用留痕**（我发了什么给模型）、操作审计 |

> **U10 与 U11 合并成了同一个「设置」tab**（两块共用一次读取、都是"系统怎么配/出过什么事"，
> 分成两个 tab 反而都是在找一个只在安装时看一次的地方）。
...
| **P9 定时与采集配置面** | 偏好时段随机触发 + 新鲜度模型 + 跳过原因 + 退避/风控暂停 + U9 采集页 + 三条入口对等 | 按 §4.6.1 的 SR-1…SR-45 验收<br>**✅ 全部达成（2026-09-17）**：`plan` 从"单点时刻"改成"偏好时段"（窗口内触发点由稳定哈希派生，SR-1/32）；`last_attempt_at`/`last_success_at` 拆开（SR-7）；三级新鲜度随频率（SR-8）；退避 15m→1h→4h + 连续 3 次或风控信号 → `risk_paused` + urgent 待办 + **人工确认才恢复**（SR-20/21/22）；每平台独立前置检查与冷却（SR-16/18/20）；跳过带**枚举原因 + 人话**（SR-17/26）；全局一键暂停只停定时（SR-30）；启动收敛悬挂 `running`（SR-15）。采集配置面：适配器声明筛选维度（SR-41）、未声明键**显式报错**（SR-42）、抓取深度进配置（SR-40）、重复方案只提示不合并（SR-43）、后处理可关（SR-44）、**三条入口共用 `domain/plan-config.ts` 一份校验**（SR-45）。界面：新增 U9「采集」+ U10/U11 合并的「设置」，**今日减负**（D6）。<br>**单测 460/460**（新增 52 条：窗口随机/同日不同点/稳定哈希/跨零点/旧形状翻译/新鲜度阈值/退避曲线/跳过原因/暂停/风控恢复/时钟跳变/`reapStale`/处置态保持/去重/箱线图口径/本地基准/简历对比）<br>**已核实**：SR-34（重抓不覆盖处置态）本来就是对的做法（`job.upsert` 的更新分支不碰 `state`）—— 补的是断言，不是代码 |
...
| **P12 多平台校验与提示通道** | 新增 `notices` 通道（合法但大概率跑不出东西：未校准平台 / 登录需求未验证 / 深度被平台上限截断 / 城市不被支持）；三条入口都回传；GUI 保存**前**就实时显示；补 `POST /plans/validate` 让**新建方案也能校验** | 见 §4.6.1「非致命提示通道」<br>**✅ 达成**：单测 683（682 通过 / 1 跳过），含一条**反面用例**（正常配置必须零提示 —— 否则用户会学会忽略提示） |
| **P13 每平台覆盖项** | `plan.platformOverrides[id] = { enabled, maxPages }`（**稀疏**：等于默认的条目不落库）+ schema v9；调度器只跑**启用**平台、每平台的页数覆盖只进该平台的条件；三条入口 + UI 对等；校验：越界覆盖报错、全停用报错、页数按**该平台自己的**上限校验（不再静默截断） | 见 README.dev.md 的 P13 与 `SCHEMA_V9` 的注释<br>**✅ 达成**：单测 690（689 通过 / 1 跳过）<br>**仍未做**：每平台各自的**筛选条件**（`criteria` 仍全方案共享）、跨平台城市目录 |
| **P14 量级基线告警** | 新增 `yield-drop`（warn，恢复即关闭）：基线 = 该平台最近 30 轮里**只取 `state='ok'`** 的 `found` 中位数；样本 ≥5、基线 ≥5、低于基线 30% 才告警；**本轮自己排除在基线外**；`/platforms` 的 `yield` 字段 + 采集页展示 | 见 §4.6.1「量级基线告警」与 README.dev.md 的 P14<br>**✅ 达成**：单测 700（699 通过 / 1 跳过），含一条**走真实 `runCrawl` 主链**的端到端断言（5 轮常态 → 一轮空结果 → 告警 → 恢复关闭）<br>**仍未做**：无 —— 遗留的两项已在 P18 完成（按新鲜度排序与两层额度已在 P16 做了） |
| **P15 去重门槛修正与"疑似"出口** | R25 两处**静默少合并**：薪资**未锚定时不作门槛**（`UNKNOWN_SALARY_BUCKET` 哨兵）、城市比较前**归一到市级**（`normalizeCityForDedupe`）；标题相似度 `[0.75, 0.9)` **不合并但报 `candidate`**（指向疑似对象 + 人话依据），并进抓取汇总"疑似重复待确认 N 条" | 见 §4.10.1<br>**✅ 达成**：单测 705（704 通过 / 1 跳过）。⚠️ 关键证据是**改完代码后 705 条一条都没红** —— 说明这两个 bug 此前完全无覆盖；已补 5 条会抓住它们的断言（含"放宽不能伤到原规则"的反面）<br>**仍未做**：疑似候选**没有持久化队列与确认界面**；批次 4 的"去重可独立触发""岗位列表按组折叠"也未做 |
| **P16 调度公平与两层额度** | ① 平台执行顺序按 `last_ok_at` **升序**（最久没成功过的最优先，并列保持方案原顺序）—— 修"数组靠后的平台在有限窗口下系统性跑不到"；② `guard` 每日额度改为 `min(用户额度, 平台侧上限)`，平台侧上限进 `platform-facts.ts` 的 `dailyCaps`（zhipin greeting 150 / zhaopin application 100），拒绝理由点名**是哪一层**限住的 | 见 README.dev.md 的 P16<br>**✅ 达成**：单测 709（708 通过 / 1 跳过），含"都没成功过时保持原顺序"（钉住 `sort` 稳定）与"平台侧上限兜住用户额度且理由点名平台侧"<br>**仍未做**：无 —— 遗留的"单轮预算 + 到点中止""平台总览矩阵屏"已在 P18 完成 |
| **P17 判墙信号表** | 新建 `platform/block-signals.ts`：`COMMON_SIGNALS`（平台无关的验证码选择器 / 限流 / 配额 / 登录墙文案）+ `signalsOf(extra)`（**并集**，不是覆盖）+ `detectBlockWithSignals(arg)`（**只引用 arg** —— 它会被 `page.evaluate` 序列化，引用模块里的任何东西都会 `X is not defined`）。判断顺序：`about:` 协议 → 重定向离开 host → URL 特征 → 验证码 DOM → 验证码文案 → 限流 → 配额 → 登录墙 → blank（越确定的越先判，最弱的 `blank` 放最后，否则"加载中"会被误报成"页面空白"） | 见 README.dev.md 的 P17<br>**✅ 达成**：单测 719（718 通过 / 1 跳过），4 条断言钉住**并集语义 / 纯数据 / 自包含 / 判断顺序**；**10 个适配器全部迁移完成**（各自的旧判墙用例原样通过）。分两条路线 —— **共享函数** 7 个：`zhipin` · `51job` · `liepin` · `indeed` · `lagou` · `guopin` · `hiredchina`（判断流程一致，只差词 / 选择器 / 开关）；**共享词表、自有结构** 3 个：`zhaopin` · `sinojobs` · `waiqi`（流程本身不同，只领 `BlockSignalSet` 词表，被判墙函数保留自己的载荷探针与组合判据）。<br>⚠️ **不能机械替换**：10 个适配器一共挖出 **9 处隐性差异**（51job 的宽口径登录墙与 `blank` 80、猎聘**刻意不判**登录墙、guopin 的 `人机验证` 归属、guopin/hiredchina 的 `loginTextLength` 无上限、zhaopin/sinojobs/waiqi 的 `blankTextLength: 80`、hiredchina **声明了但从未使用**的 `cardBox`），**全都没有任何测试会红**。为此新增 `BlockFlags`（`blankOnAboutProtocol` / `skipLoginWall`）—— "不判"也是一种决定，必须有地方写出来。<br>**第二半 · 共享构造样板**：新建 `platform/config-merge.ts` 的 `mergeAdapterConfig(base, override)`（顶层白名单 / 分组内保留新键 / 字符串空值保护 / 数组整体替换 / 脏值回落）；已迁移 `51job`，其余 9 个仍各自内联。 |
| **P18 单轮预算与平台总览矩阵** | ① **单轮预算**（SR-46）：`RoundBudget` = 本轮开始 + `ROUND_BUDGET_MS`（20 分钟）；调度器在**平台之间**收手（新 `SkipReason='round_budget'`，跑完**重记一次**判定，否则被剪掉的平台会显示成"跑过了"）、`crawl.ts` 在**页与页之间**收手（`state='aborted'` + `DEADLINE_REACHED`，已抓到的照常入库）；中止轮**不进量级基线**（`applyYieldBaseline` 跳过 + `readYieldSnapshot` 的"最近一轮"跳过 `aborted`）。② **平台总览矩阵**：`/platforms` 新增 `governance`（`blocked` / `todayRuns` / `dailyLimit` / `riskPaused` / `cooldownUntil` / `lastRun`），采集页新增「平台总览」矩阵（原来的平台列表改称「平台明细」，继续放原因与动作） | 见 §4.6.1 的「单轮预算与到点中止」<br>**✅ 达成**：单测 **727（726 通过 / 1 跳过）**。三处"不共用一份算法就必然漂移"的地方都收敛了：`platformGate` 与矩阵共用判定、`crawlQuotaOf` 共用额度算法、`cooldownUntilOf` 共用冷却读取 —— 断言只钉一个方向：**矩阵说「可以」的平台，到点真的会跑**。<br>⚠️ **8 条新断言全绿不算证据**：逐条做了**变异验证**（把 `withCut` 重记 / `aborted` 过滤 / 到点 break / `blocked` 同源四处分别改坏），每次都有**指定的那条**变红，改回即绿。<br>**仍未做**：批次 4 的"去重可独立触发""岗位列表按组折叠"、批次 3 的跨平台城市目录 |

| **P19 批次 3/4 收尾** | ① **去重可独立触发**：新建 `domain/dedupe-sweep.ts`（`dedupDepsOf` 共享候选构造器 + `sweepDedup` 全库复核），`crawl.ts` 的抓取后处理改为共用它；三条入口 `POST /dedup/run` / 模型工具 `job_dedup` / 采集页「全库复核一遍」；`GET /dedup/groups/:id` + 成员带上 `platformName/salaryRaw/sourceUrl`。② **岗位列表按组折叠**：`JobDto.dedupGroupId` + `JobQuery.groupDuplicates`（查询层折叠，组内最小 id 作代表，`total`/分页同一段 WHERE）+ 岗位库「跨平台折叠」开关与展开对照。③ **跨平台城市目录**：新建 `platform/cities.ts`（52 城目录 + `canonicalCityOf` + `orderCities` + `citySupportOf`）；`CriteriaDimension.closed` 显式声明（guopin/hiredchina `true`、lagou `false`）；取值域改为**并集**且"至少一个平台接受即放行"；`criteriaDimensionsFor` 对 city 取并集 | 见 §4.10.1「独立触发与按组折叠」与 §4.6.1「跨平台城市目录」<br>**✅ 达成**：单测 **750（749 通过 / 1 跳过）**。⚠️ 6 处行为做了**变异验证**（复核跳过已分组 / 折叠代表 MIN↔MAX / 城市归一 / 取值域并集 / `closed` 标志 / `groupDuplicates` 参数名），每次都有**指定的那条**断言变红，改回即绿。<br>**仍未做**：疑似重复仍**没有持久化的待确认队列与确认界面**（P15 末节）；跨平台城市目录**没有**接进界面做"每城 × 每平台"的覆盖度矩阵（提示里给了"支持它的是谁"，够用但不够全） |
...
| R18 | **反爬对抗收益不确定**；不引入指纹伪造 / 代理池（D-17），但已启用环境一致性手段（D-17a：patchright / stealth / 端口守卫） | 部分平台可能仍长期抓不到 | 先测量再优化：平台预研逐项验证（猎聘已确认检测 CDP 控制痕迹本身）；扩展点已留（`fingerprint.ts`）；实在不行退到"人工浏览 + 助手记录"的降级形态 |
...
| R25 | ~~**跨平台去重的门槛会被"缺值"击穿，于是同岗位分叉成两条**~~ **已修（P15，2026-09-18）** | 薪资档硬相等被**空薪资**击穿（BOSS 未登录时 `salary_raw` 为空 → 与 51job 的同岗位永不合并）；城市硬相等被"市 vs 市·区"击穿。两处都**不是报错，而是静默地少合并** —— 改完代码后 705 条用例**一条都没红**，正说明这件事在测试里原本不可见 | 已修：薪资**未锚定时不作门槛**（`UNKNOWN_SALARY_BUCKET` 哨兵）、城市比较前**归一到市级**、`[0.75, 0.9)` 报 `candidate` 说明"疑似但未合并"。见 §4.10.1。<br>**仍未做**：疑似候选**没有持久化队列与确认界面**（目前只进日志与抓取汇总） |
...
| ADR-18 | 反爬做"真实浏览器 + 低频 + 环境一致性（D-17a：patchright / stealth 注入 / 端口守卫）"；**不做指纹伪造 / UA 轮换 / 代理池** | 伪造身份类手段会提高已登录账号的风控命中率；而预研证明卡住我们的是 CDP 自动化痕迹（猎聘检测"CDP 控制页面"本身，原版 playwright-core 连 BOSS 都过不去），环境一致性手段正好对症（§4.2.5） |
