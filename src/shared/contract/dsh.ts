...
// ── 模型工具（§22）的实测契约 ─────────────────────────────────────────
// 形状来自运行态 Inspect 的 `tools` 服务契约（ToolDefinition / ToolOutputDefinition /
// ContentBlock），同样只保留我们真正用到的字段。
...
// ── `tool.call.toolview`（§22.3）的实测契约 ───────────────────────────
// 形状来自运行态 Inspect 的 Slot ownerProps（ToolCallOwnerProps）。
// **只声明我们真的会读的字段**。
...
export function serviceOf<T>(ctx: PluginContext, name: string): T | undefined {
