...
export function formatJobListLine(job: JobListLineInput): string {
...
export function parseJobListLine(line: string): JobListLine | undefined {
...
/** 详情卡片的第一行（标题行，没有键名）。 */
export function formatJobDetailTitle(id: number, title: string, company: string | null): string {
  return `#${String(id)} ${title}${company === null || company === '' ? '' : ` @ ${company}`}`
}

/** 一行 `键：值`。 */
export function formatDetailLine(key: DetailKey, value: string): string {
  return `${key}：${value}`
}
...
export function parseDetailLines(text: string): Array<DetailLine | null> {
  return text
    .split('\n')
    .map((raw) => raw.trim())
    .filter((raw) => raw !== '')
    .map((raw) => {
      ...
      if (!(Object.values(DETAIL_KEYS) as string[]).includes(key)) return null
...
export function summarizeJd(text: string | null, limit = JD_SUMMARY_CHARS): string {
  if (text === null || text === '') return '（没有抓到 JD 正文）'
