...
// ── L4 批量投递（与 D3 批量打招呼同构，但可阻塞的原因集合不同）────────
...
export interface StorageUsageDto {
  ...
  /** 附件目录（`files/`）的实际占用。 */
  attachments: { dir: string; fileCount: number; bytes: number }
  /** 导出归档目录（`exports/`）；不存在时全 0。 */
  exports: { dir: string; fileCount: number; bytes: number }
  /** 按表的真实占用，从大到小。 */
...
export interface CleanupResultDto {
  executedAt: string
  items: Array<{ id: string; label: string; rows: number; bytes: number }>
  totalRows: number
