 ...
 */
export const OFFER_COMP_FIELDS: readonly OfferCompFieldSpec[] = [
...
const MONEY_KEYS = new Set<keyof OfferComp>(
  OFFER_COMP_FIELDS.filter((field) => field.kind === 'money').map((field) => field.key),
)
const PERCENT_KEYS = new Set<keyof OfferComp>(
  OFFER_COMP_FIELDS.filter((field) => field.kind === 'percent').map((field) => field.key),
)
const COUNT_KEYS = new Set<keyof OfferComp>(
  OFFER_COMP_FIELDS.filter((field) => field.kind === 'count').map((field) => field.key),
)
const TEXT_KEYS = new Set<keyof OfferComp>(
  OFFER_COMP_FIELDS.filter((field) => field.kind === 'text').map((field) => field.key),
)
 ...
 */
export function normalizeOfferComp(input: unknown): OfferComp {
  const raw = asRecord(input)
  const out: OfferComp = {}
  for (const field of OFFER_COMP_FIELDS) {
    const value = raw[field.key as string]
    if (field.kind === 'bool') {
      if (typeof value === 'boolean') (out as Record<string, unknown>)[field.key] = value
      continue
    }
    if (field.kind === 'text') {
      if (typeof value === 'string') {
        ...
        if (trimmed !== '' && TEXT_KEYS.has(field.key)) {
          (out as Record<string, unknown>)[field.key] = trimmed.slice(0, TEXT_MAX)
        }
      }
      continue
    }
    ...
    if (field.kind === 'money' && MONEY_KEYS.has(field.key)) {
      if (parsed >= 0 && parsed <= MONEY_MAX) (out as Record<string, unknown>)[field.key] = Math.round(parsed)
    } else if (field.kind === 'percent' && PERCENT_KEYS.has(field.key)) {
      if (parsed >= 0 && parsed <= 100) (out as Record<string, unknown>)[field.key] = parsed
    } else if (field.kind === 'count' && COUNT_KEYS.has(field.key)) {
      if (parsed >= 0 && parsed <= COUNT_MAX) (out as Record<string, unknown>)[field.key] = Math.trunc(parsed)
    }
  }
  return out
}
 ...
 */
export function annualCashOf(comp: OfferComp): number | null {
  if (typeof comp.annualCash === 'number') return comp.annualCash
  if (typeof comp.monthlyBase !== 'number' || typeof comp.monthsPerYear !== 'number') return null
  return Math.round(
    comp.monthlyBase * comp.monthsPerYear + (comp.bonusYearly ?? 0) + (comp.allowanceYearly ?? 0),
  )
}

/** 明细里**一个字段都没填**（用来判断"这份 offer 是不是空壳"）。 */
export function isOfferCompEmpty(comp: OfferComp): boolean {
  return Object.keys(comp).length === 0
}

/** 把一项显示成人话；`undefined` → `null`（界面渲染成"未填写"，不是 0）。 */
export function offerCompDisplay(spec: OfferCompFieldSpec, comp: OfferComp): string | null {
  ...
  if (spec.kind === 'percent') return `${String(value)}%`
  if (spec.key === 'commuteMin') return `${String(value)} 分钟`
  if (spec.key === 'annualLeaveDays') return `${String(value)} 天`
  if (spec.key === 'monthsPerYear') return `${String(value)} 个月`
  return String(value)
}
