 ...
 */
export const EXP_BUCKETS: ReadonlyArray<{ id: ExpBucketId; label: string }> = [
...
interface Years {
  ...
  /** `null` = 没有上界（"5年以上"）。 */
  hi: number | null
  /** 是不是"以下 / 以内"这类上界写法（与"1年"这种明确档位不是一回事）。 */
  capped: boolean
}

/** 把原始串读成一个年限区间；读不出数字（且不是不限/应届）就返回 `null`。 */
function parseYears(raw: string): Years | null {
 ...
 */
export function expBucketOf(raw: string): ExpBucketId | null {
 ...
 */
export function buildExpChips(values: string[]): ExpChip[] {
  const byBucket = new Map<ExpBucketId, string[]>()
  ...
  for (const value of values) {
    ...
    if (hits === undefined) byBucket.set(bucket, [value])
 ...
 */
const EDU_TIERS: ReadonlyArray<{ re: RegExp; rank: number }> = [
  { re: /不限/, rank: 0 },
  { re: /(小学|初中|及以下)/, rank: 1 },
...
/** 学历原始串 → 层级序号（越小越低）。 */
export function eduRankOf(raw: string): number {
 ...
 */
export function sortEduValues(values: string[]): string[] {
  return values
