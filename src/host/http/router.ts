...
function requireData(runtime: HostRuntime): void {
  if (!runtime.isReady()) throw dataNotReady(runtime)
}

/** 读一个 JSON 对象体；不是对象就报 400。 */
async function readObject(req: RouteRequest): Promise<Record<string, unknown>> {
...
async function dispatch(runtime: HostRuntime, req: RouteRequest): Promise<RouteResult> {
  ...
  if (segments.length >= 1 && segments[0] === 'applications') {
    ...
    if (method === 'POST' && segments.length === 1) {
      ...
      const application = await service.recordApplication({
        ...
        ...(typeof body['channel'] === 'string' ? { channel: body['channel'] as ApplicationChannel } : {}),
    ...
    if (method === 'POST' && segments.length === 3 && segments[2] === 'advance') {
      const body = await readObject(req)
      const to = body['to']
      if (typeof to !== 'string') throw new DomainError('INVALID_INPUT', 'advance 需要 to')
  ...
  if (method === 'GET' && segments.length === 3 && segments[0] === 'jobs' && segments[2] === 'history') {
    ...
    if (!Number.isFinite(jobId)) throw new DomainError('INVALID_INPUT', `非法岗位 id：${segments[1] ?? ''}`)
    return json(200, {
  ...
  if (segments.length === 3 && segments[0] === 'messages' && segments[2] === 'read') {
    if (method !== 'POST') throw new DomainError('INVALID_INPUT', '标记已读只支持 POST')
    requireData(runtime)
    const id = Number.parseInt(segments[1] ?? '', 10)
  ...
  if (segments.length === 3 && segments[0] === 'messages' && segments[2] === 'reply') {
    if (method !== 'POST') throw new DomainError('INVALID_INPUT', '回复只支持 POST')
    requireData(runtime)
    const id = Number.parseInt(segments[1] ?? '', 10)
    if (!Number.isFinite(id)) throw new DomainError('INVALID_INPUT', `非法消息 id：${segments[1] ?? ''}`)
    const body = await readObject(req)
  ...
  if (method === 'POST' && segments.length === 3 && segments[0] === 'messages' && segments[2] === 'draft-reply') {
    ...
    if (typeof scenario !== 'string') throw new DomainError('INVALID_INPUT', 'scenario 必填')
    // 只生成草稿、不发送；真正发送仍走 /messages/:id/reply（闸门 + 两段式确认）。
    const draft = await runtime.messages().draftReply({ messageId: id, scenario: scenario as never })
  ...
  if (segments.length >= 1 && segments[0] === 'tripartite') {
    requireData(runtime)
    const service = runtime.campus()
    if (method === 'GET' && segments.length === 1) return json(200, { items: service.listTripartite() })
    if (method === 'POST' && segments.length === 1) {
      const body = await readObject(req)
...
function parseStage(raw: string): ApplicationStage {
  if (!(APPLICATION_STAGES as readonly string[]).includes(raw)) {
    throw new DomainError('INVALID_INPUT', `不支持的投递阶段：${raw}`, {
      hint: `合法取值：${APPLICATION_STAGES.join(' / ')}`,
    })
  }
  return raw as ApplicationStage
}
