 ...
 */
function resumeWriteOf(body: Record<string, unknown>): ResumeWriteInput {
...
function resumePatchOf(body: Record<string, unknown>): Partial<ResumeWriteInput> {
  const patch: Partial<ResumeWriteInput> = {}
  if (typeof body['name'] === 'string' && body['name'].trim() !== '') patch.name = body['name'].trim()
  if (typeof body['direction'] === 'string') patch.direction = body['direction']
  if (typeof body['language'] === 'string' && (RESUME_LANGUAGES as readonly string[]).includes(body['language'])) {
    patch.language = body['language'] as ResumeLanguage
  }
  if (typeof body['state'] === 'string' && (RESUME_STATES as readonly string[]).includes(body['state'])) {
    patch.state = body['state'] as ResumeState
  }
...
function parseFormat(raw: unknown): ResumeFormat {
  if (typeof raw === 'string' && (RESUME_FORMATS as readonly string[]).includes(raw)) {
    return raw as ResumeFormat
  }
...
function parseTemplate(raw: string | null): ResumeTemplate {
  if (raw === null || raw === '') return 'concise'
  if (!(RESUME_TEMPLATES as readonly string[]).includes(raw)) {
...
export async function list(ctx: RouteContext): Promise<RouteResult | undefined> {
  ...
  if (!(method === 'GET' && segments.length === 1 && segments[0] === 'resumes')) return undefined
  requireData(runtime)
...
export async function create(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
  if (!(method === 'POST' && segments.length === 1 && segments[0] === 'resumes')) return undefined
  requireData(runtime)
  const service = runtime.resumes()
...
export async function get(ctx: RouteContext): Promise<RouteResult | undefined> {
  ...
  if (!(method === 'GET' && segments.length === 2 && segments[0] === 'resumes')) return undefined
  requireData(runtime)
  const service = runtime.resumes()
...
export async function patch(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
  if (!(method === 'PATCH' && segments.length === 2 && segments[0] === 'resumes')) return undefined
  requireData(runtime)
  const service = runtime.resumes()
  const resumeId = Number.parseInt(segments[1] ?? '', 10)
  if (!Number.isFinite(resumeId)) {
...
export async function remove(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, segments, method } = ctx
  if (!(method === 'DELETE' && segments.length === 2 && segments[0] === 'resumes')) return undefined
  requireData(runtime)
  const service = runtime.resumes()
  const resumeId = Number.parseInt(segments[1] ?? '', 10)
  if (!Number.isFinite(resumeId)) {
...
export async function duplicate(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
...
export async function setDefault(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, segments, method } = ctx
...
export async function preview(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
...
export async function exportResume(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
...
export async function files(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, segments, method } = ctx
  ...
  if (segments.length === 2 && segments[0] === 'files') {
    ...
    if (!Number.isFinite(fileId)) throw new DomainError('INVALID_INPUT', `非法附件 id：${segments[1] ?? ''}`)

    // 删除附件：领域层的 removeFile 早就在了，只是一直没有入口 ——
    // 而界面文案写着"只有你显式删除才会消失"，说到就得做到。
    if (method === 'DELETE') {
      const removed = runtime.resumes().removeFile(fileId)
      if (!removed) throw new DomainError('NOT_FOUND', `附件不存在或已被删除：${String(fileId)}`)
      return json(200, { ok: true })
    }

    if (method !== 'GET') throw new DomainError('INVALID_INPUT', '附件只支持 GET / DELETE')
...
export async function tailor(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx

  // 简历定制：生成与采用
  if (segments.length === 2 && segments[0] === 'resume' && segments[1] === 'tailor') {
    if (method !== 'POST') throw new DomainError('INVALID_INPUT', '定制只支持 POST')
    requireData(runtime)
    const body = await readObject(req)
...
export async function tailoringsList(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
  if (!(method === 'GET' && segments.length === 1 && segments[0] === 'tailorings')) return undefined
  requireData(runtime)
  const service = runtime.resumes()
...
export async function tailoringsAdopt(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, req, segments, method } = ctx
