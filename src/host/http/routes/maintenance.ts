...
export async function storage(ctx: RouteContext): Promise<RouteResult | undefined> {
...
export async function cleanupPreview(ctx: RouteContext): Promise<RouteResult | undefined> {
  const { runtime, segments, method } = ctx
  if (
    !(method === 'POST' &&
      segments.length === 3 &&
...
export async function cleanup(ctx: RouteContext): Promise<RouteResult | undefined> {
  ...
  if (body['only'] !== undefined) {
    ...
    const wanted = body['only'].filter((value): value is string => typeof value === 'string')
