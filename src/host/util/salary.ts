...
function resolveScale(text: string): { unit: SalaryUnit; factor: number | null } {
  ...
  if (perDay) return { unit: 'day', factor: null }
  if (wan && perYear) return { unit: 'year', factor: 10000 / 12 }
  if (wan) return { unit: 'month', factor: 10000 }
  if (k) return { unit: 'month', factor: 1000 }
  if (yuan && perYear) return { unit: 'year', factor: 1 / 12 }
  // `元/月`、以及光秃秃的 `8000-10000` 一律按月薪处理
  return { unit: 'month', factor: 1 }
}

/** 把解析出的数值按系数折算成元/月，四舍五入到整数。 */
function toMonthly(value: number, factor: number): number {
 ...
 */
export function parseSalary(input: string): ParsedSalary {
  const raw = typeof input === 'string' ? input.trim() : ''
  const base: ParsedSalary = { raw, min: null, max: null, months: null, negotiable: false, unit: 'unknown' }
  if (raw === '') return { ...base, note: 'empty' }
  ...
  if (first === undefined) return { ...base, months, note: 'no-number' }
 ...
 */
export function annualPackage(salary: ParsedSalary): number | null {
