 ...
 */
export function strictCompanyName(raw: string): string {
 ...
 */
function stripCompanyName(raw: string, suffixes: readonly string[]): string {
  ...
  while (changed && guard < 8) {
    ...
    const head = REGION_PREFIXES.find(
      (region) => value.startsWith(region) && value.length - region.length >= 2,
    )
    if (head !== undefined) {
    ...
    const tail = REGION_PREFIXES.find(
      (region) => value.endsWith(region) && value.length - region.length >= 2,
    )
    if (tail !== undefined) {
