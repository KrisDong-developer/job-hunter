...
/** 各平台"打开一个目录"的官方做法。 */
function commandFor(dir: string): { file: string; args: string[] } {
  if (process.platform === 'win32') return { file: 'explorer.exe', args: [dir] }
  if (process.platform === 'darwin') return { file: 'open', args: [dir] }
 ...
 */
export async function revealDataFile(dataPath: string): Promise<RevealResult> {
  const dir = dirname(dataPath)
  const { file, args } = commandFor(dir)
  return await new Promise<RevealResult>((resolve) => {
