...
import { SCHEMA_V1, SCHEMA_V2, SCHEMA_V3, SCHEMA_V4, SCHEMA_V5, SCHEMA_V6 } from './schema.js'
...
/** `VACUUM INTO` 一致性快照。 */
function snapshot(db: DatabaseSync, dbPath: string, at: Date, fromVersion: number): string {
 ...
 */
export function migrate(db: DatabaseSync, options: MigrateOptions): MigrationReport {
  ...
  for (const migration of pending) {
    ...
    try {
      ...
      throw new DomainError(
        'DATA_UNAVAILABLE',
        `迁移 v${migration.version} (${migration.name}) 失败，已回滚：${messageOf(error)}`,
        {
          hint:
            backupPath === null
              ? '数据库未被改动，请排查后重试。'
