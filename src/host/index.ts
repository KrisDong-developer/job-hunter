...
export function apply(ctx: PluginContext): void {
  ...
  const approval = serviceOf<ApprovalService>(ctx, 'approval')
  ...
  const runtime = createHostRuntime({
    ...(timer === undefined
      ? {}
      : {
          timer: timer as { timeout(callback: () => void, delay: number): () => void },
        }),
    ...(llm === undefined ? {} : { llm }),
  ...
  ctx.effect(() => {
    try {
      ...
      const registerRoutes = (scope: PluginContext): void => {
