 ...
 */
export function syncDeadlineTodos(store: Store, deadlines: readonly DeadlineDto[], now: string): void {
  const existing = new Map(
    store.todo.listOpen({ kind: 'deadline', limit: 200 }).map((todo) => [todo.ref ?? '', todo]),
  )
  ...
  for (const deadline of deadlines) {
    ...
    if (!deadline.urgent) {
      if (open !== undefined) store.todo.closeByRef('deadline', ref, now)
      continue
    }
