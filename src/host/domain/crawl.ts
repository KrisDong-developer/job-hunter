 ...
 */
async function fetchNewJobDetails(
  deps: CrawlDeps,
  adapter: SiteAdapter,
  clock: Clock,
  options: { deadlineAt?: string | null; burst: BurstGuardLike },
): Promise<{ fetched: number; blocked: BlockKind | null; stopped: boolean; targets: number }> {
  const detail = adapter.detail
  if (detail === undefined) return { fetched: 0, blocked: null, stopped: false, targets: 0 }
  ...
  let fetched = 0
  let blocked: BlockKind | null = null
  let stopped = false
  try {
    for (const jobId of targets) {
      ...
      if (job === undefined || job.sourceUrl === '') continue
      ...
      try {
        ...
        if (store.job.setJdText(jobId, jdText)) fetched += 1
        else if (jdText.trim() === '') {
...
async function executeCrawl(
   ...
   */
  const identityOk: RawJob[] = []
  const withoutIdentity: RawJob[] = []
  for (const raw of partition.accepted) {
    if (raw.platformJobId.trim() === '') withoutIdentity.push(raw)
    else identityOk.push(raw)
  }

  /**
   * ── "见到"与"写成功"分家（2026-09-21 修）────────────────────────────
   *
   * `upsert` 只在**写成功**时刷新 `last_seen_at`，于是"最近见到"实际表达的是
   * "最后一次写成功" —— 列表里解析到了、但被字段断言拦下的那些记录**明明见到了却不刷新**。
   * 两个后果都不小：
   *   * 界面显示"三天前见到"，而它昨天还在列表里（**事实错了**）；
   *   * `cleanup.ts` 的保留条件正是 `last_seen_at < ?` —— **还在招的岗位会被清掉**。
   *
   * 所以只要拿到了身份键，就先记一笔"我见到它了"（只动 `last_seen_at` 一列）。
   * 被字段断言拦下的记录多数**是有 id 的**（缺的是 title/薪资/公司），它们正好走这条路；
   * 而"连身份键都没有"的那批依然救不了 —— 这反过来正是 `NO_IDENTITY` 必须大声报的原因：
   * 那种情况下我们连"我还见过它"都记不下来。
   *
   * 注意顺序：先 touch、后 upsert。upsert 命中同一行时会再写一次 `last_seen_at`
   * （同一个 `now`），所以两边不冲突；而"新岗位"在 touch 时还是 0 行（返回 false），
   * 由 upsert 插入 —— 不需要在这里判存在性。
   */
  let seenTouched = 0
  for (const rejected of partition.rejected) {
    if (rejected.raw.platformJobId.trim() === '') continue
    if (store.job.touch(adapter.id, rejected.raw.platformJobId, now())) seenTouched += 1
  }
  if (seenTouched > 0) {
    deps.logger?.info(
      `[crawl] ${adapter.id}：${String(seenTouched)} 条"见到了但没写成功"（字段断言没过）—— 已只刷新「最近见到」`,
    )
  }
  ...
  if (withoutIdentity.length > 0) {
    deps.logger?.warn(
      `[crawl] ${adapter.id}：${String(withoutIdentity.length)} 条没有平台岗位 id，**已拒绝写入**` +
  ...
  let detailFetched = 0
  let detailTargets = 0
  let detailBlocked: BlockKind | null = null
  ...
  if (!paused && adapter.detail !== undefined) {
    ...
    deps.logger?.info(
      `[crawl] ${adapter.id} 详情补抓：本轮新增 ${String(insertedJobIds.length)} 条 → 取到 JD ${String(detailFetched)} 条` +
