...
async function fetchNewJobDetails(
...
): Promise<{ fetched: number; blocked: BlockKind | null; stopped: boolean }> {
  const detail = adapter.detail
  if (detail === undefined) return { fetched: 0, blocked: null, stopped: false }
  ...
  if (jobIds.length > targets.length) {
    deps.logger?.info(
      `[crawl] ${adapter.id} 本轮新增 ${String(jobIds.length)} 条，` +
...
async function executeCrawl(
  ...
  if (!paused && insertedJobIds.length > 0 && adapter.detail !== undefined) {
    ...
    deps.logger?.info(
      `[crawl] ${adapter.id} 详情补抓：新增 ${String(insertedJobIds.length)} 条 → 取到 JD ${String(detailFetched)} 条` +
