...
const CRITICAL_KEYS = ['monthlyBase', 'monthsPerYear', 'housingFundRatio', 'probationRatio'] as const
...
export interface OfferService {
  list(filter?: { state?: OfferState; companyId?: number; openOnly?: boolean; limit?: number }): OfferDto[]
  ...
  compare(input?: { offerIds?: number[]; useLlm?: boolean }): Promise<OfferCompareDto>
...
export function createOfferService(deps: OfferServiceDeps): OfferService {
  const { store } = deps
   ...
   */
  const daysLeftOf = (deadline: string | null): number | null => {
  ...
  const companyNameOf = (record: OfferRecord): string => {
  ...
  const decorate = (record: OfferRecord): OfferDto => {
    const job = record.jobId === null ? undefined : store.job.detail(record.jobId)
    const application =
      record.applicationId === null ? undefined : store.pipeline.getApplication(record.applicationId)
    return {
      id: record.id,
  ...
  const requireOffer = (id: number): OfferRecord => {
   ...
   */
  const resolveInput = (input: OfferWriteInput): OfferWriteInput => {
    const patch: OfferWriteInput = { ...input }
    ...
    if (input.jobId !== undefined && input.jobId !== null) {
      ...
      if (patch.companyId === undefined && job.companyId !== null) patch.companyId = job.companyId
      if (patch.companyName === undefined && job.companyName !== null) patch.companyName = job.companyName
      if (patch.role === undefined || patch.role === '') patch.role = job.title
    }
  ...
  const hasIdentity = (input: OfferWriteInput): boolean =>
  ...
  return {
    ...
    update(id, patch): OfferDto {
      requireOffer(id)
      const resolved = resolveInput(patch)
      const updated = store.offer.update(id, resolved, clock())
      if (updated === undefined) throw new DomainError('NOT_FOUND', `Offer 不存在：${String(id)}`)
      return decorate(updated)
    },

    setState(id, state): OfferDto {
    ...
    byJob(jobId): OfferDto[] {
      return store.offer.listByJob(jobId).map(decorate)
    },

    async compare(input = {}): Promise<OfferCompareDto> {
      const offers =
        input.offerIds === undefined || input.offerIds.length === 0
      ...
      const table = rows
        ...
        .join('\n')
      const result = await ai.call<{ advice: string }>(
        {
        ...
        },
        {
          parse: (raw) => {
            ...
            if (typeof advice !== 'string' || advice.trim() === '') return undefined
    ...
    upcoming(days): OfferDeadlineDto[] {
...
function shortName(offer: OfferDto): string {
 ...
 */
function compareRows(offers: OfferDto[]): OfferCompareRowDto[] {
  return OFFER_COMP_FIELDS.map((spec) => {
     ...
     */
    const numbers = offers.map((offer) =>
      spec.key === 'annualCash'
        ? offer.annualCash
        : typeof offer.comp[spec.key] === 'number'
          ? (offer.comp[spec.key] as number)
          : null,
    )
    const values = offers.map((offer, index) => {
      if (spec.key !== 'annualCash') return offerCompDisplay(spec, offer.comp)
 ...
 * 不写"建议接受 X"—— 那是用户自己的决定。
 */
function compareFacts(offers: OfferDto[]): string[] {
  const facts: string[] = []
  ...
  const cash = offers
    ...
    .filter((item): item is { index: number; cash: number } => item.cash !== null)
  if (cash.length >= 2) {
    ...
    if (top !== undefined && bottom !== undefined) {
      ...
      facts.push(
        `年现金总包最高的是 ${named[top.index] ?? ''}：${formatMoney(top.cash)}` +
          (gap > 0
  ...
  const withDeadline = offers
    ...
    .filter((item): item is { index: number; daysLeft: number; deadline: string } => item.daysLeft !== null)
