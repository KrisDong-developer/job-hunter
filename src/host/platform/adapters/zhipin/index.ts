...
import {
  ...
  ZHIPIN_FILTER_OPTIONS,
  ZHIPIN_MAX_PAGES,
  ZHIPIN_MAX_SCROLL_ROUNDS,
  ZHIPIN_PAGE_SIZE,
  type ZhipinFilterKey,
} from './config.js'
...
export function createZhipinAdapter(options: ZhipinAdapterOptions = {}): SiteAdapter {
  ...
  const lastSearch = new WeakMap<
    object,
    {
      ...
      rounds: number
      /** 页面自己发过的 joblist 响应（`encryptJobId → 补充字段`）；截不到 = 空表。 */
      seenExtras: Map<string, ZhipinApiExtras>
    }
  >()
  ...
  const enrichFromApi = async (page: PageLike, jobs: RawJob[]): Promise<RawJob[]> => {
    ...
    for (const [id, extras] of remembered.seenExtras) extrasById.set(id, extras)
    ...
    if (stillMissing.length > 0) {
      const pages = Math.min(
        ...
        Math.max(1, Math.ceil(stillMissing.length / config.joblistPageSize)),
      )
      for (let index = 1; index <= pages; index += 1) {
        ...
        if (stillMissing.every((job) => extrasById.has(job.platformJobId))) break
      }
    }
  ...
  const dimensions: CriteriaDimension[] = [
    ...
    ...(
      ...
      hint:
        '取值编码来自站点筛选面板（2026-09-23 实测：编码同时出现在搜索 URL 与 joblist 表单里）。' +
  ...
  return {
    ...
    crawl: {
      async gotoSearch(page, criteria): Promise<void> {
        ...
        const seenExtras = new Map<string, ZhipinApiExtras>()
        ...
        const unsubscribe =
          page.onResponse === undefined
            ...
            : page.onResponse((response) => {
                if (!response.url().includes('joblist.json')) return
                void response
                  .text()
                  .then((text) => {
                    for (const [id, extras] of extrasMapOf(safeJsonParse(text))) seenExtras.set(id, extras)
                  })
                  .catch(() => {
