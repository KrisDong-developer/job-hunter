...
export function buildWaiqiRequestBody(
  ...
  const body: Record<string, unknown> = {
    ...
    expectId: 0,
    status: 1,
    [WAIQI_BODY_FIELDS.sort]:
      criteria.sort === undefined || criteria.sort === '' ? 0 : Number(criteria.sort),
    [WAIQI_BODY_FIELDS.type]: type === '' ? 2 : Number(type),
    // 站点初始请求里 `companyTypeList` **恒定存在**（空数组 = 不筛）——照抄，
    // 于是"没配"与"配了"在请求体指纹上分得开（对账测试靠这一点）。
    [WAIQI_BODY_FIELDS.companyType]: [] as number[],
    page,
    size,
  ...
  if (criteria.city !== undefined && criteria.city !== '') {
    ...
    if (codes.length > 0) body[WAIQI_BODY_FIELDS.city] = codes.join(',')
  }
  const workExp = platformCriterion(criteria, 'workExp')
  if (workExp !== '') body[WAIQI_BODY_FIELDS.workExp] = workExp
  const education = platformCriterion(criteria, 'education')
  if (education !== '') body[WAIQI_BODY_FIELDS.education] = education
  ...
  if (posInfo !== '') body[WAIQI_BODY_FIELDS.posInfo] = posInfo
  ...
  if (companyTypes !== '') {
    ...
    if (ids.length > 0) body[WAIQI_BODY_FIELDS.companyType] = ids
  }
