 ...
 */
export const WAIQI_COMPANY_TYPE_SEED: Array<{ id: number; name: string }> = [
  { id: 28, name: '美企' },
  { id: 29, name: '日企' },
  { id: 30, name: '韩企' },
  { id: 31, name: '德企' },
...
export interface WaiqiConfig {
  ...
  companyTypeList: Array<{ id: number; name: string }>
  fields: WaiqiFields
...
export function mergeWaiqiConfig(override: unknown): WaiqiConfig {
  ...
  const seedList = (
    value: unknown,
    fallback: Array<{ id: number; name: string }>,
  ...
  const cityCodes = (value: unknown): Record<string, number> => {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
      return { ...DEFAULT_WAIQI_CONFIG.cityCodes }
    }
    const merged: Record<string, number> = { ...DEFAULT_WAIQI_CONFIG.cityCodes }
  ...
  return {
    ...
    detailApiEnabled:
      typeof patch.detailApiEnabled === 'boolean'
