 ...
 */
export function contactRowsOf(payload: unknown): LiepinContactRow[] | null {
  if (payload === null || typeof payload !== 'object') return null
  const root = payload as { flag?: unknown; data?: { list?: unknown } }
  // `flag !== 1` = 服务端拒绝（风控 / 头不全）——不当作"没有会话"，交给调用方抛
  if (root.flag !== undefined && Number(root.flag) !== 1) return null
  const list = root.data?.list
  if (!Array.isArray(list)) return null

  const text = (value: unknown): string => (value === undefined || value === null ? '' : String(value).trim())
  const rows: LiepinContactRow[] = []
 ...
 */
export function inboxMessageOf(row: LiepinContactRow): RawInboxMessage {
  ...
  const direction: 'hr' | 'me' =
    row.unReadCnt > 0 ? 'hr' : row.extType === '200' ? 'me' : 'hr'
  return {
