...
import { contactRowsOf, fetchContactListInPage, inboxMessageOf, type LiepinContactRow } from './api.js'
...
export function createLiepinActions(ctx: LiepinActionsContext): NonNullable<SiteAdapter['actions']> {
  ...
  ): Promise<LiepinElementInfo> =>
    await page.evaluate(
      elementCenterInPage,
      textIncludes === undefined && excludeAncestor === undefined
        ? { selector }
        : {
            selector,
  ...
  const clickSelector = async (
    ...
    textIncludes?: string,
    excludeAncestor?: string,
  ): Promise<LiepinElementInfo | null> => {
  ...
  /** 读输入框当前值（回车前的上屏校验用）。 */
  const composerValue = async (page: PageLike): Promise<string> =>
    await page.evaluate(composerValueInPage, { selector: config.chatSelectors.composer })

  /** 清空输入框（有修饰键能力时；没有就跳过 —— 绝不 DOM 改写）。 */
  const clearComposer = async (page: PageLike): Promise<void> => {
    const keyboard = page.keyboard
    if (keyboard === undefined || keyboard.down === undefined || keyboard.up === undefined) return
  ...
  const typeIntoComposer = async (
    page: PageLike,
    text: string,
  ): Promise<{ ok: boolean; via: string; value: string }> => {
    const keyboard = page.keyboard
    if (keyboard === undefined) return { ok: false, via: 'no-keyboard', value: '' }
    ...
    if (value === text) return { ok: true, via: 'insertText', value }
    if (keyboard.type !== undefined) {
      ...
      if (value === text) return { ok: true, via: 'keyboard.type', value }
    }
  ...
  const waitForDelivery = async (
    ...
    let last: 'delivered' | 'pending' | 'missing' = 'missing'
   ...
   */
  const typeSendVerify = async (page: PageLike, text: string): Promise<ActionResult> => {
    ...
    if (!typed.ok) {
      return {
        ...
        evidence: 'none',
        message:
          `话术没能输入到输入框（键盘路径=${typed.via}，textarea.value=${JSON.stringify(typed.value)}）` +
    ...
    return {
      ...
      evidence: 'none',
      message:
        '已按回车，但会话里没有确认到这条消息 —— 按未发送处理（不重试，避免重复发送；请人工检查会话）。',
    }
  }
   ...
   */
  const fetchContactRows = async (page: PageLike): Promise<LiepinContactRow[]> => {
    const collected: LiepinContactRow[] = []
    ...
    for (let pageNo = 1; pageNo <= config.contactListMaxPages; pageNo += 1) {
      ...
      if (flag !== undefined && Number(flag) !== 1) {
        ...
        throw new Error(
          `猎聘会话列表接口返回 flag=${String(flag)}（第 ${String(pageNo)} 页）：${detail} —— ` +
      ...
      if (rows === null) {
        throw new Error(
          `猎聘会话列表响应结构不认识（第 ${String(pageNo)} 页）—— 既不是空表也不是会话行数组。` +
      ...
      if (rows.length < config.contactListPageSize) break
    }
    return collected
  }
  ...
  const sayHello = async (
    ...
    const button = await locate(
      page,
      config.chatSelectors.chatButton,
      config.chatSelectors.chatButtonText,
      config.chatSelectors.chatButtonExclude,
    )
    if (!button.found) {
      return {
        ok: false,
        ...
        evidence: 'none',
        message:
          '岗位详情页没出现「聊一聊」入口 —— 可能岗位已关闭、入口需登录，或已被风控拦截。不重试。',
      }
    }
    ...
    if (sawResumeModal) {
      return {
        ...
        evidence: 'dom',
        message:
          '平台**拒绝**了这次沟通：弹出「简历完整度不足」（open-chat code 30011）——' +
    ...
    if (!sawComposer) {
      return {
        ...
        evidence: 'none',
        message:
          `点了「聊一聊」，但 ${String(config.actionWaitMs / 1000)} 秒内既没弹「完善简历」也没出现会话输入框` +
          '（可能未登录 / 会话面板加载失败）—— 未发送。',
      }
    }
  ...
  const reply = async (
    ...
    if (row === null) {
      return {
        ...
        evidence: 'none',
        message:
          `会话列表里没找到「${hint}」的会话 —— 可能对方还没回过话（会话不存在）、` +
