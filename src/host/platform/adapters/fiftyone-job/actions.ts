/**
 * 51job 的**高危动作实现**（打招呼 / 回复 / 投递 / 读收件箱 / 探阶段）及交互工具。
 *
 * DB 覆盖的配置与动作链的接线在 `./index.ts`：本文件只拿 `config` 与**判墙断言入口**
 * （`assertActionPage` 的实现在 index.ts —— `guard.detectBlock` 与动作链共用那一份）。
 *
 * ## 证据状态（2026-09-21，动作语义按 zhipin 的成熟范式写，选择器证据分层）
 *
 *   * ✅ **有实测证据的链路**：投递（`button.btn.apply` → `.apply-component-resume-dialog`
 *     → 选简历 → 确认 → `.success_title`）与「去聊聊」的**未登录**形态
 *     （`.chat-popover` 微信扫码引导）—— 都来自真实夹具 `51job-sz.html`；
 *   * ⚠️ **候选待校准的链路**：登录态站内会话（输入框 / 消息列表）、消息页地址
 *     （`chatUrl`）。这些路径**fail-closed**：结构对不上就如实报 `missing` 并说清
 *     "选择器待校准 / 需要登录"，绝不把"点了按钮"讲成"发出去了"。
 *
 * ⚠️ 按 §7 的站点规则：所有点击/输入必须走 `platform/humanize.ts` 的 CDP Input 级
 *   交互（`page.mouse` / `page.keyboard`），绝不用 DOM `el.click()` / `fill`
 *   （`isTrusted=false` 是最廉价的自动化特征）。没有输入面时**fail-closed**。
 */
import type { ContactStage } from '../../../../shared/contract/enums/pipeline.js'
import {
  dwellBeforeActMs as dwellMs,
  humanClick as clickFn,
  humanHover as hoverFn,
  humanPress as pressFn,
  humanType as typeFn,
} from '../../humanize.js'
import type { ActionResult, PageLike, RawInboxMessage, SiteAdapter } from '../../types.js'
import type { FiftyOneConfig } from './config.js'
import type { FiftyOneElementInfo } from './page/list.js'
...
export function createFiftyOneActions(ctx: FiftyOneActionsContext): NonNullable<SiteAdapter['actions']> {
  ...
  const clickSelector = async (
    ...
    selector: string,
    textIncludes?: string,
  ): Promise<FiftyOneElementInfo | null> => {
   ...
   */
  const send = async (page: PageLike): Promise<void> => {
  ...
  const sayHello = async (
    ...
    if (!(await waitFor(page, config.chatSelectors.chatButton, config.actionWaitMs))) {
      return {
        ...
        evidence: 'none',
        message:
          '岗位详情页没出现「去聊聊 / 立即沟通」入口 —— 可能岗位已关闭、未登录，或已被风控拦截。不重试。',
      }
    }
    ...
    if (entry.kind === 'qr-popover') {
      return {
        ...
        evidence: 'dom',
        message:
          `点「去聊聊」后出现的是**微信扫码引导弹层**（${entry.hrName === '' ? 'HR 信息未展示' : `HR：${entry.hrName}`}，` +
          `${entry.tip === '' ? '文案未展示' : `平台提示「${entry.tip}」`}）—— ` +
          '这一形态下没有可输入的会话框，消息没有发出。请先登录 51job 再用站内沟通。',
      }
    }
    const inChat =
      entry.kind === 'chat-input' || (await waitFor(page, config.chatSelectors.chatInput, config.actionWaitMs))
    if (!inChat) {
      return {
        ok: false,
        delivery: 'missing',
        evidence: 'none',
        message:
          '点了沟通入口，但既没有出现扫码弹层、也没有出现会话输入框 —— 登录态会话的 DOM 未校准' +
  ...
  const reply = async (
    ...
    if (!(await openConversation(page, job))) {
      return {
        ...
        evidence: 'none',
        message:
          `没能在 51job 消息页里找到「${job.company === '' ? job.title : job.company}」的会话 —— ` +
  ...
  const sendResume = async (
    ...
    if (filePath !== null) {
      return {
        ...
        evidence: 'none',
        message:
          '51job 的投递走**平台简历库**（「投递」弹窗里选的是账号里的在线/附件简历），' +
    ...
    if (!(await waitFor(page, config.chatSelectors.applyButton, config.actionWaitMs))) {
      return {
        ...
        evidence: 'none',
        message:
          '岗位详情页没出现「投递」按钮 —— 可能岗位已关闭、未登录，或已被风控拦截。不重试。',
      }
    }
    ...
    if (state.rowCount === 0) {
      return {
        ...
        evidence: 'dom',
        message:
          `投递弹窗里**没有可选的简历**${state.hintText === '' ? '' : `（弹窗内容：「${state.hintText}」）`}—— ` +
    ...
    if (afterPick.confirmDisabled) {
      return {
        ...
        evidence: 'dom',
        message:
          '选中简历后确认按钮仍是**不可用**状态（带 disabled）—— 平台没接受这次选择，未投递。' +
