 ...
 */
export function readInboxInPage(arg: { selectors: FiftyOneInboxSelectors }): RawInboxMessage[] {
  ...
  if (arg.selectors.listContainer !== '') {
    ...
    if (container === null) {
      throw new Error(
        `会话列表容器未找到（${arg.selectors.listContainer}）—— 选择器未校准或当前页面不是 51job 消息页` +
...
export function detectStageInPage(arg: {
  ...
  if (statusClass.includes('read')) return { stage: 'read', reason: '状态类名含 read' }
