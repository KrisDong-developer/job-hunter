 ...
 * 命中即停（C12），**不重试**。
 */
export function detectBlockInPage(arg: { host: string; card: string }): BlockKind | null {
  let hostname = ''
  try {
  ...
  // 被重定向到别的域（Indeed.cn 停运的实际表现）→ 判 blank。
  if (hostname !== '' && arg.host !== '' && hostname !== arg.host) return 'blank'
  ...
  const cloudflare = document.querySelector(
    '#challenge-error-title, .cf-error-details, .cf-error-h1, [id^="challenge-running"], iframe[src*="challenges.cloudflare.com"]',
  )
  let cards = 0
  try {
  ...
  if (cloudflare !== null) return 'captcha'
  if (/需要进行其他验证|请稍候|Ray ID|cloudflare|cf-error/i.test(compact)) return 'captcha'
  if (/验证码|captcha|安全验证|robot/.test(compact)) return 'captcha'
  if (/访问过于频繁|请求过于频繁|请稍后再试|异常流量/.test(compact)) return 'rate-limited'
  // 整页被清空 / 无卡片且正文极短 → 大概率被换走或被拦。
