 ...
 */
export const LINKEDIN_SALARY_PATTERN =
  '[$€£¥₹]\\s?\\d[\\d,]*(?:\\.\\d+)?\\s*[Kk万]?(?:\\s*/\\s*(?:yr|year|hr|hour|mo|month|月))?(?:\\s*[-–—~]\\s*[$€£¥₹]?\\s?\\d[\\d,]*(?:\\.\\d+)?\\s*[Kk万]?(?:\\s*/\\s*(?:yr|year|hr|hour|mo|month|月))?)?'
 ...
 */
export const LINKEDIN_POSTED_WITHIN_OPTIONS: Array<{ value: string; label: string }> = [
 ...
 */
export const LINKEDIN_CITY_SUGGESTIONS: readonly string[] = [
  'China',
  'Remote',
  'Singapore',
  'Germany',
...
export function mergeLinkedInConfig(override: unknown): LinkedInConfig {
  if (override === null || typeof override !== 'object') return DEFAULT_LINKEDIN_CONFIG
  const patch = override as Partial<LinkedInConfig>
  const str = (key: keyof LinkedInConfig, fallback: string): string =>
    typeof patch[key] === 'string' && patch[key] !== '' ? (patch[key] as string) : fallback
  const num = (key: keyof LinkedInConfig, fallback: number): number =>
  ...
  return {
    ...
    salaryPanelEnabled:
      typeof patch.salaryPanelEnabled === 'boolean' ? patch.salaryPanelEnabled : DEFAULT_LINKEDIN_CONFIG.salaryPanelEnabled,
...
export function linkedinBlockFlags(host: string): { expectedHost: string; skipLoginWall: boolean } {
