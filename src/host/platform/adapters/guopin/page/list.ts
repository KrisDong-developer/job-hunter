...
export function extractJobsInPage(arg: GuopinConfig): RawJob[] {
  ...
  /**
   * 确定性内容哈希（FNV-1a **32 位** → hex）。自包含、同步，页面/夹具都可跑。
   *
   * ⚠️ **已知残余风险（2026-09-21 记账，故意没改）**：32 位在 1 万条量级约有 1.2% 的
   * 概率出现至少一对碰撞（5 万条约 29%），而碰撞的后果不是"疑似重复"——
   * 身份键相同 → upsert **覆盖**（一条岗位被另一条写掉）。
   *
   * 暴露面为什么现在可以接受：国聘已经改走列表接口（`api.ts`），接口路径给的是**真 `jobId`**，
   * 这个哈希只剩**接口挂了之后的 DOM 兜底**在用。
   *
   * 为什么没有直接换 64 位：`platform_job_id` 是身份键（`UNIQUE(platform_id, platform_job_id)`），
   * 换宽度会让**已有行的键全部变掉** → 同一批岗位在下一次抓取时全部重新插入一遍。
   * 正确做法是"换哈希 + 一条把老 `ch:` 键按 `title|company|city|district` 重算成新键的
   * **数据迁移**"，而当前迁移清单是纯 SQL（`migrate.ts` 的 `{version,name,sql}`），
   * 且迁移失败会让插件**拒绝启动** —— 迁移里如果撞上已经碰撞的两行，`UPDATE` 会违反唯一索引。
   * 所以它需要：① 迁移机制支持一段 JS；② 迁移里显式处理"重算后撞键"（先记为待人工处理，
   * 不能 `UPDATE OR REPLACE` —— 那会删掉一条岗位、并级联投递记录）。
   */
  ...
  for (const card of Array.from(cards)) {
    ...
    try {
      const salaryText = (card.querySelector(arg.selectors.salary)?.textContent ?? '')
        .replace(/\s+/g, '')
        .trim()
      if (salaryText !== '' && salaryRe !== null && salaryRe.test(salaryText)) salaryRaw = salaryText
    } catch {
      salaryRaw = ''
    }
...
export async function turnToPageInPage(arg: {
  ...
  for (let step = 0; step < 40 && current < arg.targetPage; step += 1) {
    let nextLi: Element | null = null
    try {
      nextLi = document.querySelector(arg.next)
    } catch {
      nextLi = null
    }
    if (nextLi === null) return false
    try {
      if (document.querySelector(arg.nextDisabled) !== null) return false
    } catch {
      /* 选择器非法 → 当作可点 */
    }
 ...
 */
export function hasNextPageInPage(arg: { next: string; nextDisabled: string }): boolean {
  try {
    if (document.querySelector(arg.next) === null) return false
