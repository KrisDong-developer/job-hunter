...
export function createGuopinAdapter(options: GuopinAdapterOptions = {}): SiteAdapter {
  ...
  const dimensions: CriteriaDimension[] = [
    ...
    },
    {
      ...
      hint:
        '对应接口的 `search.experience`（**数组**，2026-09-21 探针实测：点「1-3年」→ ' +
    ...
    },
    {
      ...
      hint:
        '对应接口的 `search.major`（**数组**，2026-09-21 探针实测：api-diff 验证 400→364）。' +
   ...
   */
  const listCache = new WeakMap<object, GuopinListPage>()
  ...
  return {
    ...
    criteria: {
      ...
      preview(criteria: SearchCriteria) {
        ...
        const parsed = JSON.parse(body) as { search: Record<string, unknown> }
    ...
    crawl: {
      async gotoSearch(page, criteria): Promise<void> {
        ...
        const apiPage = guopinListPageOf(
          await page
      ...
      async readListPage(page): Promise<RawJob[]> {
        ...
        if (cached !== undefined) return cached.jobs
