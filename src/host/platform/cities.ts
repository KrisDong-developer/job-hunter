 ...
 */
export const CITY_DIRECTORY: readonly string[] = [
  // 直辖市
  '北京',
  '上海',
  '天津',
 ...
 */
export function canonicalCityOf(input: string): string | null {
 ...
 */
export function orderCities(names: readonly string[]): string[] {
 ...
 */
export function citySupportOf(adapter: SiteAdapter, city: string): CitySupport {
  const dimension = adapter.criteriaDimensions.find((item) => item.key === 'city')
  if (dimension === undefined) return 'free-text'
  const closed = dimension.closed ?? dimension.values.length > 0
  if (!closed) return 'free-text'
