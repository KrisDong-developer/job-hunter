...
export function queryParamsOf(url: string): Record<string, string> {
  const index = url.indexOf('?')
  if (index < 0) return {}
  const out: Record<string, string> = {}
  for (const [key, value] of new URLSearchParams(url.slice(index + 1))) out[key] = value
  return out
}
 ...
 * 界面就会拿一句错话解释它。
 */
export function crawlOnlyKeysOf(adapter: SiteAdapter): string[] {
