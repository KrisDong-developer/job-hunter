...
/** 取一条记录上某个核心字段的值。 */
export function coreFieldValue(raw: RawJob, field: CoreField): string {
  switch (field) {
    case 'title':
      return raw.title ?? ''
    case 'salary_raw':
 ...
 */
export function partitionByRequiredFields(
  records: readonly RawJob[],
  required: readonly CoreField[],
): PartitionResult {
  const accepted: RawJob[] = []
  const rejected: RejectedRecord[] = []
  const counters = new Map<CoreField, number>()
  for (const field of required) counters.set(field, 0)

  for (const raw of records) {
    const missing: CoreField[] = []
    for (const field of required) {
      const value = coreFieldValue(raw, field).trim()
      if (value === '') missing.push(field)
      else counters.set(field, (counters.get(field) ?? 0) + 1)
    }
    if (missing.length === 0) accepted.push(raw)
    else rejected.push({ raw, missing })
  }

  const presence: FieldPresence[] = required.map((field) => ({
    field,
 ...
 */
export function isFieldMiss(presence: FieldPresence): boolean {
