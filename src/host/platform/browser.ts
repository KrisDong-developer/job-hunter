...
export function createBrowserManager(options: BrowserManagerOptions): BrowserManager {
  ...
  let playwrightModule: typeof import('playwright-core') | undefined
  ...
  const loadPlaywright = async (): Promise<typeof import('playwright-core')> => {
    ...
    try {
      ...
      throw new Error(
        `playwright-core 不可用：${error instanceof Error ? error.message : String(error)}。` +
  ...
  return {
    ...
    async release(page: BrowserPage): Promise<void> {
      ...
      if (context === undefined) return
      if (context.pages().length <= 1) return
