...
export function createPlatformLocks(): PlatformLocks {
  const inFlight = new Set<string>()

  return {
    async tryRun<T>(platformId: string, fn: () => Promise<T>): Promise<T | null> {
      // 同步判重（同 tick 内的第二次调用也看得到"忙"，不必等微任务）——
      // 这条是从原 mutex 继承的关键性质，注释见那边的历史教训。
      if (inFlight.has(platformId)) return null
      inFlight.add(platformId)
      try {
        return await fn()
      } finally {
        inFlight.delete(platformId)
      }
    },
    ...
    running(platformId: string): boolean {
      return inFlight.has(platformId)
    },
  }
}
