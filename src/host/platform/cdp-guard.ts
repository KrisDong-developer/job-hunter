 ...
 */
export async function installPortGuard(
  ...
  const onPaused = (raw: unknown): void => {
    ...
    void session
      .send('Fetch.failRequest', {
        ...
        errorReason: 'ConnectionRefused' as const,
      })
      .catch((error: unknown) => {
        logger?.warn(
          `[cdp-guard] failRequest 失败：${error instanceof Error ? error.message : String(error)}`,
        )
      })
  }
