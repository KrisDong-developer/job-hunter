 ...
 */
export function blockFromApiFailure(input: { code?: number | null; message: string }): BlockKind | null {
  const message = input.message
  if (/登录|login/i.test(message)) return 'login-required'
  if (input.code === 429 || /频繁|限流|限制|稍后再试|过于频繁/.test(message)) return 'rate-limited'
  if (/额度|次数过多|达到上限/.test(message)) return 'quota-exhausted'
  return null
}
