...
function takeDailyQuota(deps: EnrichmentDeps, now: Date): void {
  const pad = (n: number): string => String(n).padStart(2, '0')
  const today = `${String(now.getFullYear())}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`
  const current = deps.settings.get<{ date: string; count: number }>(DAILY_KEY, 'global')
  const count = current?.date === today ? current.count : 0
  if (count >= DAILY_LIMIT) {
    throw new DomainError(
      'QUOTA_EXCEEDED',
      `今天的工商查询已到上限（${String(DAILY_LIMIT)} 家）。这是刻意的自我节制 —— 明天再查，或直接点来源链接去网页上看`,
    )
  }
...
export type EnrichOutcome =
  | { kind: 'done'; enrichment: CompanyEnrichmentRecord }
...
export async function enrichCompany(
  ...
  if (company === undefined) throw new DomainError('NOT_FOUND', `公司不存在：${String(companyId)}`)

  return await serialize(async () => {
    // pick（点选候选）不再扣额度 —— 搜索那一步已经为这家公司扣过了，同一家扣两次不公平
    if (pick === undefined) takeDailyQuota(deps, new Date())
    ...
    if (provider === undefined) throw new DomainError('INTERNAL', '工商数据源未注册')
    ...
    try {
      ...
      let confidence: CompanyEnrichmentRecord['confidence']
      let candidates: EnrichmentCandidateDto[] = []
      if (pick !== undefined && pick.url !== '') {
        ...
        if (verdict.kind === 'pick-one') return { kind: 'pick-one', candidates: verdict.candidates }
