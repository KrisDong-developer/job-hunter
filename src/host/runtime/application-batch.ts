...
function guardInputOf(
  ...
  actor: Actor,
  payload: { resumeVersion: string; resumeFileId: number | null; sideEffect: string | null },
): GuardInput {
  return {
...
export async function previewApplicationBatch(
  deps: ApplicationBatchDeps,
  input: ApplicationBatchPreviewInput,
  ...
  const items: ApplicationBatchItemDto[] = []
  ...
  for (const jobId of input.jobIds) {
    ...
    const blocked = (blocker: ApplicationBatchBlockerDto): ApplicationBatchItemDto => ({
      ...base,
      willDeliver: false,
      blocker,
    ...
    if (uploadsHere) uploadsAny = true
    items.push({
      ...base,
      willDeliver: true,
      blocker: null,
  ...
  return {
    ...
    note:
      '这是**预测**：同公司冷却与当日额度余量都在这一批内模拟过，但真正的判定仍发生在每一条的闸门上，' +
...
export async function sendApplicationBatch(
  ...
  return {
    ...
    note:
      `逐条过闸门、逐条回执：成功 ${String(sent)} 条、失败 ${String(failed)} 条。` +
      (unconfirmed === 0
        ? ''
