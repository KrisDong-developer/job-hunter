...
export function readOnlyReasonOf(deps: {
  ...
  if (!deps.storeReady) return deps.failureMessage ?? '数据层尚未就绪'
  if (!deps.lease.held()) {
 ...
 * @returns 是否接管成功
 */
export function takeOverLease(deps: TakeOverDeps): boolean {
  const verdict = deps.lease.acquire()
  if (!verdict.held) return false
  const message = `[${PLUGIN_ID}] ${deps.describe(verdict)}`
  if (deps.level === 'warn') deps.logger?.warn(message)
  else deps.logger?.info(message)
