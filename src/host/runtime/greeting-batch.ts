...
export interface GreetingBatchDeps {
  store: Store
  /** 生成话术（低危，不发送）。 */
  draft(input: { jobId: number }): Promise<GreetingDraftDto>
...
export async function previewGreetingBatch(
  deps: GreetingBatchDeps,
  input: GreetingBatchPreviewInput,
  ...
  const items: GreetingBatchItemDto[] = []
  ...
  for (const jobId of input.jobIds) {
    ...
    const blocked = (blocker: GreetingBatchBlockerDto): GreetingBatchItemDto => ({
      ...base,
      willSend: false,
      blocker,
      text: null,
    ...
    const preview = deps.previewGuard(
      guardInputOf(
        {
          jobId: job.id,
          platformId: job.platformId,
  ...
  return {
    ...
    note:
      '这是**预测**：同公司冷却与当日额度都在这一批内模拟过，但真正的判定仍发生在每一条的闸门上，' +
...
export async function sendGreetingBatch(
  ...
  for (let index = 0; index < input.items.length; index += 1) {
    ...
    if (delay !== null) await deps.sleep(delay)
  ...
  return {
    ...
    note:
      `逐条过闸门、逐条回执：成功 ${String(sent)} 条、失败 ${String(receipts.length - sent)} 条。` +
