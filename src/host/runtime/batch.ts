...
export class CompanyDeduper {
   ...
   *   （用户把冷却期关掉之后，预览不该还在拦他）。
   */
  constructor(readonly cooldownMinutes: number) {}
  ...
  take(companyId: number | null): void {
    if (this.cooldownMinutes > 0 && companyId !== null) this.seen.add(companyId)
  }
}
...
export class QuotaReserver {
  ...
  constructor(private readonly remainingToday: (platformId: string) => QuotaReadout | null) {}
   ...
   */
  take(platformId: string): { reserved: true } | { reserved: false; limit: number; used: number } {
    ...
    if (left <= 0) return { reserved: false, limit: current.limit, used: current.used }
 ...
 */
export function assertBatchSize(size: number, max: number, noun: string): void {
  if (size === 0) {
  ...
  if (size > max) {
    throw new DomainError(
      'INVALID_INPUT',
      `一次最多${noun} ${String(max)} 条，这次是 ${String(size)} 条`,
      { hint: `请分批 —— 逐批${noun}与逐批确认是有意的约束（§4.4.2：确认时要能看到全部内容）。` },
    )
  }
}
 ...
 */
export function interItemDelayMs(
  index: number,
  random: () => number,
...
export function failureOf(error: unknown): { code: string; message: string; hint: string | null } {
