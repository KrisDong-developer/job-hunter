 ...
 *   * 低危 → 不打扰用户。
 */
export function needsApproval(input: GuardInput, config: GuardConfig): boolean {
  if (!config.requireApproval) return false
  if (input.danger === 'high') return true
...
export function createGuard(deps: GuardDeps): Guard {
  ...
  return {
    async run<T>(input: GuardInput, fn: (token: GuardToken) => Promise<T>): Promise<T> {
