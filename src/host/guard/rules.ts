...
const ACTION_SWITCH: Record<string, keyof GuardConfig['levels']> = {
...
/** 动作 → 额度桶。 */
const ACTION_QUOTA: Record<string, keyof GuardConfig['dailyLimits']> = {
...
function deny(reason: GuardDeniedReason, message: string, hint?: string): RuleVerdict {
...
export function readGuardConfig(store: Store): GuardConfig {
  ...
  return {
    ...
    cooldownMinutes:
      typeof stored.cooldownMinutes === 'number' ? stored.cooldownMinutes : DEFAULT_GUARD_CONFIG.cooldownMinutes,
...
export function writeGuardConfig(store: Store, patch: Partial<GuardConfig>, now: string): GuardConfig {
...
/** 第 1 项：功能开关（D-3 发送分层）。 */
export function checkSwitch(store: Store, input: GuardInput): RuleVerdict {
  const key = ACTION_SWITCH[input.action]
  if (key === undefined) return OK
  const config = readGuardConfig(store)
  if (config.levels[key]) return OK
  return deny(
    'switch',
...
export function checkStealth(ctx: RuleContext, input: GuardInput): RuleVerdict {
  if (input.danger !== 'high') return OK
  ...
  if (state.hiddenFromCurrentEmployer === true) return OK
  return deny(
    'stealth',
    `未确认对当前雇主隐藏（${platformId}）`,
    '在平台设置里开启「对当前公司隐藏」，并完成一次隐身检查后再试。',
  )
}

/** 第 3 项：批量上限（§22.4）。 */
export function checkBatch(store: Store, input: GuardInput): RuleVerdict {
  if (input.actor !== 'model') return OK
  const config = readGuardConfig(store)
  ...
  if (count <= config.batchLimit) return OK
  return deny(
    'batch',
    `模型单次调用涉及 ${String(count)} 个岗位，超过上限 ${String(config.batchLimit)}`,
    `请分批，每批不超过 ${String(config.batchLimit)} 个（§22.4：超出必须分批并逐批审批）。`,
  )
}

/** 第 4 项：每日额度（D7）。计数直接读审计表 —— 它就是"今天做了什么"的事实来源。 */
export function checkQuota(ctx: RuleContext, input: GuardInput): RuleVerdict {
  const bucket = ACTION_QUOTA[input.action]
  if (bucket === undefined) return OK
  const platformId = input.target?.platformId
  if (platformId === undefined) return OK
  ...
  return deny(
    'quota',
    `${platformId} 今天的${input.action}已达上限（${String(used)}/${String(limit)}）` +
      (capped ? `，受限于平台侧上限 ${String(cap)}` : `，受限于你设的每日额度 ${String(budget)}`),
    capped
...
export function checkCooldown(ctx: RuleContext, input: GuardInput): RuleVerdict {
  ...
  if (companyId === undefined) return OK

  const config = readGuardConfig(ctx.store)
  if (config.cooldownMinutes <= 0) return OK
  const clock = ctx.clock ?? systemClock
  ...
  return deny(
    'cooldown',
    `同一公司（#${String(companyId)}）在冷却期内已经操作过（上次 ${last.at}）`,
    `冷却期 ${String(config.cooldownMinutes)} 分钟。这是防误投与防风控的保护，不建议绕过。`,
  )
}
 ...
 */
export function checkForbidden(_ctx: RuleContext, input: GuardInput): RuleVerdict {
  if (input.actor !== 'model') return OK
  const patch = input.payload?.['patch']
  if (input.action !== 'settings.write' || patch === null || typeof patch !== 'object') return OK

  const keys = Object.keys(patch as Record<string, unknown>)
  const touched = keys.filter((key) => (FORBIDDEN_FOR_MODEL as readonly string[]).includes(key))
  if (touched.length === 0) return OK
  return deny(
    'forbidden',
    `模型不得修改这些配置：${touched.join('、')}`,
    '审批开关、审计开关、批量上限、额度与冷却期只能由用户在界面上改（§22.4 禁止项）。',
  )
}

/** 走完整条链（审批在 `guard/index.ts` 里做，因为它需要用户交互）。 */
export function runRuleChain(ctx: RuleContext, input: GuardInput): RuleVerdict {
  const checks = [
    ...
    (): RuleVerdict => checkCooldown(ctx, input),
  ]
...
/** 把规则拒绝翻译成领域错误。 */
export function toDomainError(verdict: RuleVerdict): DomainError {
