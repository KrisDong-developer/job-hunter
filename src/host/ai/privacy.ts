...
export const HARD_BLOCKED_PATTERNS: Array<{ name: string; pattern: RegExp }> = [
...
/** 对字符串值做模式脱敏。 */
export function redactPatterns(value: string): { text: string; hits: string[] } {
  let text = value
  const hits: string[] = []
...
export function applyPrivacy(
  ...
  for (const [key, raw] of Object.entries(payload)) {
    ...
    if (raw === null || raw === undefined) {
      ...
      if (hits.length > 0) redactedFields.push(...hits.map((hit) => `${key}:${hit}`))
      safe[key] = text
