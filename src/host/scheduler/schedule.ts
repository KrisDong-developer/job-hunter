...
/** 空 weekdays 视为每天。 */
function effectiveWeekdays(schedule: PlanSchedule): number[] {
  return schedule.weekdays.length === 0 ? [0, 1, 2, 3, 4, 5, 6] : schedule.weekdays
}

/** `from` 之后的下一个触发时刻；`jitter` 直接加到结果上。 */
export function nextRunAt(schedule: PlanSchedule, from: Date, jitterMs = 0): Date {
...
export function previousRunAt(schedule: PlanSchedule, now: Date): Date | null {
  const weekdays = effectiveWeekdays(schedule)
  for (let offset = 0; offset <= 8; offset += 1) {
    const candidate = new Date(
      now.getFullYear(),
...
export function missedRun(
  ...
  if (lastRunAt === null) return { missed: false, expectedAt: previousRunAt(schedule, now) }

  const last = new Date(lastRunAt)
  if (Number.isNaN(last.getTime())) return { missed: false, expectedAt: previousRunAt(schedule, now) }
...
export function jitterFor(schedule: PlanSchedule, random: () => number = Math.random): number {
