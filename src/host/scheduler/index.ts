...
import type { PlanDto, PlanScheduleStatusDto, SchedulerStatusDto, TriggerDecisionDto, WeeklyTriggerDto } from '../../shared/contract/dto/plan.js'
...
const toPlatformOutcome = (outcome: LaneOutcome<CrawlSummaryDto>): PlatformOutcome => ({
...
export function createScheduler(deps: SchedulerDeps): Scheduler {
  ...
  const publishOutcome = (
    planId: number,
    planName: string,
    outcome: LaneOutcome<CrawlSummaryDto>,
    source: string,
  ): void => {
  ...
  return {
    ...
    async runPlan(planId, reason): Promise<CrawlSummaryDto> {
      ...
      for (const outcome of outcomes) {
        if (outcome.summary !== null) last = outcome.summary
      }
      const firstFailure: unknown = laneOutcomes.find((outcome) => outcome.error !== null)?.error ?? null
