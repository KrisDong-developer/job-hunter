...
export function campusTools(runtime: HostRuntime): ToolDefinition[] {
  ...
  return [
    tool<Record<string, unknown>, { text: string }>({
      ...
      parameters: schema(
        {
          action: enumStr(
            ['list', 'windows', 'create', 'advance', 'assessment', 'assessment-state', 'tripartite', 'tripartite-state', 'talks'],
            '要做的操作',
          ),
