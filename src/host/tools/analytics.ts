...
export function analyticsTools(runtime: HostRuntime): ToolDefinition[] {
  ...
  return [
    tool<Record<string, unknown>, { text: string }>({
      ...
      parameters: schema({
        what: enumStr(
          ['funnel', 'attribution', 'salary', 'box', 'baseline', 'resume', 'all'],
        ...
        q: str('薪资与基准的岗位关键词'),
        basis: enumStr(
          ['monthly_min', 'annualized'],
          '箱线图口径：月薪下限 / 年薪折算。**必须显式** —— 两个口径算出来的中位数不一样',
        ),
      }),
      ...textResult,
      async run(args) {
