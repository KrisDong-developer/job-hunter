...
function buildTools(runtime: HostRuntime): ToolDefinition[] {
  ...
  return [
    ...
    tool<Record<string, unknown>, { text: string }>({
      name: 'job_report',
      description:
        '数据看板：投递漏斗、渠道/简历版本归因、薪资分位。低危，只读。' +
      ...
      async run(args) {
        ...
        if (what === 'salary' || what === 'all') {
          const band = analytics.salaryBand({
            ...(asString(args['city']) === undefined ? {} : { city: asString(args['city']) as string }),
            ...(asString(args['q']) === undefined ? {} : { keyword: asString(args['q']) as string }),
          })
