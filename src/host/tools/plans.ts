 ...
 */
function describePlans(plans: PlanService, scheduler: SchedulerStatusDto): string {
  return [
    '现有方案：',
    ...plans.list().map((plan) => {
...
function configPatchOf(
  ...
  if (Array.isArray(args['keywords'])) {
    patch['keywords'] = args['keywords'].filter((item): item is string => typeof item === 'string')
  }
...
export function plansTools(runtime: HostRuntime): ToolDefinition[] {
  ...
  return [
    tool<Record<string, unknown>, { text: string; planId?: number }>({
      ...
      parameters: schema(
        {
          action: enumStr(
            ['list', 'create', 'update', 'enable', 'disable', 'remove', 'run', 'dimensions', 'pause', 'resume'],
                ...
                ...dimensions.map((dimension) => {
                  ...
                  const wiring = dimension.platforms
                    ...
                    .map((item) =>
                      item.wire === null
                        ? `${item.id}：不进请求`
                        : `${item.id}：${item.wire.target === 'url' ? 'URL' : '请求体'}${
                            item.wire.param === null ? '' : ` 参数 ${item.wire.param}`
                          }`,
                    )
                    .join('；')
                  return (
                    `· ${dimension.label}：${values} —— ${dimension.hint}` +
                    (wiring === '' ? '' : `\n    落到：${wiring}`) +
                    (dimension.conflict
