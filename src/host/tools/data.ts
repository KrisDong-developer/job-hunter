...
export function dataTools(runtime: HostRuntime): ToolDefinition[] {
      ...
      async run(args) {
        ...
        if (action === 'storage') {
          ...
          const lines = [
            ...
            ...usage.types.map(
              (type) =>
                `· ${type.label}：${String(type.rows)} 行 / ${formatBytes(type.bytes)}` +
        ...
        if (action === 'cleanup_preview') {
          ...
          const lines = [
            ...
            ...plan.items.map(
              (item) =>
                `· ${item.label}：${String(item.rows)} 行 / ${formatBytes(item.bytes)}${item.willRun ? '' : '（不执行）'}\n` +
                `  ${item.reason}\n  ${item.describe}`,
            ),
            '',
            plan.note,
        ...
        if (action === 'export') {
          ...
          return {
            text: [
              ...
              '',
              '归档内容：',
              ...saved.entries.slice(0, 10).map(
                (entry) => `· ${entry.name}：${formatBytes(entry.bytes)}${entry.rows === null ? '' : `（${String(entry.rows)} 行）`}`,
              ),
        ...
        if (action === 'import') {
          ...
          let payload: { format: 'csv' | 'json'; content: string }
          ...
          if (result.errors.length > 0) {
            lines.push('', '逐行问题：')
            for (const error of result.errors) lines.push(`· 第 ${String(error.row)} 行：${error.message}`)
            if (result.moreErrors > 0) lines.push(`· …还有 ${String(result.moreErrors)} 条未列出`)
          }
          return { text: lines.join('\n') }
        }
